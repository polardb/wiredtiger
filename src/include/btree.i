 ...
 */
static inline bool __wt_ref_is_root(WT_REF* ref) {
...
static inline bool __wt_page_evict_clean(WT_PAGE* page) {
    return (page->modify == NULL ||
            (page->modify->write_gen == 0 && page->modify->rec_result == 0));
}
 ...
 */
static inline int __wt_btree_block_free(WT_SESSION_IMPL* session,
                                        const uint8_t* addr,
                                        size_t addr_size) {
    WT_BM* bm;
    WT_BTREE* btree;
 ...
 */
static inline void __wt_cache_page_inmem_incr(WT_SESSION_IMPL* session,
                                              WT_PAGE* page,
                                              size_t size) {
    WT_BTREE* btree;
    WT_CACHE* cache;
 ...
 */
static inline void __wt_cache_decr_check_size(WT_SESSION_IMPL* session,
                                              size_t* vp,
                                              size_t v,
                                              const char* fld) {
 ...
 */
static inline void __wt_cache_decr_check_uint64(WT_SESSION_IMPL* session,
                                                uint64_t* vp,
                                                uint64_t v,
                                                const char* fld) {
    uint64_t orig = *vp;
 ...
 */
static inline void __wt_cache_page_byte_dirty_decr(WT_SESSION_IMPL* session,
                                                   WT_PAGE* page,
                                                   size_t size) {
    WT_BTREE* btree;
    WT_CACHE* cache;
 ...
 */
static inline void __wt_cache_page_inmem_decr(WT_SESSION_IMPL* session,
                                              WT_PAGE* page,
                                              size_t size) {
    WT_CACHE* cache;
    ...
    if (modify != NULL && modify->bytes_dirty != 0) {
        if (WT_PAGE_IS_INTERNAL(page)) {
            ...
            __wt_cache_decr_check_uint64(session,
                                         ...
                                         modify->bytes_dirty,
                                         "WT_CACHE.bytes_dirty_intl");
        } else if (!btree->lsm_primary) {
            __wt_cache_decr_check_uint64(session,
                                         &btree->bytes_dirty_leaf,
                                         modify->bytes_dirty,
                                         "WT_BTREE.bytes_dirty_leaf");
            __wt_cache_decr_check_uint64(session,
                                         &cache->bytes_dirty_leaf,
                                         modify->bytes_dirty,
                                         "WT_CACHE.bytes_dirty_leaf");
        }
    }
 ...
 */
static inline int __wt_page_parent_modify_set(WT_SESSION_IMPL* session,
                                              WT_REF* ref,
                                              bool page_only) {
    WT_PAGE* parent;
 ...
 */
static inline bool __wt_row_leaf_key_info(
 ...
 * possible.
 */
static inline int __wt_row_leaf_key(
 ...
 */
static inline void __wt_row_leaf_value_cell(WT_SESSION_IMPL* session,
                                            WT_PAGE* page,
                                            WT_ROW* rip,
    ...
    if (WT_READGEN_EVICT_SOON(page->read_gen) && btree->evict_disabled == 0 &&
        ...
        if (!__wt_page_evict_clean(page) &&
            (LF_ISSET(WT_READ_NO_SPLIT) ||
             (!inmem_split && F_ISSET(session, WT_SESSION_NO_RECONCILE)))) {
            if (!WT_SESSION_BTREE_SYNC(session))
 ...
 */
static inline bool __wt_split_descent_race(WT_SESSION_IMPL* session,
                                           WT_REF* ref,
                                           WT_PAGE_INDEX* saved_pindex) {
    WT_PAGE_INDEX* pindex;
 ...
 */
static inline int __wt_page_swap_func(WT_SESSION_IMPL* session,
                                      WT_REF* held,
                                      WT_REF* want,
                                      uint32_t flags
#ifdef HAVE_DIAGNOSTIC
