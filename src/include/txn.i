 ...
 */
static inline void __txn_resolve_prepared_update(WT_SESSION_IMPL* session, WT_UPDATE* upd) {
    WT_TXN* txn;
 ...
 */
static inline int __wt_txn_resolve_prepared_op(WT_SESSION_IMPL* session,
                                               WT_TXN_OP* op,
                                               bool commit) {
    WT_CURSOR* cursor;
    WT_DECL_RET;
 ...
 */
static inline void __wt_txn_op_apply_prepare_state(WT_SESSION_IMPL* session,
                                                   WT_REF* ref,
                                                   bool commit) {
    WT_TXN* txn;
    WT_UPDATE** updp;
 ...
 */
static inline bool __txn_visible_all_id(WT_SESSION_IMPL* session, uint64_t id) {
    uint64_t oldest_id;
 ...
 */
static inline bool __wt_txn_visible_all(WT_SESSION_IMPL* session,
                                        uint64_t id,
                                        const wt_timestamp_t timestamp) {
 ...
 */
static inline bool __txn_visible_id(WT_SESSION_IMPL* session, uint64_t id) {
    WT_TXN* txn;
    bool found;
 ...
 */
static inline bool __wt_txn_visible(WT_SESSION_IMPL* session,
                                    uint64_t id,
                                    const wt_timestamp_t timestamp) {
    WT_TXN* txn;
...
static inline WT_VISIBLE_TYPE __wt_txn_upd_visible_type(WT_SESSION_IMPL* session, WT_UPDATE* upd) {
    ...
    if (prepare_state == WT_PREPARE_INPROGRESS)
        return (F_ISSET(&session->txn, WT_TXN_IGNORE_PREPARE) ? WT_VISIBLE_FALSE
                                                              : WT_VISIBLE_PREPARE);
