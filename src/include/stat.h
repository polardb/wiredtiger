 ...
 */
static inline int64_t __wt_stats_aggregate(void* stats_arg, int slot) {
 ...
 */
static inline void __wt_stats_clear(void* stats_arg, int slot) {
    int64_t** stats;
    int i;
...
#define WT_STAT_MSECS_HIST_INCR_FUNC(name, stat, min_val)                         \
    static inline void __wt_stat_msecs_hist_incr_##name(WT_SESSION_IMPL* session, \
                                                        uint64_t msecs) {         \
        if (msecs < (min_val))                                                    \
            return;                                                               \
        if (msecs < 50)                                                           \
...
#define WT_STAT_USECS_HIST_INCR_FUNC(name, stat, min_val)                         \
    static inline void __wt_stat_usecs_hist_incr_##name(WT_SESSION_IMPL* session, \
                                                        uint64_t usecs) {         \
        if (usecs < (min_val))                                                    \
            return;                                                               \
        if (usecs < 250)                                                          \
