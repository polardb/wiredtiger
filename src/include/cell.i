...
struct __wt_cell {
     ...
     */
    uint8_t __chunk[1 + 1 + 3 * WT_INTPACK64_MAXSIZE + WT_INTPACK32_MAXSIZE];
};
 ...
 */
static inline void __wt_timestamp_value_check(WT_SESSION_IMPL* session,
 ...
 */
static inline void __cell_pack_timestamp_value(WT_SESSION_IMPL* session,
                                               uint8_t** pp,
                                               wt_timestamp_t start_ts,
                                               wt_timestamp_t stop_ts) {
     ...
     */
    if (!__wt_process.page_version_ts || (start_ts == WT_TS_NONE && stop_ts == WT_TS_MAX))
        ++*pp;
    else {
 ...
 */
static inline void __wt_timestamp_addr_check(WT_SESSION_IMPL* session,
 ...
 * address.
 */
static inline void __cell_pack_timestamp_addr(WT_SESSION_IMPL* session,
                                              uint8_t** pp,
                                              wt_timestamp_t oldest_start_ts,
                                              wt_timestamp_t newest_start_ts,
                                              wt_timestamp_t newest_stop_ts) {
 ...
 */
static inline size_t __wt_cell_pack_addr(WT_SESSION_IMPL* session,
                                         WT_CELL* cell,
                                         u_int cell_type,
                                         uint64_t recno,
                                         wt_timestamp_t oldest_start_ts,
    ...
    *p = '\0';
    ...
    __cell_pack_timestamp_addr(session, &p, oldest_start_ts, newest_start_ts, newest_stop_ts);
 ...
 */
static inline size_t __wt_cell_pack_value(WT_SESSION_IMPL* session,
                                          WT_CELL* cell,
                                          wt_timestamp_t start_ts,
                                          wt_timestamp_t stop_ts,
                                          uint64_t rle,
 ...
 */
static inline int __wt_cell_pack_value_match(WT_CELL* page_cell,
                                             WT_CELL* val_cell,
 ...
 */
static inline size_t __wt_cell_pack_copy(WT_SESSION_IMPL* session,
                                         WT_CELL* cell,
                                         wt_timestamp_t start_ts,
                                         wt_timestamp_t stop_ts,
                                         uint64_t rle,
 ...
 */
static inline size_t __wt_cell_pack_del(WT_SESSION_IMPL* session,
                                        WT_CELL* cell,
                                        wt_timestamp_t start_ts,
                                        wt_timestamp_t stop_ts,
                                        uint64_t rle) {
 ...
 */
static inline size_t __wt_cell_pack_ovfl(WT_SESSION_IMPL* session,
                                         WT_CELL* cell,
                                         uint8_t type,
                                         wt_timestamp_t start_ts,
                                         wt_timestamp_t stop_ts,
 ...
 */
static inline void __wt_cell_type_reset(WT_SESSION_IMPL* session,
                                        WT_CELL* cell,
                                        u_int old_type,
                                        u_int new_type) {
    /*
 ...
 */
static inline int __wt_cell_unpack_safe(WT_SESSION_IMPL* session,
...
restart:
    ...
    switch (unpack->raw) {
        ...
        case WT_CELL_ADDR_LEAF_NO:
            ...
            WT_RET(__wt_vunpack_uint(
                &p, end == NULL ? 0 : WT_PTRDIFF(end, p), &unpack->newest_start_ts));
            unpack->newest_start_ts += unpack->oldest_start_ts;
            WT_RET(__wt_vunpack_uint(
                &p, end == NULL ? 0 : WT_PTRDIFF(end, p), &unpack->newest_stop_ts));
            unpack->newest_stop_ts += unpack->newest_start_ts;

            __wt_timestamp_addr_check(
                session, unpack->oldest_start_ts, unpack->newest_start_ts, unpack->newest_stop_ts);
            break;
        case WT_CELL_DEL:
        case WT_CELL_VALUE:
        ...
        case WT_CELL_VALUE_OVFL_RM:
            ...
            __wt_timestamp_value_check(session, unpack->start_ts, unpack->stop_ts);
            break;
    }
 ...
 */
static inline void __wt_cell_unpack_dsk(WT_SESSION_IMPL* session,
                                        const WT_PAGE_HEADER* dsk,
                                        WT_CELL* cell,
                                        WT_CELL_UNPACK* unpack) {
    /*
 ...
 *	Unpack a WT_CELL into a structure.
 */
static inline void __wt_cell_unpack(WT_SESSION_IMPL* session,
                                    WT_PAGE* page,
                                    WT_CELL* cell,
                                    WT_CELL_UNPACK* unpack) {
 ...
 */
static inline int __cell_data_ref(WT_SESSION_IMPL* session,
                                  WT_PAGE* page,
                                  int page_type,
                                  WT_CELL_UNPACK* unpack,
                                  WT_ITEM* store) {
 ...
 */
static inline int __wt_dsk_cell_data_ref(WT_SESSION_IMPL* session,
                                         int page_type,
                                         WT_CELL_UNPACK* unpack,
                                         WT_ITEM* store) {
 ...
 *	Set a buffer to reference the data from an unpacked cell.
 */
static inline int __wt_page_cell_data_ref(WT_SESSION_IMPL* session,
                                          WT_PAGE* page,
                                          WT_CELL_UNPACK* unpack,
                                          WT_ITEM* store) {
 ...
 */
#define WT_CELL_FOREACH_BEGIN(session, btree, dsk, unpack, skip_ts)                          \
    do {                                                                                     \
        uint32_t __i;                                                                        \
        uint8_t* __cell;                                                                     \
        for (__cell = WT_PAGE_HEADER_BYTE(btree, dsk), __i = (dsk)->u.entries; __i > 0;      \
             ...
             * cells ourselves.                                                              \
             */                                                                              \
            if ((skip_ts) && (unpack).stop_ts != WT_TS_MAX && !__wt_process.page_version_ts) \
                continue;
