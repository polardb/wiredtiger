 ...
 */
static inline int __pack_initn(WT_SESSION_IMPL* session,
                               WT_PACK* pack,
                               const char* fmt,
                               size_t len) {
    if (*fmt == '@' || *fmt == '<' || *fmt == '>')
 ...
 */
static inline void __pack_name_init(WT_SESSION_IMPL* session,
...
next:
    ...
    switch (pv->type) {
        ...
        case 's':
            if (pv->size < 1)
                WT_RET_MSG(pack->session,
                           EINVAL,
                           "Fixed length strings must be at least 1 byte "
                           "in format '%.*s'",
                           (int)(pack->end - pack->orig),
                           pack->orig);
            return (0);
        case 'x':
            return (0);
 ...
 */
static inline int __pack_size(WT_SESSION_IMPL* session, WT_PACK_VALUE* pv, size_t* vp) {
    size_t s, pad;
 ...
 */
static inline int __pack_write(WT_SESSION_IMPL* session,
                               WT_PACK_VALUE* pv,
                               uint8_t** pp,
                               size_t maxlen) {
    size_t s, pad;
 ...
 */
static inline int __unpack_read(WT_SESSION_IMPL* session,
                                WT_PACK_VALUE* pv,
                                const uint8_t** pp,
                                size_t maxlen) {
    size_t s;
 ...
 */
static inline int __wt_struct_packv(
    WT_SESSION_IMPL* session, void* buffer, size_t size, const char* fmt, va_list ap) {
 ...
 */
static inline int __wt_struct_sizev(WT_SESSION_IMPL* session,
                                    size_t* sizep,
                                    const char* fmt,
                                    va_list ap) {
    WT_DECL_PACK_VALUE(pv);
 ...
 */
static inline int __wt_struct_unpackv(
