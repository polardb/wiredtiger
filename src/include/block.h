 ...
 */
static inline void __wt_block_desc_byteswap(WT_BLOCK_DESC* desc) {
 ...
 */
static inline void __wt_block_header_byteswap_copy(WT_BLOCK_HEADER* from, WT_BLOCK_HEADER* to) {
    *to = *from;
#ifdef WORDS_BIGENDIAN
 ...
 *	Handle big- and little-endian transformation of a header block.
 */
static inline void __wt_block_header_byteswap(WT_BLOCK_HEADER* blk) {
#ifdef WORDS_BIGENDIAN
 ...
 */
static inline u_int __wt_block_header(WT_BLOCK* block) {
    WT_UNUSED(block);
