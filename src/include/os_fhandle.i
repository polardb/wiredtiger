...
static inline int __wt_fsync(WT_SESSION_IMPL* session, WT_FH* fh, bool block) {
    ...
    if (block)
        ...
        ret = (handle->fh_sync_nowait == NULL ? 0 : handle->fh_sync_nowait(handle,
                                                                           (WT_SESSION*)session));
 ...
 *	POSIX pwrite.
 */
static inline int __wt_write(
    WT_SESSION_IMPL* session, WT_FH* fh, wt_off_t offset, size_t len, const void* buf) {
    WT_DECL_RET;
    uint64_t time_start, time_stop;
