 ...
 */
static inline void __wt_cache_update_lookaside_score(WT_SESSION_IMPL* session,
                                                     u_int updates_seen,
                                                     u_int updates_unstable) {
    WT_CACHE* cache;
 ...
 */
static inline bool __wt_eviction_needed(WT_SESSION_IMPL* session,
                                        bool busy,
                                        bool readonly,
                                        double* pct_fullp) {
    WT_CACHE* cache;
    ...
    if (pct_fullp != NULL)
        *pct_fullp = WT_MAX(0.0,
                            100.0 - WT_MIN(cache->eviction_trigger - pct_full,
 ...
 */
static inline int __wt_cache_eviction_check(WT_SESSION_IMPL* session,
                                            bool busy,
                                            bool readonly,
                                            bool* didworkp) {
    WT_BTREE* btree;
     ...
     */
    if (F_ISSET(session,
                WT_SESSION_IGNORE_CACHE_SIZE | WT_SESSION_LOCKED_HANDLE_LIST |
