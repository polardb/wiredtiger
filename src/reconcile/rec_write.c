...
struct __rec_chunk;
typedef struct __rec_chunk WT_CHUNK;
struct __rec_dictionary;
typedef struct __rec_dictionary WT_DICTIONARY;
struct __rec_kv;
typedef struct __rec_kv WT_KV;
...
static void __rec_cell_build_addr(WT_SESSION_IMPL*, WT_RECONCILE*, WT_ADDR*, bool, uint64_t);
static int __rec_cell_build_int_key(WT_SESSION_IMPL*, WT_RECONCILE*, const void*, size_t, bool*);
static int __rec_cell_build_leaf_key(WT_SESSION_IMPL*, WT_RECONCILE*, const void*, size_t, bool*);
static int __rec_cell_build_ovfl(
    WT_SESSION_IMPL*, WT_RECONCILE*, WT_KV*, uint8_t, wt_timestamp_t, wt_timestamp_t, uint64_t);
static int __rec_cell_build_val(
    WT_SESSION_IMPL*, WT_RECONCILE*, const void*, size_t, wt_timestamp_t, wt_timestamp_t, uint64_t);
static void __rec_cleanup(WT_SESSION_IMPL*, WT_RECONCILE*);
static int __rec_col_fix(WT_SESSION_IMPL*, WT_RECONCILE*, WT_REF*);
static int __rec_col_fix_slvg(WT_SESSION_IMPL*, WT_RECONCILE*, WT_REF*, WT_SALVAGE_COOKIE*);
...
static int __rec_col_var(WT_SESSION_IMPL*, WT_RECONCILE*, WT_REF*, WT_SALVAGE_COOKIE*);
...
static int __rec_split_write(WT_SESSION_IMPL*, WT_RECONCILE*, WT_CHUNK*, WT_ITEM*, bool);
 ...
 */
int __wt_reconcile(WT_SESSION_IMPL* session,
                   WT_REF* ref,
    ...
    switch (page->type) {
        ...
        case WT_PAGE_COL_INT:
            WT_WITH_PAGE_INDEX(session, ret = __rec_col_int(session, r, ref));
            break;
 ...
 */
static int __rec_write_check_complete(WT_SESSION_IMPL* session,
                                      WT_RECONCILE* r,
                                      int tret,
                                      bool* lookaside_retryp) {
    /*
 ...
 */
static void __rec_write_page_status(WT_SESSION_IMPL* session, WT_RECONCILE* r) {
    WT_BTREE* btree;
    WT_PAGE* page;
    WT_PAGE_MODIFY* mod;
 ...
 */
static int __rec_init(WT_SESSION_IMPL* session,
                      WT_REF* ref,
                      uint32_t flags,
                      WT_SALVAGE_COOKIE* salvage,
                      void* reconcilep) {
    ...
    if (btree->dictionary != 0 && btree->dictionary > r->dictionary_slots)
        WT_RET(
            __rec_dictionary_init(session, r, btree->dictionary < 100 ? 100 : btree->dictionary));
 ...
 *	across runs.
 */
static void __rec_cleanup(WT_SESSION_IMPL* session, WT_RECONCILE* r) {
    WT_BTREE* btree;
    WT_MULTI* multi;
    uint32_t i;
 ...
 */
static int __rec_update_save(WT_SESSION_IMPL* session,
                             WT_RECONCILE* r,
                             WT_INSERT* ins,
                             void* ripcip,
 ...
 */
static int __rec_append_orig_value(WT_SESSION_IMPL* session,
                                   WT_PAGE* page,
                                   WT_UPDATE* upd,
 ...
 *	be written).
 */
static int __rec_upd_select(WT_SESSION_IMPL* session,
                            WT_RECONCILE* r,
                            WT_INSERT* ins,
                            void* ripcip,
    ...
    for (upd = first_upd; upd != NULL; upd = upd->next) {
        ...
        if (F_ISSET(r, WT_REC_EVICT)) {
            ...
            if (upd->start_ts != WT_TS_NONE && !__wt_txn_upd_durable(session, upd)) {
                uncommitted = r->update_uncommitted = true;
                continue;
            }
        }
    ...
    if (upd != NULL) {
        ...
        if (upd_select->upd->start_ts == WT_TS_NONE) {
            ...
            upd_select->start_ts = upd_select->stop_ts = upd_select->upd->start_ts;
        ...
        if (upd_select->start_ts != WT_TS_NONE &&
            __wt_txn_visible_all(session, WT_TXN_NONE, upd_select->start_ts))
            upd_select->start_ts = WT_TS_NONE;
    }
...
check_original_value:
     ...
     */
    if (upd_select->upd != NULL &&
        (!WT_UPDATE_DATA_VALUE(upd_select->upd) || F_ISSET(r, WT_REC_LOOKASIDE) ||
 ...
 */
static int __rec_child_deleted(WT_SESSION_IMPL* session,
                               WT_RECONCILE* r,
                               WT_REF* ref,
                               WT_CHILD_STATE* statep) {
 ...
 */
static inline void __rec_image_copy(WT_SESSION_IMPL* session, WT_RECONCILE* r, WT_KV* kv) {
    size_t len;
    uint8_t *p, *t;
 ...
 */
static int __rec_dict_replace(WT_SESSION_IMPL* session,
                              WT_RECONCILE* r,
    ...
    if (dp->offset == 0)
        ...
        val->len = val->cell_len =
            __wt_cell_pack_copy(session, &val->cell, start_ts, stop_ts, rle, offset);
        val->buf.data = NULL;
        val->buf.size = 0;
    }
    return (0);
 ...
 */
static inline void __rec_key_state_update(WT_RECONCILE* r, bool ovfl_key) {
    WT_ITEM* a;
    ...
    switch (page->type) {
        case WT_PAGE_COL_FIX:
            ...
            page_size = (uint32_t)WT_ALIGN(
                WT_FIX_ENTRIES_TO_BYTES(btree, r->salvage->take + r->salvage->missing),
                btree->allocsize);
            break;
        case WT_PAGE_COL_VAR:
            /*
 ...
 */
static bool __rec_need_split(WT_RECONCILE* r, size_t len) {
    /*
 ...
 */
uint32_t __wt_split_page_size(int split_pct, uint32_t maxpagesize, uint32_t allocsize) {
    uintmax_t a;
    uint32_t split_size;
 ...
 */
static void __rec_addr_ts_init(WT_RECONCILE* r,
                               wt_timestamp_t* oldest_start_tsp,
                               wt_timestamp_t* newest_start_tsp,
                               wt_timestamp_t* newest_stop_tsp) {
    /*
 ...
 */
static inline void __rec_addr_ts_update(WT_RECONCILE* r,
                                        wt_timestamp_t oldest_start_ts,
                                        wt_timestamp_t newest_start_ts,
                                        wt_timestamp_t newest_stop_ts) {
    r->cur_ptr->oldest_start_ts = WT_MIN(oldest_start_ts, r->cur_ptr->oldest_start_ts);
    r->cur_ptr->newest_start_ts = WT_MAX(newest_start_ts, r->cur_ptr->newest_start_ts);
 ...
 */
static int __rec_split_chunk_init(WT_SESSION_IMPL* session,
                                  WT_RECONCILE* r,
                                  WT_CHUNK* chunk,
                                  size_t memsize) {
    ...
    chunk->key.size = 0;
    chunk->entries = 0;
    __rec_addr_ts_init(r, &chunk->oldest_start_ts, &chunk->newest_start_ts, &chunk->newest_stop_ts);
    ...
    chunk->min_entries = 0;
    __rec_addr_ts_init(
        r, &chunk->min_oldest_start_ts, &chunk->min_newest_start_ts, &chunk->min_newest_stop_ts);
    chunk->min_offset = 0;
 ...
 */
static bool __rec_is_checkpoint(WT_SESSION_IMPL* session, WT_RECONCILE* r) {
    WT_BTREE* btree;
 ...
 */
static int __rec_split_row_promote(WT_SESSION_IMPL* session,
                                   WT_RECONCILE* r,
                                   WT_ITEM* key,
                                   uint8_t type) {
    WT_BTREE* btree;
 ...
 * 	split.
 */
static inline int __rec_split_crossing_bnd(WT_SESSION_IMPL* session,
                                           WT_RECONCILE* r,
                                           size_t next_len) {
 ...
 * 	pointer.
 */
static int __rec_split_finish_process_prev(WT_SESSION_IMPL* session, WT_RECONCILE* r) {
    WT_BTREE* btree;
    ...
    if (combined_size <= r->page_size) {
        ...
        prev_ptr->newest_start_ts = WT_MAX(prev_ptr->newest_start_ts, cur_ptr->newest_start_ts);
    ...
    if (prev_ptr->min_offset != 0 && cur_ptr->image.size < r->min_split_size) {
        ...
        memmove(cur_dsk_start + len_to_move,
                cur_dsk_start,
                cur_ptr->image.size - WT_PAGE_HEADER_BYTE_SIZE(btree));
        memcpy(cur_dsk_start, (uint8_t*)r->prev_ptr->image.mem + prev_ptr->min_offset, len_to_move);
        ...
        cur_ptr->newest_start_ts = WT_MAX(prev_ptr->newest_start_ts, cur_ptr->newest_start_ts);
 ...
 *	block's list.
 */
static int __rec_supd_move(WT_SESSION_IMPL* session,
                           WT_MULTI* multi,
                           WT_SAVE_UPD* supd,
                           uint32_t n) {
    uint32_t i;
 ...
 */
static int __rec_split_write_supd(
    WT_SESSION_IMPL* session, WT_RECONCILE* r, WT_CHUNK* chunk, WT_MULTI* multi, bool last_block) {
    WT_BTREE* btree;
    WT_CHUNK* next;
    WT_DECL_ITEM(key);
    WT_DECL_RET;
 ...
 */
static void __rec_split_write_header(WT_SESSION_IMPL* session,
                                     WT_RECONCILE* r,
                                     WT_CHUNK* chunk,
                                     WT_MULTI* multi,
                                     WT_PAGE_HEADER* dsk) {
 ...
 */
static bool __rec_split_write_reuse(
 ...
 */
static inline void __rec_compression_adjust(WT_SESSION_IMPL* session,
                                            uint32_t max,
 ...
 */
static int __rec_split_write(WT_SESSION_IMPL* session,
                             WT_RECONCILE* r,
                             WT_CHUNK* chunk,
     ...
     */
    if (__rec_split_write_reuse(session,
                                r,
                                multi,
...
int __wt_bulk_init(WT_SESSION_IMPL* session, WT_CURSOR_BULK* cbulk) {
    ...
    return (__rec_split_init(session, r, cbulk->leaf, recno, btree->maxleafpage_precomp));
}
...
int __wt_bulk_insert_row(WT_SESSION_IMPL* session, WT_CURSOR_BULK* cbulk) {
    ...
    key = &r->k;
    val = &r->v;
    WT_RET(__rec_cell_build_leaf_key(session,
                                     r, /* Build key cell */
                                     cursor->key.data,
                                     cursor->key.size,
                                     &ovfl_key));
    ...
    if (WT_CROSSING_SPLIT_BND(r, key->len + val->len)) {
        ...
        WT_RET(__rec_split_crossing_bnd(session, r, key->len + val->len));
    }
    ...
    if (val->len == 0)
        ...
        if (btree->dictionary)
            WT_RET(__rec_dict_replace(session, r, WT_TS_NONE, WT_TS_MAX, 0, val));
 ...
 */
int __wt_bulk_insert_fix(WT_SESSION_IMPL* session, WT_CURSOR_BULK* cbulk, bool deleted) {
    WT_BTREE* btree;
    WT_CURSOR* cursor;
    WT_RECONCILE* r;
    ...
    __bit_setv(r->first_free,
               cbulk->entry,
               btree->bitcnt,
               deleted ? 0 : ((uint8_t*)cursor->value.data)[0]);
    ++cbulk->entry;
    ++r->recno;
 ...
 *	Fixed-length column-store bulk insert.
 */
int __wt_bulk_insert_fix_bitmap(WT_SESSION_IMPL* session, WT_CURSOR_BULK* cbulk) {
    WT_BTREE* btree;
    WT_CURSOR* cursor;
    WT_RECONCILE* r;
 ...
 */
int __wt_bulk_insert_var(WT_SESSION_IMPL* session, WT_CURSOR_BULK* cbulk, bool deleted) {
    WT_BTREE* btree;
    WT_KV* val;
    WT_RECONCILE* r;
    ...
    val = &r->v;
    if (deleted) {
        val->cell_len = __wt_cell_pack_del(session, &val->cell, WT_TS_NONE, WT_TS_MAX, cbulk->rle);
        val->buf.data = NULL;
        val->buf.size = 0;
        val->len = val->cell_len;
    } else
        ...
        WT_RET(__rec_cell_build_val(
            session, r, cbulk->last.data, cbulk->last.size, WT_TS_NONE, WT_TS_MAX, cbulk->rle));
    ...
    if (btree->dictionary)
        WT_RET(__rec_dict_replace(session, r, WT_TS_NONE, WT_TS_MAX, cbulk->rle, val));
    __rec_image_copy(session, r, val);
    __rec_addr_ts_update(r, WT_TS_NONE, WT_TS_NONE, WT_TS_MAX);
 ...
 */
static int __rec_col_int(WT_SESSION_IMPL* session, WT_RECONCILE* r, WT_REF* pageref) {
    WT_ADDR* addr;
    WT_BTREE* btree;
    ...
    WT_RET(__rec_split_init(session, r, page, pageref->ref_recno, btree->maxintlpage_precomp));
    ...
    WT_INTL_FOREACH_BEGIN(session, page, ref) {
            ...
            case WT_CHILD_MODIFIED:
                ...
                switch (child->modify->rec_result) {
                    ...
                    case WT_PM_REC_REPLACE:
                        ...
                        break;
                        WT_ILLEGAL_VALUE_ERR(session, child->modify->rec_result);
                }
                break;
        ...
        if (addr == NULL) {
            ...
            __rec_cell_build_addr(session, r, addr, false, ref->ref_recno);
        ...
        /* Copy the value onto the page. */
        __rec_image_copy(session, r, val);
        __rec_addr_ts_update(r, oldest_start_ts, newest_start_ts, newest_stop_ts);
    }
    WT_INTL_FOREACH_END;
...
static int __rec_col_merge(WT_SESSION_IMPL* session, WT_RECONCILE* r, WT_PAGE* page) {
    ...
    for (multi = mod->mod_multi, i = 0; i < mod->mod_multi_entries; ++multi, ++i) {
        ...
        /* Copy the value onto the page. */
        __rec_image_copy(session, r, val);
        __rec_addr_ts_update(r, addr->oldest_start_ts, addr->newest_start_ts, addr->newest_stop_ts);
    }
    return (0);
}
 ...
 */
static int __rec_col_fix(WT_SESSION_IMPL* session, WT_RECONCILE* r, WT_REF* pageref) {
    WT_BTREE* btree;
    WT_INSERT* ins;
    WT_PAGE* page;
    WT_UPDATE* upd;
    ...
    WT_RET(__rec_split_init(session, r, page, pageref->ref_recno, btree->maxleafpage));
    ...
    WT_SKIP_FOREACH(ins, WT_COL_UPDATE_SINGLE(page)) {
        WT_RET(__rec_upd_select(session, r, ins, NULL, NULL, &upd_select));
        upd = upd_select.upd;
        if (upd != NULL)
            __bit_setv(r->first_free,
 ...
 */
static int __rec_col_fix_slvg(WT_SESSION_IMPL* session,
                              WT_RECONCILE* r,
                              WT_REF* pageref,
                              WT_SALVAGE_COOKIE* salvage) {
    WT_BTREE* btree;
 ...
 *	page.
 */
static int __rec_col_var_helper(WT_SESSION_IMPL* session,
                                WT_RECONCILE* r,
                                WT_SALVAGE_COOKIE* salvage,
                                WT_ITEM* value,
                                wt_timestamp_t start_ts,
    ...
    if (deleted) {
        val->cell_len = __wt_cell_pack_del(session, &val->cell, start_ts, stop_ts, rle);
        val->buf.data = NULL;
        val->buf.size = 0;
        val->len = val->cell_len;
    } else if (overflow_type) {
        val->cell_len = __wt_cell_pack_ovfl(
            session, &val->cell, WT_CELL_VALUE_OVFL, start_ts, stop_ts, rle, value->size);
        val->buf.data = value->data;
        val->buf.size = value->size;
        val->len = val->cell_len + value->size;
    } else
        WT_RET(__rec_cell_build_val(session, r, value->data, value->size, start_ts, stop_ts, rle));
    ...
    if (!deleted && !overflow_type && btree->dictionary)
        WT_RET(__rec_dict_replace(session, r, start_ts, stop_ts, rle, val));
    __rec_image_copy(session, r, val);
 ...
 */
static int __rec_col_var(WT_SESSION_IMPL* session,
                         WT_RECONCILE* r,
                         WT_REF* pageref,
                         WT_SALVAGE_COOKIE* salvage) {
    ...
    WT_RET(__rec_split_init(session, r, page, pageref->ref_recno, btree->maxleafpage_precomp));
    ...
    if (salvage != NULL && salvage->missing != 0) {
        if (salvage->skip == 0) {
            ...
            WT_ERR(__rec_col_var_helper(
                session, r, NULL, NULL, WT_TS_NONE, WT_TS_MAX, salvage->missing, true, false));
    }
    ...
    record_loop: /*
        ...
        for (n = 0; n < nrepeat; n += repeat_count, src_recno += repeat_count) {
            ...
            if (ins != NULL && WT_INSERT_RECNO(ins) == src_recno) {
                WT_ERR(__rec_upd_select(session, r, ins, cip, vpack, &upd_select));
                upd = upd_select.upd;
                if (upd == NULL) {
                    /*
                     * TIMESTAMP-FIXME
        ...
        compare: /*
            ...
            if (rle != 0) {
                if ((!__wt_process.page_version_ts ||
                     (last.start_ts == start_ts && last.stop_ts == stop_ts)) &&
    ...
    for (ins = WT_SKIP_FIRST(WT_COL_APPEND(page));; ins = WT_SKIP_NEXT(ins)) {
        if (ins == NULL) {
            ...
            WT_ERR(__rec_upd_select(session, r, ins, NULL, NULL, &upd_select));
            upd = upd_select.upd;
...
static int __rec_row_int(WT_SESSION_IMPL* session, WT_RECONCILE* r, WT_PAGE* page) {
    ...
    WT_RET(__rec_split_init(session, r, page, 0, btree->maxintlpage_precomp));
    ...
    WT_INTL_FOREACH_BEGIN(session, page, ref) {
        ...
        if (__wt_off_page(page, addr)) {
            __rec_cell_build_addr(session, r, addr, state == WT_CHILD_PROXY, WT_RECNO_OOB);
            oldest_start_ts = addr->oldest_start_ts;
            newest_start_ts = addr->newest_start_ts;
            newest_stop_ts = addr->newest_stop_ts;
        } else {
        ...
        if (__rec_need_split(r, key->len + val->len)) {
            ...
            WT_ERR(__rec_split_crossing_bnd(session, r, key->len + val->len));
        }
...
static int __rec_row_merge(WT_SESSION_IMPL* session, WT_RECONCILE* r, WT_PAGE* page) {
    ...
    for (multi = mod->mod_multi, i = 0; i < mod->mod_multi_entries; ++multi, ++i) {
        ...
        WT_RET(__rec_cell_build_int_key(session,
                                        r,
 ...
 */
static bool __rec_row_zero_len(WT_SESSION_IMPL* session,
                               uint64_t txnid,
                               wt_timestamp_t start_ts,
                               wt_timestamp_t stop_ts) {
     ...
     * on the page.
     */
    return ((start_ts == WT_TS_NONE || __wt_txn_visible_all(session, txnid, start_ts)) &&
            stop_ts == WT_TS_MAX);
}
 ...
 */
static int __rec_row_leaf(WT_SESSION_IMPL* session,
                          WT_RECONCILE* r,
                          WT_PAGE* page,
                          WT_SALVAGE_COOKIE* salvage) {
    WT_BTREE* btree;
    ...
    WT_RET(__rec_split_init(session, r, page, 0, btree->maxleafpage_precomp));
    ...
    WT_ROW_FOREACH(page, rip, i) {
        ...
        /* Look for an update. */
        WT_ERR(__rec_upd_select(session, r, NULL, rip, vpack, &upd_select));
        ...
        if (upd == NULL) {
            ...
            if (vpack->raw == WT_CELL_VALUE_COPY) {
                ...
                WT_ERR(__rec_cell_build_val(session, r, p, size, start_ts, stop_ts, 0));
                dictionary = true;
            } else if (vpack->raw == WT_CELL_VALUE_OVFL_RM) {
                /*
                 * If doing an update save and restore, and the
                ...
                WT_ERR(__rec_cell_build_val(
                    session, r, "ovfl-unused", strlen("ovfl-unused"), start_ts, stop_ts, 0));
            } else {
            ...
            switch (upd->type) {
                ...
                case WT_UPDATE_STANDARD:
                    ...
                    WT_ERR(__rec_cell_build_val(
                        session, r, upd->data, upd->size, start_ts, stop_ts, 0));
                    dictionary = true;
                    break;
                case WT_UPDATE_TOMBSTONE:
                    /*
        ...
        /* Copy the key/value pair onto the page. */
        __rec_image_copy(session, r, key);
        if (val->len == 0 && __rec_row_zero_len(session, txnid, start_ts, stop_ts))
            r->any_empty_value = true;
        else {
            r->all_empty_value = false;
            if (dictionary && btree->dictionary)
                WT_ERR(__rec_dict_replace(session, r, start_ts, stop_ts, 0, val));
            __rec_image_copy(session, r, val);
        }
        __rec_addr_ts_update(r, start_ts, start_ts, stop_ts);
...
static int __rec_row_leaf_insert(WT_SESSION_IMPL* session, WT_RECONCILE* r, WT_INSERT* ins) {
    ...
    for (; ins != NULL; ins = WT_SKIP_NEXT(ins)) {
        ...
        if (upd == NULL) {
            ...
            WT_RET(__rec_split_crossing_bnd(session, r, WT_INSERT_KEY_SIZE(ins)));
        ...
        switch (upd->type) {
            ...
            case WT_UPDATE_STANDARD:
                /* Take the value from the update. */
                WT_RET(
                    __rec_cell_build_val(session, r, upd->data, upd->size, start_ts, stop_ts, 0));
                break;
            case WT_UPDATE_TOMBSTONE:
                continue;
        ...
        if (val->len == 0 && __rec_row_zero_len(session, txnid, start_ts, stop_ts))
            ...
            r->all_empty_value = false;
            if (btree->dictionary)
                WT_RET(__rec_dict_replace(session, r, start_ts, stop_ts, 0, val));
            __rec_image_copy(session, r, val);
        }
        __rec_addr_ts_update(r, start_ts, start_ts, stop_ts);
 ...
 */
static int __rec_split_dump_keys(WT_SESSION_IMPL* session, WT_RECONCILE* r) {
    WT_BTREE* btree;
    WT_DECL_ITEM(tkey);
    WT_MULTI* multi;
    uint32_t i;
...
static int __rec_write_wrapup(WT_SESSION_IMPL* session, WT_RECONCILE* r, WT_PAGE* page) {
    ...
    switch (mod->rec_result) {
        ...
        case WT_PM_REC_REPLACE: /* 1-for-1 page swap */
            ...
            if (!__wt_ref_is_root(ref))
                WT_RET(
                    __wt_btree_block_free(session, mod->mod_replace.addr, mod->mod_replace.size));
    ...
    switch (r->multi_next) {
        case 0: /* Page delete */
            ...
            if (__wt_ref_is_root(ref)) {
                ...
                __wt_checkpoint_tree_reconcile_update(session, WT_TS_NONE, WT_TS_NONE, WT_TS_MAX);
            }
 ...
 */
static int __rec_las_wrapup(WT_SESSION_IMPL* session, WT_RECONCILE* r) {
    WT_CURSOR* cursor;
    WT_DECL_ITEM(key);
    WT_DECL_RET;
    WT_MULTI* multi;
 ...
 */
static int __rec_las_wrapup_err(WT_SESSION_IMPL* session, WT_RECONCILE* r) {
    WT_DECL_RET;
    WT_MULTI* multi;
    uint64_t las_pageid;
    uint32_t i;
 ...
 */
static int __rec_cell_build_int_key(
    ...
    *is_ovflp = false;
    ...
    if (size > btree->maxintlkey) {
        ...
        *is_ovflp = true;
        return (
            __rec_cell_build_ovfl(session, r, key, WT_CELL_KEY_OVFL, WT_TS_NONE, WT_TS_NONE, 0));
    }
 ...
 */
static int __rec_cell_build_leaf_key(
    WT_SESSION_IMPL* session, WT_RECONCILE* r, const void* data, size_t size, bool* is_ovflp) {
    WT_BTREE* btree;
    WT_KV* key;
    size_t pfx_max;
        ...
        if (pfx == 0) {
            ...
            *is_ovflp = true;
            return (__rec_cell_build_ovfl(
                session, r, key, WT_CELL_KEY_OVFL, WT_TS_NONE, WT_TS_NONE, 0));
        }
...
static void __rec_cell_build_addr(
    ...
    val->cell_len = __wt_cell_pack_addr(session,
                                        ...
                                        cell_type,
                                        recno,
                                        addr->oldest_start_ts,
                                        addr->newest_start_ts,
                                        addr->newest_stop_ts,
                                        val->buf.size);
 ...
 */
static int __rec_cell_build_val(WT_SESSION_IMPL* session,
                                WT_RECONCILE* r,
                                const void* data,
                                size_t size,
                                wt_timestamp_t start_ts,
    ...
    if (size != 0) {
        ...
        if (val->buf.size > btree->maxleafvalue) {
            ...
            return (
                __rec_cell_build_ovfl(session, r, val, WT_CELL_VALUE_OVFL, start_ts, stop_ts, rle));
        }
    }
    val->cell_len =
        __wt_cell_pack_value(session, &val->cell, start_ts, stop_ts, rle, val->buf.size);
    val->len = val->cell_len + val->buf.size;
 ...
 */
static int __rec_cell_build_ovfl(WT_SESSION_IMPL* session,
                                 WT_RECONCILE* r,
                                 WT_KV* kv,
                                 uint8_t type,
                                 wt_timestamp_t start_ts,
    ...
    kv->cell_len =
        __wt_cell_pack_ovfl(session, &kv->cell, type, start_ts, stop_ts, rle, kv->buf.size);
 ...
 */
static void __rec_dictionary_skip_search_stack(WT_DICTIONARY** head,
                                               WT_DICTIONARY*** stack,
                                               uint64_t hash) {
    WT_DICTIONARY** e;
    int i;
...
static int __rec_dictionary_init(WT_SESSION_IMPL* session, WT_RECONCILE* r, u_int slots) {
    ...
    for (i = 0; i < r->dictionary_slots; ++i) {
        ...
        WT_RET(__wt_calloc(
            session, 1, sizeof(WT_DICTIONARY) + depth * sizeof(WT_DICTIONARY*), &r->dictionary[i]));
 ...
 */
static int __rec_dictionary_lookup(WT_SESSION_IMPL* session,
                                   WT_RECONCILE* r,
                                   WT_KV* val,
                                   WT_DICTIONARY** dpp) {
    ...
    *dpp = NULL;
    ...
    for (dp = __rec_dictionary_skip_search(r->dictionary_head, hash);
         dp != NULL && dp->hash == hash;
         dp = dp->next[0]) {
        WT_RET(__wt_cell_pack_value_match((WT_CELL*)((uint8_t*)r->cur_ptr->image.mem + dp->offset),
                                          &val->cell,
                                          val->buf.data,
                                          &match));
        if (match) {
