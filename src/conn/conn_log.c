 ...
 */
static int __logmgr_force_archive(WT_SESSION_IMPL* session, uint32_t lognum) {
    WT_CONNECTION_IMPL* conn;
    WT_LOG* log;
 ...
 */
static int __logmgr_version(WT_SESSION_IMPL* session, bool reconfig) {
    WT_CONNECTION_IMPL* conn;
    WT_LOG* log;
 ...
 */
static int __logmgr_config(WT_SESSION_IMPL* session, const char** cfg, bool* runp, bool reconfig) {
    WT_CONFIG_ITEM cval;
    WT_CONNECTION_IMPL* conn;
    bool enabled;
 ...
 *	Reconfigure logging.
 */
int __wt_logmgr_reconfig(WT_SESSION_IMPL* session, const char** cfg) {
    bool dummy;
...
static int __log_prealloc_once(WT_SESSION_IMPL* session) {
    ...
    if (log->prep_missed > 0) {
        ...
        __wt_verbose(session,
                     WT_VERB_LOG,
                     "Adjust down. Did not use %" PRIu32 ". Now pre-allocating %" PRIu32,
                     reccount,
                     conn->log_prealloc);
    }
 ...
 *	currently running.
 */
int __wt_log_truncate_files(WT_SESSION_IMPL* session, WT_CURSOR* cursor, bool force) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_LOG* log;
 ...
 */
static WT_THREAD_RET __log_file_server(void* arg) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_FH* close_fh;
    WT_LOG* log;
...
#define WT_WRLSN_ENTRY_CMP_LT(entry1, entry2)                                                    \
    ((entry1).lsn.l.file < (entry2).lsn.l.file || ((entry1).lsn.l.file == (entry2).lsn.l.file && \
 ...
 */
void __wt_log_wrlsn(WT_SESSION_IMPL* session, int* yield) {
    WT_CONNECTION_IMPL* conn;
    WT_LOG* log;
