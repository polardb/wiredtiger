 ...
 */
static int ext_collate(WT_EXTENSION_API* wt_api,
 ...
 */
static int ext_collator_config(WT_EXTENSION_API* wt_api,
                               WT_SESSION* wt_session,
                               const char* uri,
 ...
 */
static int __collator_confchk(WT_SESSION_IMPL* session,
 ...
 */
int __wt_collator_config(WT_SESSION_IMPL* session,
                         const char* uri,
                         WT_CONFIG_ITEM* cname,
 ...
 */
static int __conn_add_collator(WT_CONNECTION* wt_conn,
                               const char* name,
                               WT_COLLATOR* collator,
                               const char* config) {
    WT_CONNECTION_IMPL* conn;
 ...
 */
static int __compressor_confchk(WT_SESSION_IMPL* session,
                                WT_CONFIG_ITEM* cval,
 ...
 */
int __wt_compressor_config(WT_SESSION_IMPL* session,
                           WT_CONFIG_ITEM* cval,
                           WT_COMPRESSOR** compressorp) {
 ...
 */
static int __conn_add_compressor(WT_CONNECTION* wt_conn,
                                 const char* name,
 ...
 */
static int __conn_add_data_source(WT_CONNECTION* wt_conn,
                                  const char* prefix,
                                  WT_DATA_SOURCE* dsrc,
                                  const char* config) {
    WT_CONNECTION_IMPL* conn;
 ...
 */
int __wt_conn_remove_data_source(WT_SESSION_IMPL* session) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_NAMED_DATA_SOURCE* ndsrc;
 ...
 */
static int __encryptor_confchk(WT_SESSION_IMPL* session,
                               WT_CONFIG_ITEM* cval,
 ...
 */
int __wt_encryptor_config(WT_SESSION_IMPL* session,
                          WT_CONFIG_ITEM* cval,
 ...
 */
static int __conn_add_encryptor(WT_CONNECTION* wt_conn,
                                const char* name,
 ...
 * internally.
 */
int __wt_conn_remove_encryptor(WT_SESSION_IMPL* session) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_KEYED_ENCRYPTOR* kenc;
    WT_NAMED_ENCRYPTOR* nenc;
 ...
 */
static int __conn_add_extractor(WT_CONNECTION* wt_conn,
                                const char* name,
 ...
 */
static int __extractor_confchk(WT_SESSION_IMPL* session,
                               WT_CONFIG_ITEM* cname,
 ...
 */
int __wt_extractor_config(WT_SESSION_IMPL* session,
                          const char* uri,
                          const char* config,
                          WT_EXTRACTOR** extractorp,
                          int* ownp) {
 ...
 */
static int __conn_async_new_op(WT_CONNECTION* wt_conn,
                               const char* uri,
                               const char* config,
 ...
 */
static int __conn_builtin_init(WT_CONNECTION_IMPL* conn,
                               const char* name,
 ...
 */
static int __conn_builtin_extensions(WT_CONNECTION_IMPL* conn, const char* cfg[]) {
#ifdef HAVE_BUILTIN_EXTENSION_LZ4
 ...
 */
static int __conn_load_extension_int(WT_SESSION_IMPL* session,
                                     const char* path,
                                     const char* cfg[],
                                     bool early_load) {
    WT_CONFIG_ITEM cval;
 ...
 */
static int __conn_load_extensions(WT_SESSION_IMPL* session, const char* cfg[], bool early_load) {
    WT_CONFIG subconfig;
 ...
 */
static int __conn_configure_method(WT_CONNECTION* wt_conn,
                                   const char* method,
                                   const char* uri,
                                   const char* config,
                                   const char* type,
 ...
 */
static int __conn_debug_info(WT_CONNECTION* wt_conn, const char* config) {
    WT_CONFIG_ITEM cval;
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __conn_open_session(WT_CONNECTION* wt_conn,
 ...
 */
static int __conn_query_timestamp(WT_CONNECTION* wt_conn, char* hex_timestamp, const char* config) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __conn_set_timestamp(WT_CONNECTION* wt_conn, const char* config) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __conn_rollback_to_stable(WT_CONNECTION* wt_conn, const char* config) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static void __conn_config_readonly(const char* cfg[]) {
    const char* readonly;
 ...
 */
static int __conn_env_var(WT_SESSION_IMPL* session,
                          const char* cfg[],
                          const char* name,
                          const char** configp) {
    WT_CONFIG_ITEM cval;
...
static int __conn_single(WT_SESSION_IMPL* session, const char* cfg[]) {
    ...
    ret = __wt_open(session,
                    ...
                    is_create || exist ? WT_FS_OPEN_CREATE : 0,
                    &conn->lock_fh);
...
int __wt_verbose_config(WT_SESSION_IMPL* session, const char* cfg[]) {
    static const WT_NAME_FLAG verbtypes[] = {{"api", WT_VERB_API},
                                             {"block", WT_VERB_BLOCK},
 ...
 */
int __wt_verbose_dump_sessions(WT_SESSION_IMPL* session, bool show_cursors) {
    WT_CONNECTION_IMPL* conn;
    WT_CURSOR* cursor;
    WT_DECL_ITEM(buf);
    WT_DECL_RET;
    ...
    WT_RET(__wt_msg(session,
                    "Active sessions: %" PRIu32 " Max: %" PRIu32,
                    conn->session_cnt,
                    conn->session_size));
    ...
    for (s = conn->sessions, i = 0; i < conn->session_cnt; ++s, ++i) {
        ...
        if (!show_cursors) {
            ...
            WT_ERR(__wt_msg(
                session, "  Backup in progress: %s", s->bkp_cursor == NULL ? "no" : "yes"));
            WT_ERR(
                __wt_msg(session,
                         "  Compact state: %s",
 ...
 */
int __wt_timing_stress_config(WT_SESSION_IMPL* session, const char* cfg[]) {
    /*
...
static int __conn_write_base_config(WT_SESSION_IMPL* session, const char* cfg[]) {
    ...
    WT_RET(__wt_fopen(session,
                      WT_BASECONFIG_SET,
                      WT_FS_OPEN_CREATE | WT_FS_OPEN_EXCLUSIVE,
                      WT_STREAM_WRITE,
                      &fs));
 ...
 */
static int __conn_set_file_system(WT_CONNECTION* wt_conn,
 ...
 */
static int __conn_session_size(WT_SESSION_IMPL* session, const char* cfg[], uint32_t* vp) {
    WT_CONFIG_ITEM cval;
    int64_t v;
 ...
 */
static int __conn_chk_file_system(WT_SESSION_IMPL* session, bool readonly) {
    WT_CONNECTION_IMPL* conn;
 ...
 */
static void wiredtiger_dummy_session_init(WT_CONNECTION_IMPL* conn,
                                          WT_EVENT_HANDLER* event_handler) {
    WT_SESSION_IMPL* session;
...
int wiredtiger_open(const char* home,
    ...
    static const WT_CONNECTION stdc = {
        __conn_async_flush,      __conn_async_new_op,     __conn_close,
        __conn_debug_info,       __conn_reconfigure,      __conn_get_home,
        __conn_configure_method, __conn_is_new,           __conn_open_session,
        __conn_query_timestamp,  __conn_set_timestamp,    __conn_rollback_to_stable,
        __conn_load_extension,   __conn_add_data_source,  __conn_add_collator,
        __conn_add_compressor,   __conn_add_encryptor,    __conn_add_extractor,
            ...
            switch (ft->flag) {
                ...
                case WT_DIRECT_IO_LOG:
                    ...
                    if (sval.val == 1)
                        ...
                        WT_ERR_MSG(
                            session, EINVAL, "invalid log extend length: %" PRId64, sval.val);
                    break;
            }
        } else
            WT_ERR_NOTFOUND_OK(ret);
