 ...
 */
static int __capacity_config(WT_SESSION_IMPL* session, const char* cfg[]) {
    WT_CAPACITY* cap;
    WT_CONFIG_ITEM cval;
 ...
 */
static bool __capacity_server_run_chk(WT_SESSION_IMPL* session) {
 ...
 */
static WT_THREAD_RET __capacity_server(void* arg) {
    WT_CAPACITY* cap;
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
 ...
 */
static int __capacity_server_start(WT_CONNECTION_IMPL* conn) {
    WT_SESSION_IMPL* session;
 ...
 */
int __wt_capacity_server_create(WT_SESSION_IMPL* session, const char* cfg[]) {
    WT_CONNECTION_IMPL* conn;
 ...
 */
int __wt_capacity_server_destroy(WT_SESSION_IMPL* session) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
 ...
 */
static void __capacity_signal(WT_SESSION_IMPL* session) {
    WT_CAPACITY* cap;
    WT_CONNECTION_IMPL* conn;
 ...
 */
static void __capacity_reserve(
 ...
 */
void __wt_capacity_throttle(WT_SESSION_IMPL* session, uint64_t bytes, WT_THROTTLE_TYPE type) {
    struct timespec now;
    WT_CAPACITY* cap;
    WT_CONNECTION_IMPL* conn;
