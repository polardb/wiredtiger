 ...
 *	handles.
 */
static void __sweep_mark(WT_SESSION_IMPL* session, uint64_t now) {
 ...
 */
static int __sweep_expire_one(WT_SESSION_IMPL* session) {
    WT_BTREE* btree;
    WT_DATA_HANDLE* dhandle;
    WT_DECL_RET;
 ...
 */
static int __sweep_expire(WT_SESSION_IMPL* session, uint64_t now) {
    WT_CONNECTION_IMPL* conn;
    WT_DATA_HANDLE* dhandle;
    WT_DECL_RET;
 ...
 */
static int __sweep_discard_trees(WT_SESSION_IMPL* session, u_int* dead_handlesp) {
    WT_CONNECTION_IMPL* conn;
    WT_DATA_HANDLE* dhandle;
    WT_DECL_RET;
 ...
 */
static int __sweep_remove_one(WT_SESSION_IMPL* session, WT_DATA_HANDLE* dhandle) {
    WT_DECL_RET;
        ...
        if (dhandle->type == WT_DHANDLE_TYPE_TABLE)
            WT_WITH_TABLE_WRITE_LOCK(session,
                                     WT_WITH_HANDLE_LIST_WRITE_LOCK(
                                         session, ret = __sweep_remove_one(session, dhandle)));
        else
 ...
 */
static bool __sweep_server_run_chk(WT_SESSION_IMPL* session) {
