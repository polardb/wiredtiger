 ...
 */
static inline int __cache_config_abs_to_pct(WT_SESSION_IMPL* session,
                                            double* param,
 ...
 */
static int __cache_config_local(WT_SESSION_IMPL* session, bool shared, const char* cfg[]) {
    WT_CACHE* cache;
    WT_CONFIG_ITEM cval;
    WT_CONNECTION_IMPL* conn;
 ...
 */
int __wt_cache_config(WT_SESSION_IMPL* session, bool reconfigure, const char* cfg[]) {
    WT_CONFIG_ITEM cval;
    WT_CONNECTION_IMPL* conn;
     ...
     */
    if (reconfigure)
        WT_RET(__wt_thread_group_resize(session,
                                        &conn->evict_threads,
