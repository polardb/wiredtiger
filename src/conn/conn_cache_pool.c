...
int __wt_cache_pool_config(WT_SESSION_IMPL* session, const char** cfg) {
    ...
    __wt_verbose(session,
                 ...
                 "Configured cache pool %s. Size: %" PRIu64 ", chunk size: %" PRIu64,
                 cp->name,
                 cp->size,
                 cp->chunk);
 ...
 *	effectively used.
 */
static void __cache_pool_balance(WT_SESSION_IMPL* session, bool forward) {
    WT_CACHE_POOL* cp;
 ...
 */
static void __cache_pool_assess(WT_SESSION_IMPL* session, uint64_t* phighest) {
    WT_CACHE* cache;
    WT_CACHE_POOL* cp;
 ...
 */
static void __cache_pool_adjust(WT_SESSION_IMPL* session,
                                uint64_t highest,
                                uint64_t bump_threshold,
                                bool forward,
                                bool* adjustedp) {
    WT_CACHE* cache;
    WT_CACHE_POOL* cp;
        ...
        if (adjustment > 0) {
            *adjustedp = true;
            if (grow) {
                ...
                WT_ASSERT(session,
                          entry->cache_size >= adjustment && cp->currently_used >= adjustment);
                entry->cache_size -= adjustment;
                cp->currently_used -= adjustment;
            }
            __wt_verbose(session,
