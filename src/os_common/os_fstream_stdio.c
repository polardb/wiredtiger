 ...
 *	ANSI C close/fclose.
 */
static int __stdio_close(WT_SESSION_IMPL* session, WT_FSTREAM* fs) {
 ...
 *	POSIX fflush.
 */
static int __stdio_flush(WT_SESSION_IMPL* session, WT_FSTREAM* fs) {
    if (fflush(fs->fp) == 0)
        return (0);
 ...
 *	ANSI C getline.
 */
static int __stdio_getline(WT_SESSION_IMPL* session, WT_FSTREAM* fs, WT_ITEM* buf) {
    WT_UNUSED(buf);
 ...
 *	ANSI C vfprintf.
 */
static int __stdio_printf(WT_SESSION_IMPL* session, WT_FSTREAM* fs, const char* fmt, va_list ap) {
 ...
 */
static void __stdio_init(WT_FSTREAM* fs, const char* name, FILE* fp) {
    fs->name = name;
    fs->fp = fp;
 ...
 */
void __wt_os_stdio(WT_SESSION_IMPL* session) {
