 ...
 */
int __wt_filename_construct(WT_SESSION_IMPL* session,
                            const char* path,
 ...
 */
int __wt_remove_if_exists(WT_SESSION_IMPL* session, const char* name, bool durable) {
    bool exist;
 ...
 *	Copy a file safely.
 */
int __wt_copy_and_sync(WT_SESSION* wt_session, const char* from, const char* to)
