 ...
 */
static WT_FILE_HANDLE_INMEM* __im_handle_search(WT_FILE_SYSTEM* file_system, const char* name) {
 ...
 *	shutdown.
 */
static int __im_handle_remove(WT_SESSION_IMPL* session,
 ...
 */
static int __im_fs_directory_list(WT_FILE_SYSTEM* file_system,
 ...
 */
static int __im_fs_directory_list_free(WT_FILE_SYSTEM* file_system,
                                       WT_SESSION* wt_session,
                                       char** dirlist,
                                       uint32_t count) {
    WT_SESSION_IMPL* session;
 ...
 */
static int __im_fs_exist(WT_FILE_SYSTEM* file_system,
                         WT_SESSION* wt_session,
                         const char* name,
                         bool* existp) {
    WT_FILE_SYSTEM_INMEM* im_fs;
 ...
 *	POSIX remove.
 */
static int __im_fs_remove(WT_FILE_SYSTEM* file_system,
                          WT_SESSION* wt_session,
                          const char* name,
                          uint32_t flags) {
    WT_DECL_RET;
 ...
 *	POSIX rename.
 */
static int __im_fs_rename(WT_FILE_SYSTEM* file_system,
                          WT_SESSION* wt_session,
                          const char* from,
                          const char* to,
                          uint32_t flags) {
 ...
 */
static int __im_fs_size(WT_FILE_SYSTEM* file_system,
                        WT_SESSION* wt_session,
                        const char* name,
                        wt_off_t* sizep) {
    WT_DECL_RET;
 ...
 *	ANSI C close.
 */
static int __im_file_close(WT_FILE_HANDLE* file_handle, WT_SESSION* wt_session) {
    WT_FILE_HANDLE_INMEM* im_fh;
    WT_FILE_SYSTEM_INMEM* im_fs;
    WT_SESSION_IMPL* session;
 ...
 *	Lock/unlock a file.
 */
static int __im_file_lock(WT_FILE_HANDLE* file_handle, WT_SESSION* wt_session, bool lock) {
 ...
 */
static int __im_file_size(WT_FILE_HANDLE* file_handle, WT_SESSION* wt_session, wt_off_t* sizep) {
    WT_FILE_HANDLE_INMEM* im_fh;
    WT_FILE_SYSTEM_INMEM* im_fs;
    WT_SESSION_IMPL* session;
 ...
 *	In-memory sync.
 */
static int __im_file_sync(WT_FILE_HANDLE* file_handle, WT_SESSION* wt_session) {
    WT_UNUSED(file_handle);
    WT_UNUSED(wt_session);
    return (0);
}
 ...
 *	POSIX pwrite.
 */
static int __im_file_write(WT_FILE_HANDLE* file_handle,
                           WT_SESSION* wt_session,
                           wt_off_t offset,
                           size_t len,
                           const void* buf) {
...
err:
    ...
    WT_RET_MSG(session,
               ret,
               "%s: handle-write: failed to write %" WT_SIZET_FMT
               " bytes at "
               "offset %" WT_SIZET_FMT,
               file_handle->name,
               len,
 ...
 *	POSIX fopen/open.
 */
static int __im_file_open(WT_FILE_SYSTEM* file_system,
                          WT_SESSION* wt_session,
                          const char* name,
 ...
 */
static int __im_terminate(WT_FILE_SYSTEM* file_system, WT_SESSION* wt_session) {
    WT_DECL_RET;
 ...
 */
int __wt_os_inmemory(WT_SESSION_IMPL* session) {
    WT_DECL_RET;
    WT_FILE_SYSTEM* file_system;
    WT_FILE_SYSTEM_INMEM* im_fs;
    u_int i;
