 ...
 */
static int __fstream_close(WT_SESSION_IMPL* session, WT_FSTREAM* fstr) {
    WT_DECL_RET;
 ...
 */
static int __fstream_flush(WT_SESSION_IMPL* session, WT_FSTREAM* fstr) {
    if (fstr->buf.size > 0) {
 ...
 */
static int __fstream_flush_notsup(WT_SESSION_IMPL* session, WT_FSTREAM* fstr) {
 ...
 */
static int __fstream_getline(WT_SESSION_IMPL* session, WT_FSTREAM* fstr, WT_ITEM* buf) {
    size_t len;
    char c;
    const char* p;
 ...
 */
static int __fstream_getline_notsup(WT_SESSION_IMPL* session, WT_FSTREAM* fstr, WT_ITEM* buf) {
    WT_UNUSED(buf);
 ...
 *	ANSI C vfprintf.
 */
static int __fstream_printf(WT_SESSION_IMPL* session,
                            WT_FSTREAM* fstr,
                            const char* fmt,
                            va_list ap) {
    WT_ITEM* buf;
 ...
 */
static int __fstream_printf_notsup(WT_SESSION_IMPL* session,
                                   WT_FSTREAM* fstr,
                                   const char* fmt,
                                   va_list ap) {
    WT_UNUSED(fmt);
 ...
 */
int __wt_fopen(WT_SESSION_IMPL* session,
               const char* name,
               uint32_t open_flags,
               uint32_t flags,
               WT_FSTREAM** fstrp) {
