 ...
 */
static int __fhandle_method_finalize(WT_SESSION_IMPL* session,
 ...
 */
bool __wt_handle_is_open(WT_SESSION_IMPL* session, const char* name) {
 ...
 */
static bool __handle_search(WT_SESSION_IMPL* session, const char* name, WT_FH* newfh, WT_FH** fhp) {
    WT_CONNECTION_IMPL* conn;
    WT_FH* fh;
    uint64_t bucket, hash;
    bool found;
 ...
 */
static const char* __open_verbose_file_type_tag(WT_FS_OPEN_FILE_TYPE file_type) {
 ...
 */
static inline int __open_verbose(WT_SESSION_IMPL* session,
                                 const char* name,
...
#define WT_FS_OPEN_VERBOSE_FLAG(f, name)                          \
    ...
    __wt_verbose(session,
                 ...
                 name,
                 __open_verbose_file_type_tag(file_type),
 ...
 */
static int __handle_close(WT_SESSION_IMPL* session, WT_FH* fh, bool locked) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    uint64_t bucket;
 ...
 */
bool __wt_fsync_background_chk(WT_SESSION_IMPL* session) {
    WT_CONNECTION_IMPL* conn;
    WT_FH* fh;
    WT_FILE_HANDLE* handle;
    bool supported;
 ...
 */
static int __fsync_background(WT_SESSION_IMPL* session, WT_FH* fh) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_FILE_HANDLE* handle;
    uint64_t now;
 ...
 */
int __wt_fsync_background(WT_SESSION_IMPL* session) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_FH *fh, *fhnext;
 ...
 */
int __wt_close_connection_close(WT_SESSION_IMPL* session) {
    WT_DECL_RET;
    WT_FH *fh, *fh_tmp;
