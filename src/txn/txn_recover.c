 ...
 */
static int __recovery_cursor(WT_SESSION_IMPL* session,
                             WT_RECOVERY* r,
                             WT_LSN* lsnp,
                             u_int id,
                             bool duplicate,
...
#define GET_RECOVERY_CURSOR(session, r, lsnp, fileid, cp)                             \
    ...
    __wt_verbose(session,                                                             \
                 ...
                 ret != 0 ? "Error" : cursor == NULL ? "Skipping" : "Applying",       \
                 optype,                                                              \
                 fileid,                                                              \
                 (lsnp)->l.file,                                                      \
                 (lsnp)->l.offset);                                                   \
 ...
 */
static int __txn_op_apply(WT_RECOVERY* r, WT_LSN* lsnp, const uint8_t** pp, const uint8_t* end) {
    ...
    switch (optype) {
        ...
        case WT_LOGOP_COL_TRUNCATE:
            WT_ERR(__wt_logop_col_truncate_unpack(
 ...
 */
static int __txn_commit_apply(WT_RECOVERY* r,
                              WT_LSN* lsnp,
                              const uint8_t** pp,
                              const uint8_t* end) {
 ...
 */
static int __txn_log_recover(WT_SESSION_IMPL* session,
                             WT_ITEM* logrec,
                             WT_LSN* lsnp,
                             WT_LSN* next_lsnp,
                             void* cookie,
 ...
 */
static int __recovery_set_checkpoint_timestamp(WT_RECOVERY* r) {
    WT_CONFIG_ITEM cval;
    ...
    if (WT_VERBOSE_ISSET(session, WT_VERB_RECOVERY | WT_VERB_RECOVERY_PROGRESS)) {
...
int __wt_txn_recover(WT_SESSION_IMPL* session) {
    ...
    __wt_verbose(session,
                 ...
                 r.ckpt_lsn.l.file,
                 r.ckpt_lsn.l.offset,
                 r.max_rec_lsn.l.file,
                 r.max_rec_lsn.l.offset);
