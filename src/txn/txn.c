 ...
 */
static uint32_t __snapsort_partition(uint64_t* array, uint32_t f, uint32_t l, uint64_t pivot) {
    uint32_t i, j;
 ...
 */
static void __snapsort_impl(uint64_t* array, uint32_t f, uint32_t l) {
    while (f + 16 < l) {
 ...
 */
static void __snapsort(uint64_t* array, uint32_t size) {
 ...
 */
static inline void __txn_remove_from_global_table(WT_SESSION_IMPL* session) {
 ...
 *	Allocate a snapshot.
 */
void __wt_txn_get_snapshot(WT_SESSION_IMPL* session) {
 ...
 */
static void __txn_oldest_scan(WT_SESSION_IMPL* session,
                              uint64_t* oldest_idp,
                              uint64_t* last_runningp,
                              uint64_t* metadata_pinnedp,
 ...
 */
int __wt_txn_update_oldest(WT_SESSION_IMPL* session, uint32_t flags) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_SESSION_IMPL* oldest_session;
    WT_TXN_GLOBAL* txn_global;
 ...
 */
int __wt_txn_config(WT_SESSION_IMPL* session, const char* cfg[]) {
    WT_CONFIG_ITEM cval;
    WT_TXN* txn;
 ...
 */
int __wt_txn_reconfigure(WT_SESSION_IMPL* session, const char* config) {
    WT_CONFIG_ITEM cval;
    WT_DECL_RET;
    WT_TXN* txn;
 ...
 */
static inline int __txn_commit_timestamps_assert(WT_SESSION_IMPL* session) {
    WT_CURSOR* cursor;
    WT_DECL_RET;
    WT_TXN* txn;
    WT_TXN_OP* op;
    ...
    for (i = 0, op = txn->mod; i < txn->mod_count; i++, op++)
        if (op->type == WT_TXN_OP_BASIC_COL || op->type == WT_TXN_OP_BASIC_ROW) {
            ...
            if (F_ISSET(txn, WT_TXN_TS_COMMIT_KEYS) && op_timestamp < upd->start_ts)
                WT_RET_MSG(session, EINVAL, "out of order commit timestamps");
            if (F_ISSET(txn, WT_TXN_TS_DURABLE_KEYS) && txn->durable_timestamp < upd->durable_ts)
 ...
 */
int __wt_txn_prepare(WT_SESSION_IMPL* session, const char* cfg[]) {
    WT_TXN* txn;
    WT_TXN_OP* op;
    WT_UPDATE* upd;
    u_int i;
    ...
    for (i = 0, op = txn->mod; i < txn->mod_count; i++, op++) {
        ...
        WT_ASSERT(session,
                  S2C(session)->cache->las_fileid == 0 || !F_ISSET(op->btree, WT_BTREE_LOOKASIDE));
 ...
 */
int __wt_txn_rollback_required(WT_SESSION_IMPL* session, const char* reason) {
 ...
 */
int __wt_txn_init(WT_SESSION_IMPL* session, WT_SESSION_IMPL* session_ret) {
    WT_TXN* txn;
 ...
 */
void __wt_txn_release_resources(WT_SESSION_IMPL* session) {
    WT_TXN* txn;
 ...
 */
int __wt_txn_activity_drain(WT_SESSION_IMPL* session) {
    bool txn_active;
 ...
 */
void __wt_txn_global_shutdown(WT_SESSION_IMPL* session) {
    /*
 ...
 */
int __wt_verbose_dump_txn_one(WT_SESSION_IMPL* session, WT_TXN* txn) {
    const char* iso_tag;
 ...
 */
int __wt_verbose_dump_txn(WT_SESSION_IMPL* session) {
    WT_CONNECTION_IMPL* conn;
    ...
    WT_RET(__wt_msg(
        session, "checkpoint pinned ID: %" PRIu64, txn_global->checkpoint_state.pinned_id));
    ...
    for (i = 0, s = txn_global->states; i < session_cnt; i++, s++) {
        ...
        WT_RET(__wt_msg(session,
                        "ID: %" PRIu64 ", pinned ID: %" PRIu64 ", metadata pinned ID: %" PRIu64
                        ", name: %s",
                        id,
                        s->pinned_id,
                        s->metadata_pinned,
