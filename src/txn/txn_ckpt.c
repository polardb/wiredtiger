 ...
 */
static int __checkpoint_name_ok(WT_SESSION_IMPL* session, const char* name, size_t len) {
 ...
 *	checkpoint.
 */
static void __checkpoint_update_generation(WT_SESSION_IMPL* session) {
    WT_BTREE* btree;
 ...
 */
static int __checkpoint_apply_all(WT_SESSION_IMPL* session,
                                  const char* cfg[],
 ...
 */
static int __checkpoint_apply(WT_SESSION_IMPL* session,
                              const char* cfg[],
                              int (*op)(WT_SESSION_IMPL*, const char* [])) {
    WT_DECL_RET;
    u_int i;
 ...
 */
int __wt_checkpoint_get_handles(WT_SESSION_IMPL* session, const char* cfg[]) {
    WT_BTREE* btree;
    WT_CONFIG_ITEM cval;
    WT_DECL_RET;
    const char* name;
    ...
    WT_ASSERT(session,
              session->dhandle->type == WT_DHANDLE_TYPE_BTREE &&
    ...
    WT_RET(__wt_realloc_def(session,
                            &session->ckpt_handle_allocated,
                            session->ckpt_handle_next + 1,
                            &session->ckpt_handle));
 ...
 */
static void __checkpoint_reduce_dirty_cache(WT_SESSION_IMPL* session) {
    WT_CACHE* cache;
 ...
 */
void __wt_checkpoint_progress(WT_SESSION_IMPL* session, bool closing) {
 ...
 */
static void __checkpoint_stats(WT_SESSION_IMPL* session) {
 ...
 */
static void __checkpoint_verbose_track(WT_SESSION_IMPL* session, const char* msg) {
    struct timespec stop;
    WT_CONNECTION_IMPL* conn;
    uint64_t msec;
    ...
    __wt_verbose(session,
                 WT_VERB_CHECKPOINT,
                 "time: %" PRIu64 " ms, gen: %" PRIu64 ": Full database checkpoint %s",
                 msec,
 ...
 */
static void __checkpoint_fail_reset(WT_SESSION_IMPL* session) {
    WT_BTREE* btree;
 ...
 */
static int __checkpoint_prepare(WT_SESSION_IMPL* session, bool* trackingp, const char* cfg[]) {
    WT_CONFIG_ITEM cval;
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_TXN* txn;
 ...
 */
static int __txn_checkpoint_can_skip(WT_SESSION_IMPL* session,
                                     const char* cfg[],
                                     bool* fullp,
                                     bool* use_timestampp,
                                     bool* can_skipp) {
 ...
 */
static int __txn_checkpoint(WT_SESSION_IMPL* session, const char* cfg[]) {
    WT_CACHE* cache;
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_TXN* txn;
  ...
  */
    ...
    if (full && logging) {
        if (ret == 0 &&
            ...
            idle = true;
        WT_TRET(__wt_txn_checkpoint_log(session,
                                        full,
                                        (ret == 0 && !idle) ? WT_TXN_LOG_CKPT_STOP
                                                            : WT_TXN_LOG_CKPT_CLEANUP,
                                        NULL));
    }
 ...
 *	Checkpoint wrapper.
 */
static int __txn_checkpoint_wrapper(WT_SESSION_IMPL* session, const char* cfg[]) {
    WT_DECL_RET;
    WT_TXN_GLOBAL* txn_global;
 ...
 *	Checkpoint a database or a list of objects in the database.
 */
int __wt_txn_checkpoint(WT_SESSION_IMPL* session, const char* cfg[], bool waiting) {
    WT_DECL_RET;
    uint32_t orig_flags;
 ...
 */
static int __checkpoint_lock_dirty_tree(WT_SESSION_IMPL* session,
                                        bool is_checkpoint,
                                        bool force,
                                        bool need_tracking,
                                        const char* cfg[]) {
 ...
 */
static int __checkpoint_mark_skip(WT_SESSION_IMPL* session, WT_CKPT* ckptbase, bool force) {
    WT_BTREE* btree;
    WT_CKPT* ckpt;
    int deleted;
    const char* name;
 ...
 */
void __wt_checkpoint_tree_reconcile_update(WT_SESSION_IMPL* session,
    ...
    __wt_timestamp_addr_check(session, oldest_start_ts, newest_start_ts, newest_stop_ts);
 ...
 */
static int __checkpoint_tree(WT_SESSION_IMPL* session, bool is_checkpoint, const char* cfg[]) {
    WT_BM* bm;
    WT_BTREE* btree;
    WT_CONNECTION_IMPL* conn;
    WT_DATA_HANDLE* dhandle;
 ...
 *	business.
 */
static int __checkpoint_presync(WT_SESSION_IMPL* session, const char* cfg[]) {
    WT_BTREE* btree;
 ...
 */
static int __checkpoint_tree_helper(WT_SESSION_IMPL* session, const char* cfg[]) {
    WT_BTREE* btree;
    WT_DECL_RET;
    WT_TXN* txn;
    bool with_timestamp;
...
int __wt_checkpoint(WT_SESSION_IMPL* session, const char* cfg[]) {
    ...
    WT_ASSERT(session,
              !WT_IS_METADATA(session->dhandle) || F_ISSET(session, WT_SESSION_LOCKED_METADATA));
 ...
 */
int __wt_checkpoint_close(WT_SESSION_IMPL* session, bool final) {
    WT_BTREE* btree;
    WT_DECL_RET;
    ...
    WT_SAVE_DHANDLE(session,
                    ret = __checkpoint_lock_dirty_tree(session, false, false, need_tracking, NULL));
 ...
 *	complete.
 */
static void __checkpoint_timing_stress(WT_SESSION_IMPL* session) {
    WT_CONNECTION_IMPL* conn;
