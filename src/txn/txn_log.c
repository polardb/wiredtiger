 ...
 */
static void __txn_op_log_row_key_check(WT_SESSION_IMPL* session, WT_CURSOR_BTREE* cbt) {
    WT_CURSOR* cursor;
    WT_ITEM key;
    WT_PAGE* page;
    WT_ROW* rip;
 ...
 */
static int __txn_op_log(WT_SESSION_IMPL* session,
                        WT_ITEM* logrec,
                        WT_TXN_OP* op,
                        WT_CURSOR_BTREE* cbt) {
    WT_CURSOR* cursor;
 ...
 */
static int __txn_oplist_printlog(WT_SESSION_IMPL* session,
                                 const uint8_t** pp,
                                 const uint8_t* end,
...
int __wt_txn_log_op(WT_SESSION_IMPL* session, WT_CURSOR_BTREE* cbt) {
    ...
    if (!FLD_ISSET(S2C(session)->log_flags, WT_CONN_LOG_ENABLED) ||
        F_ISSET(session, WT_SESSION_NO_LOGGING) || F_ISSET(S2BT(session), WT_BTREE_NO_LOGGING))
        return (0);
    ...
    switch (op->type) {
        ...
        case WT_TXN_OP_TRUNCATE_COL:
            ...
            break;
        case WT_TXN_OP_TRUNCATE_ROW:
            ret = __wt_logop_row_truncate_pack(session,
                                               logrec,
                                               fileid,
                                               &op->u.truncate_row.start,
                                               &op->u.truncate_row.stop,
 ...
 */
int __wt_txn_checkpoint_logread(WT_SESSION_IMPL* session,
                                const uint8_t** pp,
                                const uint8_t* end,
                                WT_LSN* ckpt_lsn) {
    WT_DECL_RET;
    ...
    if ((ret = __wt_struct_unpack(session,
                                  *pp,
                                  ...
                                  &ckpt_file,
                                  &ckpt_offset,
                                  &ckpt_nsnapshot_unused,
                                  &ckpt_snapshot_unused)) != 0)
 ...
 */
int __wt_txn_checkpoint_log(WT_SESSION_IMPL* session, bool full, uint32_t flags, WT_LSN* lsnp) {
    ...
    switch (flags) {
        ...
        case WT_TXN_LOG_CKPT_STOP:
            ...
            WT_ERR(__wt_struct_size(session,
                                    &recsize,
                                    fmt,
                                    rectype,
                                    ckpt_lsn->l.file,
...
int __wt_txn_truncate_log(WT_SESSION_IMPL* session, WT_CURSOR_BTREE* start, WT_CURSOR_BTREE* stop) {
    ...
    if (btree->type == BTREE_ROW) {
        ...
        if (stop != NULL) {
            op->u.truncate_row.mode = (op->u.truncate_row.mode == WT_TXN_TRUNC_ALL)
                ? WT_TXN_TRUNC_STOP
                : WT_TXN_TRUNC_BOTH;
 ...
 */
void __wt_txn_truncate_end(WT_SESSION_IMPL* session) {
 ...
 */
static int __txn_printlog(WT_SESSION_IMPL* session,
                          WT_ITEM* rawrec,
                          WT_LSN* lsnp,
                          WT_LSN* next_lsnp,
                          void* cookie,
    ...
    switch (rectype) {
        ...
        case WT_LOGREC_MESSAGE:
            WT_RET(
                __wt_struct_unpack(session, p, WT_PTRDIFF(end, p), WT_UNCHECKED_STRING(S), &msg));
 ...
 */
int __wt_txn_printlog(WT_SESSION* wt_session, const char* ofile, uint32_t flags)
