 ...
 */
static int __bloom_init(WT_SESSION_IMPL* session,
                        const char* uri,
                        const char* config,
                        WT_BLOOM** bloomp) {
    WT_BLOOM* bloom;
 ...
 * open.
 */
static int __bloom_setup(WT_BLOOM* bloom, uint64_t n, uint64_t m, uint32_t factor, uint32_t k) {
    if (k < 2)
 ...
 */
int __wt_bloom_create(WT_SESSION_IMPL* session,
                      const char* uri,
                      const char* config,
                      uint64_t count,
                      uint32_t factor,
                      uint32_t k,
                      WT_BLOOM** bloomp) WT_GCC_FUNC_ATTRIBUTE((visibility("default"))) {
    WT_BLOOM* bloom;
    WT_DECL_RET;
 ...
 */
static int __bloom_open_cursor(WT_BLOOM* bloom, WT_CURSOR* owner) {
    WT_CURSOR* c;
 ...
 */
int __wt_bloom_open(WT_SESSION_IMPL* session,
                    const char* uri,
                    uint32_t factor,
                    uint32_t k,
                    WT_CURSOR* owner,
 ...
 */
void __wt_bloom_insert(WT_BLOOM* bloom, WT_ITEM* key)
 ...
 */
int __wt_bloom_finalize(WT_BLOOM* bloom) WT_GCC_FUNC_ATTRIBUTE((visibility("default"))) {
    WT_CURSOR* c;
    WT_DECL_RET;
    WT_ITEM values;
 ...
 */
void __wt_bloom_hash(WT_BLOOM* bloom, WT_ITEM* key, WT_BLOOM_HASH* bhash) {
    WT_UNUSED(bloom);
 ...
 */
int __wt_bloom_hash_get(WT_BLOOM* bloom, WT_BLOOM_HASH* bhash) {
    WT_CURSOR* c;
    WT_DECL_RET;
    uint64_t h1, h2;
    uint32_t i;
 ...
 */
int __wt_bloom_get(WT_BLOOM* bloom, WT_ITEM* key) WT_GCC_FUNC_ATTRIBUTE((visibility("default"))) {
    WT_BLOOM_HASH bhash;
 ...
 *	filter with another.
 */
int __wt_bloom_intersection(WT_BLOOM* bloom, WT_BLOOM* other) {
    uint64_t i, nbytes;
 ...
 */
int __wt_bloom_close(WT_BLOOM* bloom) WT_GCC_FUNC_ATTRIBUTE((visibility("default"))) {
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
int __wt_bloom_drop(WT_BLOOM* bloom, const char* config)
    WT_GCC_FUNC_ATTRIBUTE((visibility("default"))) {
    WT_DECL_RET;
    WT_SESSION* wt_session;
