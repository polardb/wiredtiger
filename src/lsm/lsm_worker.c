 ...
 */
int __wt_lsm_worker_stop(WT_SESSION_IMPL* session, WT_LSM_WORKER_ARGS* args) {
    args->running = false;
    args->tid_set = false;
 ...
 *	work unit types.
 */
static int __lsm_worker_general_op(WT_SESSION_IMPL* session,
 ...
 */
static WT_THREAD_RET __lsm_worker(void* arg) {
    WT_DECL_RET;
    WT_LSM_WORKER_ARGS* cookie;
    WT_LSM_WORK_UNIT* entry;
