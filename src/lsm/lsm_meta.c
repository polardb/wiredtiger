 ...
 */
static int __lsm_meta_read_v0(WT_SESSION_IMPL* session,
 ...
 */
static int __lsm_meta_read_v1(WT_SESSION_IMPL* session,
                              WT_LSM_TREE* lsm_tree,
                              const char* lsmconf) {
    WT_CONFIG lparser;
    ...
    if (cv.len != 0 && !WT_STRING_MATCH("none", cv.str, cv.len)) {
        ...
        WT_ERR(__wt_collator_config(session,
                                    lsm_tree->name,
                                    &cv,
                                    &metadata,
                                    &lsm_tree->collator,
 ...
 */
static int __lsm_meta_upgrade_v1(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree) {
    WT_DECL_ITEM(buf);
    WT_DECL_RET;
    ...
    WT_ERR(__wt_buf_catfmt(
        session, buf, ",bloom=%d", FLD_ISSET(lsm_tree->bloom, WT_LSM_BLOOM_MERGED)));
    WT_ERR(__wt_buf_catfmt(
 ...
 */
int __wt_lsm_meta_read(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree) {
    WT_CONFIG_ITEM cval;
    WT_DECL_RET;
    char* lsmconf;
    bool upgrade;
 ...
 */
int __wt_lsm_meta_write(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree, const char* newconfig) {
    WT_DECL_ITEM(buf);
    WT_DECL_RET;
    WT_LSM_CHUNK* chunk;
    u_int i;
