 ...
 *	threads.
 */
static WT_THREAD_RET __lsm_worker_manager(void* arg) {
    WT_DECL_RET;
    WT_LSM_MANAGER* manager;
 ...
 *	regularly.
 */
void __wt_lsm_manager_clear_tree(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree) {
    WT_LSM_MANAGER* manager;
 ...
 */
int __wt_lsm_manager_push_entry(WT_SESSION_IMPL* session,
                                uint32_t type,
                                uint32_t flags,
                                WT_LSM_TREE* lsm_tree) {
    WT_LSM_MANAGER* manager;
