 ...
 */
int __wt_lsm_merge_update_tree(WT_SESSION_IMPL* session,
 ...
 */
static void __lsm_merge_aggressive_clear(WT_LSM_TREE* lsm_tree) {
 ...
 */
static void __lsm_merge_aggressive_update(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree) {
    struct timespec now;
 ...
 */
static void __lsm_merge_clear(WT_SESSION_IMPL* session,
                              WT_LSM_TREE* lsm_tree,
                              u_int start,
                              u_int nchunks) {
    WT_LSM_CHUNK* chunk;
 ...
 *	locked.
 */
static int __lsm_merge_span(WT_SESSION_IMPL* session,
                            WT_LSM_TREE* lsm_tree,
                            u_int id,
                            u_int* start,
                            u_int* end,
    ...
    if (ret != 0 && created_chunk) {
        ...
        if (create_bloom && chunk->bloom_uri != NULL) {
            WT_WITH_SCHEMA_LOCK(session,
                                tret = __wt_schema_drop(session, chunk->bloom_uri, drop_cfg));
            WT_TRET(tret);
        }
