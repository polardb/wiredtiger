 ...
 */
static void __lsm_tree_discard_state(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree) {
    WT_LSM_CHUNK* chunk;
    u_int i;
 ...
 */
static int __lsm_tree_discard(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree, bool final) {
    WT_DECL_RET;
 ...
 */
static void __lsm_tree_close(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree, bool final) {
    /*
 ...
 */
int __wt_lsm_tree_bloom_name(WT_SESSION_IMPL* session,
 ...
 */
int __wt_lsm_tree_chunk_name(WT_SESSION_IMPL* session,
                             WT_LSM_TREE* lsm_tree,
                             uint32_t id,
                             uint32_t generation,
                             const char** retp) {
 ...
 */
int __wt_lsm_tree_set_chunk_size(WT_SESSION_IMPL* session,
                                 WT_LSM_TREE* lsm_tree,
                                 WT_LSM_CHUNK* chunk) {
    WT_DATA_SOURCE* dsrc;
    wt_off_t size;
 ...
 */
int __wt_lsm_tree_setup_chunk(WT_SESSION_IMPL* session,
                              WT_LSM_TREE* lsm_tree,
                              WT_LSM_CHUNK* chunk) {
 ...
 */
int __wt_lsm_tree_setup_bloom(WT_SESSION_IMPL* session,
                              WT_LSM_TREE* lsm_tree,
                              WT_LSM_CHUNK* chunk) {
    /*
 ...
 */
int __wt_lsm_tree_create(WT_SESSION_IMPL* session,
                         const char* uri,
                         bool exclusive,
                         const char* config) {
    WT_CONFIG_ITEM cval;
 ...
 */
static int __lsm_tree_find(WT_SESSION_IMPL* session,
                           const char* uri,
                           bool exclusive,
                           WT_LSM_TREE** treep) {
    WT_LSM_TREE* lsm_tree;
...
static int __lsm_tree_open_check(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree) {
    ...
    if (conn->cache_size < required)
        WT_RET_MSG(session,
                   EINVAL,
                   "LSM cache size %" PRIu64 " (%" PRIu64
                   "MB) too small, "
                   "must be at least %" PRIu64 " (%" PRIu64 "MB)",
                   conn->cache_size,
                   conn->cache_size / WT_MEGABYTE,
                   required,
 ...
 */
static int __lsm_tree_open(WT_SESSION_IMPL* session,
                           const char* uri,
                           bool exclusive,
                           WT_LSM_TREE** treep) {
    WT_CONNECTION_IMPL* conn;
 ...
 */
int __wt_lsm_tree_get(WT_SESSION_IMPL* session,
                      const char* uri,
                      bool exclusive,
                      WT_LSM_TREE** treep) {
    WT_DECL_RET;
 ...
 */
void __wt_lsm_tree_throttle(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree, bool decrease_only) {
 ...
 */
int __wt_lsm_tree_retire_chunks(WT_SESSION_IMPL* session,
                                WT_LSM_TREE* lsm_tree,
                                u_int start_chunk,
                                u_int nchunks) {
    u_int i;
 ...
 *	Rename an LSM tree.
 */
int __wt_lsm_tree_rename(WT_SESSION_IMPL* session,
                         const char* olduri,
                         const char* newuri,
                         const char* cfg[]) {
    WT_DECL_RET;
 ...
 */
int __wt_lsm_tree_truncate(WT_SESSION_IMPL* session, const char* name, const char* cfg[]) {
    WT_DECL_RET;
    WT_LSM_CHUNK* chunk;
    WT_LSM_TREE* lsm_tree;
    int tret;
 ...
 */
void __wt_lsm_tree_readlock(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree) {
 ...
 */
void __wt_lsm_tree_readunlock(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree) {
 ...
 */
void __wt_lsm_tree_writelock(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree) {
 ...
 */
void __wt_lsm_tree_writeunlock(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree) {
    F_CLR(session, WT_SESSION_IGNORE_CACHE_SIZE | WT_SESSION_NO_SCHEMA_LOCK);
 ...
 */
int __wt_lsm_compact(WT_SESSION_IMPL* session, const char* name, bool* skipp) {
    WT_DECL_RET;
    WT_LSM_CHUNK* chunk;
    WT_LSM_TREE* lsm_tree;
    uint64_t progress;
    ...
    *skipp = true;
    ...
    if (push_flush) {
        __wt_verbose(session,
                     WT_VERB_LSM,
                     "Compact force flush %s flags 0x%" PRIx32 " chunk %" PRIu32
                     " flags 0x%" PRIx32,
                     name,
                     lsm_tree->flags,
                     chunk->id,
 ...
 */
int __wt_lsm_tree_worker(WT_SESSION_IMPL* session,
                         const char* uri,
