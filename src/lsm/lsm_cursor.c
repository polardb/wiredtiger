 ...
 */
int __wt_clsm_request_switch(WT_CURSOR_LSM* clsm) {
    WT_DECL_RET;
 ...
 */
int __wt_clsm_await_switch(WT_CURSOR_LSM* clsm) {
    WT_LSM_TREE* lsm_tree;
    WT_SESSION_IMPL* session;
    int waited;
 ...
 */
static inline int __clsm_enter(WT_CURSOR_LSM* clsm, bool reset, bool update) {
    WT_DECL_RET;
    WT_LSM_TREE* lsm_tree;
    WT_SESSION_IMPL* session;
    WT_TXN* txn;
 ...
 */
static inline bool __clsm_deleted(WT_CURSOR_LSM* clsm, const WT_ITEM* item) {
 ...
 */
static inline int __clsm_deleted_encode(WT_SESSION_IMPL* session,
                                        const WT_ITEM* value,
                                        WT_ITEM* final_value,
                                        WT_ITEM** tmpp) {
    WT_ITEM* tmp;
 ...
 */
static int __clsm_close_cursors(WT_SESSION_IMPL* session,
                                WT_CURSOR_LSM* clsm,
                                u_int start,
                                u_int end) {
    WT_BLOOM* bloom;
 ...
 */
static int __clsm_resize_chunks(WT_SESSION_IMPL* session, WT_CURSOR_LSM* clsm, u_int nchunks) {
 ...
 *	Allocates an array of unit objects for each chunk.
 */
static void __clsm_free_chunks(WT_SESSION_IMPL* session, WT_CURSOR_LSM* clsm) {
    size_t i;
 ...
 */
static int __clsm_open_cursors(WT_CURSOR_LSM* clsm,
                               bool update,
                               u_int start_chunk,
                               uint32_t start_id) {
    WT_BTREE* btree;
...
retry:
    ...
    for (i = ngood; i != nchunks; i++) {
        ...
        ret = __wt_open_cursor(session,
                               chunk->uri,
                               c,
                               (F_ISSET(chunk, WT_LSM_CHUNK_ONDISK) && !chunk->empty) ? ckpt_cfg
                                                                                      : NULL,
        ...
        if (!F_ISSET(clsm, WT_CLSM_MERGE) && F_ISSET(chunk, WT_LSM_CHUNK_BLOOM))
            WT_ERR(__wt_bloom_open(session,
                                   chunk->bloom_uri,
 ...
 */
static int __clsm_get_current(WT_SESSION_IMPL* session,
                              WT_CURSOR_LSM* clsm,
                              bool smallest,
                              bool* deletedp) {
    WT_CURSOR *c, *current;
 ...
 */
static int __clsm_position_chunk(WT_CURSOR_LSM* clsm, WT_CURSOR* c, bool forward, int* cmpp) {
    WT_CURSOR* cursor;
    WT_SESSION_IMPL* session;
 ...
 */
static int __clsm_random_chunk(WT_SESSION_IMPL* session, WT_CURSOR_LSM* clsm, WT_CURSOR** cursor) {
 ...
 * next_random.
 */
static int __clsm_next_random(WT_CURSOR* cursor) {
    WT_CURSOR* c;
    WT_CURSOR_LSM* clsm;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 *	necessary.
 */
static inline int __clsm_put(WT_SESSION_IMPL* session,
                             WT_CURSOR_LSM* clsm,
                             const WT_ITEM* key,
                             const WT_ITEM* value,
                             bool position,
 ...
 */
static int __clsm_reserve(WT_CURSOR* cursor) {
    WT_CURSOR_LSM* clsm;
    WT_DECL_RET;
    WT_ITEM value;
    WT_SESSION_IMPL* session;
 ...
 */
int __wt_clsm_close(WT_CURSOR* cursor) {
    WT_CURSOR_LSM* clsm;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
