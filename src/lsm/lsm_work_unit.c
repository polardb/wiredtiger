 ...
 */
static int __lsm_copy_chunks(WT_SESSION_IMPL* session,
 ...
 */
int __wt_lsm_get_chunk_to_flush(WT_SESSION_IMPL* session,
                                WT_LSM_TREE* lsm_tree,
                                bool force,
                                WT_LSM_CHUNK** chunkp) {
    WT_DECL_RET;
 ...
 */
int __wt_lsm_work_switch(WT_SESSION_IMPL* session, WT_LSM_WORK_UNIT** entryp, bool* ran) {
    WT_DECL_RET;
 ...
 */
bool __wt_lsm_chunk_visible_all(WT_SESSION_IMPL* session, WT_LSM_CHUNK* chunk) {
     ...
     */
    if (txn_global->has_commit_timestamp || txn_global->has_pinned_timestamp) {
        if (!F_ISSET(chunk, WT_LSM_CHUNK_HAS_TIMESTAMP)) {
            ...
            if (!F_ISSET(chunk, WT_LSM_CHUNK_HAS_TIMESTAMP)) {
                ...
                chunk->switch_timestamp = txn_global->commit_timestamp;
 ...
 */
static int __lsm_set_chunk_evictable(WT_SESSION_IMPL* session,
                                     WT_LSM_CHUNK* chunk,
                                     bool need_handle) {
    WT_BTREE* btree;
    WT_DECL_RET;
 ...
 */
static int __lsm_checkpoint_chunk(WT_SESSION_IMPL* session) {
    WT_DECL_RET;
 ...
 */
int __wt_lsm_checkpoint_chunk(WT_SESSION_IMPL* session,
                              WT_LSM_TREE* lsm_tree,
                              WT_LSM_CHUNK* chunk) {
    WT_DECL_RET;
    ...
    WT_WITH_CHECKPOINT_LOCK(session,
                            WT_WITH_SCHEMA_LOCK(session, ret = __lsm_checkpoint_chunk(session)));
    if (ret != 0)
 ...
 */
int __wt_lsm_work_enable_evict(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree) {
    WT_DECL_RET;
    WT_LSM_CHUNK* chunk;
    WT_LSM_WORKER_COOKIE cookie;
    u_int i;
 ...
 */
static int __lsm_bloom_create(WT_SESSION_IMPL* session,
                              WT_LSM_TREE* lsm_tree,
                              WT_LSM_CHUNK* chunk,
                              u_int chunk_off) {
    WT_BLOOM* bloom;
    ...
    WT_RET(__wt_bloom_create(session,
                             chunk->bloom_uri,
 ...
 */
static int __lsm_free_chunks(WT_SESSION_IMPL* session, WT_LSM_TREE* lsm_tree) {
    WT_DECL_RET;
    WT_LSM_CHUNK* chunk;
    WT_LSM_WORKER_COOKIE cookie;
    u_int i, skipped;
