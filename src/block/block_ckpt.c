 ...
 *	Load a checkpoint.
 */
int __wt_block_checkpoint_load(WT_SESSION_IMPL* session,
                               WT_BLOCK* block,
                               const uint8_t* addr,
                               size_t addr_size,
                               uint8_t* root_addr,
 ...
 *	Unload a checkpoint.
 */
int __wt_block_checkpoint_unload(WT_SESSION_IMPL* session, WT_BLOCK* block, bool checkpoint) {
    WT_DECL_RET;
 ...
 *	Start a checkpoint.
 */
int __wt_block_checkpoint_start(WT_SESSION_IMPL* session, WT_BLOCK* block) {
    WT_DECL_RET;
 ...
 */
int __wt_block_checkpoint(WT_SESSION_IMPL* session,
                          WT_BLOCK* block,
                          WT_ITEM* buf,
                          WT_CKPT* ckptbase,
                          bool data_checksum) {
 ...
 */
static int __ckpt_verify(WT_SESSION_IMPL* session, WT_CKPT* ckptbase) {
    WT_CKPT* ckpt;
 ...
 */
int __wt_block_checkpoint_resolve(WT_SESSION_IMPL* session, WT_BLOCK* block, bool failed) {
    WT_BLOCK_CKPT* ci;
    WT_DECL_RET;
 ...
 */
static int __ckpt_string(WT_SESSION_IMPL* session,
                         WT_BLOCK* block,
                         const uint8_t* addr,
                         WT_ITEM* buf) {
    WT_BLOCK_CKPT *ci, _ci;
