 ...
 *	Drop a file.
 */
int __wt_block_manager_drop(WT_SESSION_IMPL* session, const char* filename, bool durable) {
...
int __wt_block_manager_create(WT_SESSION_IMPL* session, const char* filename, uint32_t allocsize) {
    ...
    for (;;) {
        if ((ret = __wt_open(session,
                             ...
                             WT_FS_OPEN_CREATE | WT_FS_OPEN_DURABLE | WT_FS_OPEN_EXCLUSIVE,
                             &fh)) == 0)
            break;
 ...
 */
void __wt_block_configure_first_fit(WT_BLOCK* block, bool on) {
    /*
 ...
 *	Open a block handle.
 */
int __wt_block_open(WT_SESSION_IMPL* session,
                    const char* filename,
                    const char* cfg[],
                    bool forced_salvage,
                    bool readonly,
 ...
 */
int __wt_desc_write(WT_SESSION_IMPL* session, WT_FH* fh, uint32_t allocsize) {
    WT_BLOCK_DESC* desc;
    WT_DECL_ITEM(buf);
    WT_DECL_RET;
...
static int __desc_read(WT_SESSION_IMPL* session, WT_BLOCK* block) {
    ...
    if (desc->majorv > WT_BLOCK_MAJOR_VERSION ||
        ...
        WT_ERR_MSG(session,
                   ...
                   WT_BLOCK_MAJOR_VERSION,
                   WT_BLOCK_MINOR_VERSION,
                   desc->majorv,
                   desc->minorv);

    __wt_verbose(session,
                 WT_VERB_BLOCK,
                 "%s: magic %" PRIu32 ", major/minor: %" PRIu32 "/%" PRIu32,
                 block->name,
                 desc->magic,
                 desc->majorv,
                 desc->minorv);
 ...
 */
int __wt_block_manager_size(WT_BM* bm, WT_SESSION_IMPL* session, wt_off_t* sizep) {
    WT_UNUSED(session);
 ...
 */
int __wt_block_manager_named_size(WT_SESSION_IMPL* session, const char* name, wt_off_t* sizep) {
