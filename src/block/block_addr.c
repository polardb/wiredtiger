 ...
 */
static int __block_buffer_to_addr(uint32_t allocsize,
                                  const uint8_t** pp,
                                  wt_off_t* offsetp,
                                  uint32_t* sizep,
                                  uint32_t* checksump) {
 ...
 */
int __wt_block_addr_string(WT_SESSION_IMPL* session,
                           WT_BLOCK* block,
                           WT_ITEM* buf,
                           const uint8_t* addr,
                           size_t addr_size) {
 ...
 */
static int __block_buffer_to_ckpt(WT_SESSION_IMPL* session,
                                  uint32_t allocsize,
                                  const uint8_t* p,
                                  WT_BLOCK_CKPT* ci) {
    uint64_t a;
    ...
    WT_RET(__block_buffer_to_addr(
        allocsize, pp, &ci->root_offset, &ci->root_size, &ci->root_checksum));
    WT_RET(__block_buffer_to_addr(
 ...
 */
int __wt_block_buffer_to_ckpt(WT_SESSION_IMPL* session,
                              WT_BLOCK* block,
                              const uint8_t* p,
                              WT_BLOCK_CKPT* ci) {
 ...
 * version.
 */
int __wt_block_ckpt_decode(WT_SESSION* wt_session,
                           size_t allocsize,
                           const uint8_t* p,
 ...
 */
int __wt_block_ckpt_to_buffer(WT_SESSION_IMPL* session,
                              WT_BLOCK* block,
                              uint8_t** pp,
                              WT_BLOCK_CKPT* ci) {
    uint64_t a;
    ...
    WT_RET(
        __wt_block_addr_to_buffer(block, pp, ci->avail.offset, ci->avail.size, ci->avail.checksum));
