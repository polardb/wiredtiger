 ...
 */
int __wt_block_verify_start(WT_SESSION_IMPL* session,
                            WT_BLOCK* block,
                            WT_CKPT* ckptbase,
                            const char* cfg[]) {
    WT_CKPT *ckpt, *t;
 ...
 */
static int __verify_set_file_size(WT_SESSION_IMPL* session, WT_BLOCK* block, WT_CKPT* ckpt) {
    WT_BLOCK_CKPT *ci, _ci;
    WT_DECL_ITEM(tmp);
    WT_DECL_RET;
    ...
    if (block->verify_layout) {
        ...
        WT_ERR(__wt_msg(session,
                        "%s: correcting to %s checkpoint size %s",
                        block->name,
                        ckpt->name,
 ...
 */
int __wt_block_verify_addr(WT_SESSION_IMPL* session,
                           WT_BLOCK* block,
                           const uint8_t* addr,
                           size_t addr_size) {
    wt_off_t offset;
 ...
 */
static int __verify_filefrag_add(WT_SESSION_IMPL* session,
                                 WT_BLOCK* block,
                                 const char* type,
                                 wt_off_t offset,
                                 wt_off_t size,
 ...
 */
static int __verify_ckptfrag_add(WT_SESSION_IMPL* session,
                                 WT_BLOCK* block,
                                 wt_off_t offset,
                                 wt_off_t size) {
    uint64_t f, frag, frags, i;
