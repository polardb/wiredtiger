 ...
 */
int __wt_block_compact_skip(WT_SESSION_IMPL* session, WT_BLOCK* block, bool* skipp) {
    WT_EXT* ext;
    WT_EXTLIST* el;
    ...
    *skipp = true; /* Return a default skip. */
    ...
    __wt_verbose(session,
                 WT_VERB_COMPACT,
                 "%s: %" PRIuMAX "MB (%" PRIuMAX
                 ") available space in the first "
                 "80%% of the file",
                 block->name,
                 (uintmax_t)avail_eighty / WT_MEGABYTE,
    ...
    __wt_verbose(session,
                 ...
                 "90%% of the file",
                 block->name,
                 (uintmax_t)avail_ninety / WT_MEGABYTE,
 ...
 */
int __wt_block_compact_page_skip(
 ...
 */
static void __block_dump_avail(WT_SESSION_IMPL* session, WT_BLOCK* block, bool start) {
    WT_EXT* ext;
    WT_EXTLIST* el;
    ...
    __wt_verbose(session,
                 ...
                 (uintmax_t)size / WT_MEGABYTE,
                 (uintmax_t)size,
                 ((uintmax_t)el->bytes * 100) / (uintmax_t)size,
                 (uintmax_t)el->bytes / WT_MEGABYTE,
                 (uintmax_t)el->bytes);
    ...
    for (i = 0; i < WT_ELEMENTS(percentile); ++i) {
        ...
        __wt_verbose(session,
                     WT_VERB_COMPACT,
                     "%2u%%: %12" PRIuMAX "MB, (%" PRIuMAX "B, %" PRIuMAX "%%)",
                     i,
                     (uintmax_t)v / WT_MEGABYTE,
                     (uintmax_t)v,
