...
int __wt_bm_read(
    ...
    WT_RET(__wt_block_misplaced(
        session, block, "read", offset, size, bm->is_live, __func__, __LINE__));
#endif
    /* Read the block. */
 ...
 */
static int __wt_bm_corrupt_dump(WT_SESSION_IMPL* session,
                                WT_ITEM* buf,
                                wt_off_t offset,
                                uint32_t size,
 ...
 */
int __wt_bm_corrupt(WT_BM* bm, WT_SESSION_IMPL* session, const uint8_t* addr, size_t addr_size) {
    WT_DECL_ITEM(tmp);
    WT_DECL_RET;
    wt_off_t offset;
    uint32_t checksum, size;
 ...
 * only.
 */
int __wt_block_read_off_blind(WT_SESSION_IMPL* session,
                              WT_BLOCK* block,
                              wt_off_t offset,
                              uint32_t* sizep,
                              uint32_t* checksump) {
 ...
 */
int __wt_block_read_off(WT_SESSION_IMPL* session,
                        WT_BLOCK* block,
                        WT_ITEM* buf,
                        wt_off_t offset,
                        uint32_t size,
    ...
    if (swap.checksum == checksum) {
        blk->checksum = 0;
        page_checksum = __wt_checksum(
            buf->mem, F_ISSET(&swap, WT_BLOCK_DATA_CKSUM) ? size : WT_BLOCK_COMPRESS_SKIP);
