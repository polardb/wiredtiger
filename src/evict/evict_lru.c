 ...
 */
static int __evict_lock_handle_list(WT_SESSION_IMPL* session) {
    WT_CACHE* cache;
 ...
 */
static inline uint64_t __evict_entry_priority(WT_SESSION_IMPL* session, WT_REF* ref) {
    WT_BTREE* btree;
    WT_PAGE* page;
    uint64_t read_gen;
 ...
 */
static int WT_CDECL __evict_lru_cmp(const void* a_arg, const void* b_arg) {
 ...
 *	queue as empty.
 */
static inline bool __evict_queue_empty(WT_EVICT_QUEUE* queue, bool server_check) {
 ...
 */
void __wt_evict_server_wake(WT_SESSION_IMPL* session) {
    WT_CACHE* cache;
    WT_CONNECTION_IMPL* conn;
 ...
 */
bool __wt_evict_thread_chk(WT_SESSION_IMPL* session) {
 ...
 */
int __wt_evict_thread_run(WT_SESSION_IMPL* session, WT_THREAD* thread) {
    WT_CACHE* cache;
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
 ...
 */
int __wt_evict_thread_stop(WT_SESSION_IMPL* session, WT_THREAD* thread) {
    WT_CACHE* cache;
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
 ...
 */
static bool __evict_update_work(WT_SESSION_IMPL* session) {
    WT_BTREE* las_tree;
    WT_CACHE* cache;
    WT_CONNECTION_IMPL* conn;
     ...
     */
    if (__wt_cache_aggressive(session) && F_ISSET(cache, WT_CACHE_EVICT_CLEAN_HARD))
        F_SET(cache, WT_CACHE_EVICT_DIRTY);
 ...
 */
static int __evict_clear_walk(WT_SESSION_IMPL* session) {
    WT_BTREE* btree;
    WT_CACHE* cache;
    WT_DECL_RET;
    WT_REF* ref;
    ...
    WT_WITH_DHANDLE(cache->walk_session,
                    session->dhandle,
 ...
 */
static int __evict_clear_all_walks(WT_SESSION_IMPL* session) {
    WT_CONNECTION_IMPL* conn;
 ...
 */
int __wt_evict_file_exclusive_on(WT_SESSION_IMPL* session) {
    WT_BTREE* btree;
    WT_CACHE* cache;
    WT_DECL_RET;
    WT_EVICT_ENTRY* evict;
 ...
 */
static int __evict_lru_walk(WT_SESSION_IMPL* session) {
    WT_CACHE* cache;
    WT_DECL_RET;
 ...
 */
static int __evict_walk(WT_SESSION_IMPL* session, WT_EVICT_QUEUE* queue) {
    WT_BTREE* btree;
    WT_CACHE* cache;
    WT_CONNECTION_IMPL* conn;
    WT_DATA_HANDLE* dhandle;
     ...
     */
    total_candidates = (u_int)(F_ISSET(cache, WT_CACHE_EVICT_CLEAN) ? __wt_cache_pages_inuse(cache)
 ...
 */
static bool __evict_push_candidate(WT_SESSION_IMPL* session,
 ...
 */
static uint32_t __evict_walk_target(WT_SESSION_IMPL* session, u_int max_entries) {
    WT_CACHE* cache;
...
#define MIN_PAGES_PER_TREE 10
     ...
     */
    target_pages = WT_MAX(target_pages_clean, target_pages_dirty) * QUEUE_FILLS_PER_PASS;
 ...
 */
static int __evict_walk_tree(WT_SESSION_IMPL* session,
                             WT_EVICT_QUEUE* queue,
                             u_int max_entries,
                             u_int* slotp) {
    WT_BTREE* btree;
    ...
    if (btree->evict_walk_progress >= btree->evict_walk_target) {
        btree->evict_walk_target = __evict_walk_target(session, max_entries);
 ...
 */
static int __evict_get_ref(WT_SESSION_IMPL* session,
                           bool is_server,
                           WT_BTREE** btreep,
                           WT_REF** refp,
    ...
    *refp = NULL;
    ...
    for (evict = queue->evict_current;
         evict >= queue->evict_queue && evict < queue->evict_queue + candidates;
         ++evict) {
        if (evict->ref == NULL)
            continue;
        ...
        if (((previous_state = evict->ref->state) != WT_REF_MEM &&
            ...
            !__wt_atomic_casv32(&evict->ref->state, previous_state, WT_REF_LOCKED)) {
            __evict_list_clear(session, evict);
            continue;
        }
...
static int __evict_page(WT_SESSION_IMPL* session, bool is_server) {
    ...
    WT_WITH_BTREE(session, btree, ret = __wt_evict(session, ref, false, previous_state));
 ...
 */
int __wt_cache_eviction_worker(WT_SESSION_IMPL* session,
                               bool busy,
                               bool readonly,
                               double pct_full) {
    WT_CACHE* cache;
 ...
 */
bool __wt_page_evict_urgent(WT_SESSION_IMPL* session, WT_REF* ref) {
    WT_CACHE* cache;
    WT_EVICT_ENTRY* evict;
 ...
 */
void __wt_evict_priority_set(WT_SESSION_IMPL* session, uint64_t v) {
 ...
 */
void __wt_evict_priority_clear(WT_SESSION_IMPL* session) {
 ...
 */
static int __verbose_dump_cache_single(WT_SESSION_IMPL* session,
                                       uint64_t* total_bytesp,
                                       uint64_t* total_dirty_bytesp) {
    WT_BTREE* btree;
    WT_DATA_HANDLE* dhandle;
    WT_PAGE* page;
    WT_REF* next_walk;
 ...
 */
static int __verbose_dump_cache_apply(WT_SESSION_IMPL* session,
                                      uint64_t* total_bytesp,
                                      uint64_t* total_dirty_bytesp) {
    WT_CONNECTION_IMPL* conn;
    WT_DATA_HANDLE* dhandle;
    ...
    for (dhandle = NULL;;) {
        ...
        WT_WITH_DHANDLE(session,
                        dhandle,
                        ret =
                            __verbose_dump_cache_single(session, total_bytesp, total_dirty_bytesp));
        if (ret != 0)
            WT_RET(ret);
    }
    return (0);
 ...
 */
int __wt_verbose_dump_cache(WT_SESSION_IMPL* session) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    double pct;
