 ...
 */
int __wt_thread_create(WT_SESSION_IMPL* session,
                       wt_thread_t* tidret,
                       WT_THREAD_CALLBACK (*func)(void*),
                       void* arg) WT_GCC_FUNC_ATTRIBUTE((visibility("default"))) {
    WT_DECL_RET;
 ...
 */
void __wt_thread_id(uintmax_t* id) WT_GCC_FUNC_ATTRIBUTE((visibility("default"))) {
    pthread_t self;
 ...
 */
int __wt_thread_str(char* buf, size_t buflen) WT_GCC_FUNC_ATTRIBUTE((visibility("default"))) {
    pthread_t self;
 ...
 */
uintmax_t __wt_process_id(void) {
