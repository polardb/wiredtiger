 ...
 */
static int __schema_source_config(WT_SESSION_IMPL* session,
                                  WT_CURSOR* srch,
                                  const char* config,
                                  const char** result) {
    WT_CONFIG_ITEM cval;
 ...
 */
static int __schema_create_collapse(WT_SESSION_IMPL* session,
 ...
 *	"create-only" cursors.
 */
static int __curmetadata_setkv(WT_CURSOR_METADATA* mdc, WT_CURSOR* fc) {
    WT_CURSOR* c;
    WT_DECL_RET;
 ...
 */
static int __curmetadata_metadata_search(WT_SESSION_IMPL* session, WT_CURSOR* cursor) {
    WT_CURSOR_METADATA* mdc;
    WT_DECL_RET;
    char *value, *stripped;
 ...
 */
static int __curmetadata_compare(WT_CURSOR* a, WT_CURSOR* b, int* cmpp) {
 ...
 */
static int __curmetadata_next(WT_CURSOR* cursor) {
 ...
 */
static int __curmetadata_prev(WT_CURSOR* cursor) {
    WT_CURSOR* file_cursor;
    WT_CURSOR_METADATA* mdc;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __curmetadata_reset(WT_CURSOR* cursor) {
    WT_CURSOR* file_cursor;
    WT_CURSOR_METADATA* mdc;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
    ...
    mdc = (WT_CURSOR_METADATA*)cursor;
    file_cursor = mdc->file_cursor;
    CURSOR_API_CALL_PREPARE_ALLOWED(cursor, session, reset, ((WT_CURSOR_BTREE*)file_cursor)->btree);
 ...
 */
static int __curmetadata_search(WT_CURSOR* cursor) {
    WT_CURSOR* file_cursor;
    WT_CURSOR_METADATA* mdc;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __curmetadata_search_near(WT_CURSOR* cursor, int* exact) {
    WT_CURSOR* file_cursor;
    WT_CURSOR_METADATA* mdc;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __curmetadata_insert(WT_CURSOR* cursor) {
    WT_CURSOR* file_cursor;
    WT_CURSOR_METADATA* mdc;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __curmetadata_update(WT_CURSOR* cursor) {
    WT_CURSOR* file_cursor;
    WT_CURSOR_METADATA* mdc;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __curmetadata_remove(WT_CURSOR* cursor) {
    WT_CURSOR* file_cursor;
    WT_CURSOR_METADATA* mdc;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __curmetadata_close(WT_CURSOR* cursor) {
    WT_CURSOR* c;
    WT_CURSOR_METADATA* mdc;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
int __wt_curmetadata_open(WT_SESSION_IMPL* session,
                          const char* uri,
                          WT_CURSOR* owner,
                          const char* cfg[],
                          WT_CURSOR** cursorp) {
