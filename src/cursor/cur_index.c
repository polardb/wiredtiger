 ...
 */
static int __curindex_get_value(WT_CURSOR* cursor, ...) {
    WT_DECL_RET;
 ...
 */
static void __curindex_set_value(WT_CURSOR* cursor, ...) {
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __curindex_compare(WT_CURSOR* a, WT_CURSOR* b, int* cmpp) {
 ...
 */
static int __curindex_move(WT_CURSOR_INDEX* cindex) {
    WT_CURSOR **cp, *first;
    WT_SESSION_IMPL* session;
    u_int i;
    ...
    for (i = 0, cp = cindex->cg_cursors; i < WT_COLGROUPS(cindex->table); i++, cp++) {
        ...
        if (first == NULL) {
            ...
            WT_RET(__wt_schema_project_slice(session,
                                             cp,
                                             cindex->index->key_plan,
                                             1,
                                             cindex->index->key_format,
                                             &cindex->iface.key));
            first = *cp;
        } else {
 ...
 */
static int __curindex_next(WT_CURSOR* cursor) {
    WT_CURSOR_INDEX* cindex;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __curindex_prev(WT_CURSOR* cursor) {
    WT_CURSOR_INDEX* cindex;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __curindex_reset(WT_CURSOR* cursor) {
    WT_CURSOR** cp;
    WT_CURSOR_INDEX* cindex;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __curindex_search(WT_CURSOR* cursor) {
    WT_CURSOR* child;
    WT_CURSOR_INDEX* cindex;
    WT_DECL_RET;
    WT_ITEM found_key;
 ...
 */
static int __curindex_search_near(WT_CURSOR* cursor, int* exact) {
    WT_CURSOR* child;
    WT_CURSOR_INDEX* cindex;
    WT_DECL_RET;
    WT_ITEM found_key;
 ...
 */
static int __curindex_close(WT_CURSOR* cursor) {
    WT_CURSOR** cp;
    WT_CURSOR_INDEX* cindex;
    WT_DECL_RET;
    WT_INDEX* idx;
 ...
 */
static int __curindex_open_colgroups(WT_SESSION_IMPL* session,
                                     WT_CURSOR_INDEX* cindex,
                                     const char* cfg_arg[]) {
    WT_CURSOR** cp;
    WT_TABLE* table;
 ...
 */
int __wt_curindex_open(WT_SESSION_IMPL* session,
                       const char* uri,
                       WT_CURSOR* owner,
                       const char* cfg[],
                       WT_CURSOR** cursorp) {
