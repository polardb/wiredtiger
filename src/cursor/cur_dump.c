 ...
 */
static int __raw_to_dump(WT_SESSION_IMPL* session, WT_ITEM* from, WT_ITEM* to, bool hexonly) {
    if (hexonly)
 ...
 */
static int __dump_to_raw(WT_SESSION_IMPL* session,
                         const char* src_arg,
                         WT_ITEM* item,
                         bool hexonly) {
    if (hexonly)
 ...
 */
static int __curdump_get_key(WT_CURSOR* cursor, ...) {
    WT_CURSOR* child;
 ...
 */
static int str2recno(WT_SESSION_IMPL* session, const char* p, uint64_t* recnop) {
    uint64_t recno;
    char* endptr;
 ...
 */
static void __curdump_set_key(WT_CURSOR* cursor, ...) {
    WT_CURSOR* child;
    WT_CURSOR_DUMP* cdump;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
    ...
    if (json)
        WT_ERR(__wt_json_to_item(session,
                                 p,
                                 cursor->key_format,
                                 (WT_CURSOR_JSON*)cursor->json_private,
                                 true,
                                 &cursor->key));
 ...
 */
static int __curdump_get_value(WT_CURSOR* cursor, ...) {
    WT_CURSOR* child;
    WT_CURSOR_DUMP* cdump;
    WT_CURSOR_JSON* json;
    WT_DECL_RET;
 ...
 */
static void __curdump_set_value(WT_CURSOR* cursor, ...) {
    WT_CURSOR* child;
    WT_CURSOR_DUMP* cdump;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __curdump_search_near(WT_CURSOR* cursor, int* exact) {
    WT_CURSOR_DUMP* cdump;
 ...
 */
static int __curdump_close(WT_CURSOR* cursor) {
    WT_CURSOR* child;
    WT_CURSOR_DUMP* cdump;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
int __wt_curdump_create(WT_CURSOR* child, WT_CURSOR* owner, WT_CURSOR** cursorp) {
