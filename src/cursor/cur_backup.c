 ...
 */
static int __curbackup_next(WT_CURSOR* cursor) {
    WT_CURSOR_BACKUP* cb;
    WT_DECL_RET;
 ...
 */
static int __curbackup_reset(WT_CURSOR* cursor) {
    WT_CURSOR_BACKUP* cb;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static void __backup_free(WT_SESSION_IMPL* session, WT_CURSOR_BACKUP* cb) {
    int i;
 ...
 */
static int __curbackup_close(WT_CURSOR* cursor) {
    WT_CURSOR_BACKUP* cb;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
int __wt_curbackup_open(WT_SESSION_IMPL* session,
                        const char* uri,
                        WT_CURSOR* other,
                        const char* cfg[],
                        WT_CURSOR** cursorp) {
 ...
 */
static int __backup_log_append(WT_SESSION_IMPL* session, WT_CURSOR_BACKUP* cb, bool active) {
 ...
 *	Start a backup.
 */
static int __backup_start(WT_SESSION_IMPL* session,
                          WT_CURSOR_BACKUP* cb,
                          bool is_dup,
                          const char* cfg[]) {
    WT_CONNECTION_IMPL* conn;
 ...
 *	Stop a backup.
 */
static int __backup_stop(WT_SESSION_IMPL* session, WT_CURSOR_BACKUP* cb) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
 ...
 */
static int __backup_all(WT_SESSION_IMPL* session) {
...
static int __backup_uri(
    WT_SESSION_IMPL* session, const char* cfg[], bool is_dup, bool* foundp, bool* log_only) {
 ...
 */
int __wt_backup_file_remove(WT_SESSION_IMPL* session) {
    WT_DECL_RET;
 ...
 */
static int __backup_list_uri_append(WT_SESSION_IMPL* session, const char* name, bool* skip) {
    WT_CURSOR_BACKUP* cb;
    WT_DECL_RET;
    char* value;
 ...
 *	Append a new file name to the list, allocate space as necessary.
 */
static int __backup_list_append(WT_SESSION_IMPL* session, WT_CURSOR_BACKUP* cb, const char* uri) {
    const char* name;
    char** p;
