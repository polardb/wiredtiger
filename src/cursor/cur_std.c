 ...
 */
int __wt_cursor_cached(WT_CURSOR* cursor) {
 ...
 */
int __wt_cursor_get_value_notsup(WT_CURSOR* cursor, ...) {
 ...
 */
void __wt_cursor_set_key_notsup(WT_CURSOR* cursor, ...) {
 ...
 */
void __wt_cursor_set_value_notsup(WT_CURSOR* cursor, ...) {
    WT_IGNORE_RET(__wt_cursor_notsup(cursor));
}
 ...
 */
int __wt_cursor_compare_notsup(WT_CURSOR* a, WT_CURSOR* b, int* cmpp) {
    WT_UNUSED(b);
    WT_UNUSED(cmpp);
 ...
 */
int __wt_cursor_equals_notsup(WT_CURSOR* cursor, WT_CURSOR* other, int* equalp) {
    WT_UNUSED(other);
    WT_UNUSED(equalp);
 ...
 */
int __wt_cursor_modify_notsup(WT_CURSOR* cursor, WT_MODIFY* entries, int nentries) {
    WT_SESSION_IMPL* session;
 ...
 */
int __wt_cursor_search_near_notsup(WT_CURSOR* cursor, int* exact) {
    WT_UNUSED(exact);
 ...
 */
int __wt_cursor_reconfigure_notsup(WT_CURSOR* cursor, const char* config) {
    WT_UNUSED(config);
 ...
 */
int __wt_cursor_reopen_notsup(WT_CURSOR* cursor, bool check_only) {
 ...
 */
int __wt_cursor_kv_not_set(WT_CURSOR* cursor, bool key) WT_GCC_FUNC_ATTRIBUTE((cold)) {
    WT_SESSION_IMPL* session;
 ...
 */
int __wt_cursor_cache(WT_CURSOR* cursor, WT_DATA_HANDLE* dhandle) {
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
    uint64_t bucket;
 ...
 */
void __wt_cursor_reopen(WT_CURSOR* cursor, WT_DATA_HANDLE* dhandle) {
    WT_SESSION_IMPL* session;
    uint64_t bucket;
 ...
 * operations.
 */
int __wt_cursor_cache_release(WT_SESSION_IMPL* session, WT_CURSOR* cursor, bool* released) {
    WT_DECL_RET;
 ...
 */
int __wt_cursor_cache_get(WT_SESSION_IMPL* session,
                          const char* uri,
                          WT_CURSOR* to_dup,
                          const char* cfg[],
                          WT_CURSOR** cursorp) {
 ...
 */
void __wt_cursor_close(WT_CURSOR* cursor) {
    WT_SESSION_IMPL* session;
 ...
 */
static int __cursor_modify(WT_CURSOR* cursor, WT_MODIFY* entries, int nentries) {
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
