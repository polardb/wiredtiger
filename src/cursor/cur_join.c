...
static int __curjoin_entries_in_range(WT_SESSION_IMPL*,
                                      WT_CURSOR_JOIN*,
                                      WT_ITEM*,
                                      WT_CURSOR_JOIN_ITER*);
static int __curjoin_entry_in_range(WT_SESSION_IMPL*,
                                    WT_CURSOR_JOIN_ENTRY*,
                                    WT_ITEM*,
                                    WT_CURSOR_JOIN_ITER*);
static int __curjoin_entry_member(WT_SESSION_IMPL*,
                                  WT_CURSOR_JOIN_ENTRY*,
                                  WT_ITEM*,
                                  WT_CURSOR_JOIN_ITER*);
 ...
 */
int __wt_curjoin_joined(WT_CURSOR* cursor) WT_GCC_FUNC_ATTRIBUTE((cold)) {
 ...
 */
static int __curjoin_iter_init(WT_SESSION_IMPL* session,
 ...
 */
static int __curjoin_iter_close(WT_CURSOR_JOIN_ITER* iter) {
    WT_DECL_RET;
 ...
 */
static int __curjoin_iter_close_all(WT_CURSOR_JOIN_ITER* iter) {
    ...
    iter->child = NULL;
    WT_ASSERT(iter->session,
              iter->cjoin->parent == NULL || iter->cjoin->parent->iter->child == iter);
 ...
 */
static int __curjoin_iter_reset(WT_CURSOR_JOIN_ITER* iter) {
    if (iter->child != NULL)
 ...
 */
static bool __curjoin_iter_ready(WT_CURSOR_JOIN_ITER* iter) {
    while (iter != NULL) {
        if (!iter->positioned)
            return (false);
        iter = iter->child;
 ...
 */
static int __curjoin_iter_set_entry(WT_CURSOR_JOIN_ITER* iter, u_int entry_pos) {
    WT_CURSOR *c, *to_dup;
    ...
    iter->end_skip =
        (entry->ends_next > 0 && WT_CURJOIN_END_RANGE(&entry->ends[0]) == WT_CURJOIN_END_GE) ? 1
                                                                                             : 0;
 ...
 */
static int __curjoin_iter_bump(WT_CURSOR_JOIN_ITER* iter) {
    WT_CURSOR_JOIN_ENTRY* entry;
    WT_SESSION_IMPL* session;
 ...
 *
 */
static int __curjoin_iter_next(WT_CURSOR_JOIN_ITER* iter, WT_CURSOR* cursor) {
    WT_CURSOR_JOIN_ENTRY* entry;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
...
again:
     ...
     * to check membership.
     */
    WT_RET(__curjoin_split_key(iter->session,
                               iter->cjoin,
                               &iter->idxkey,
                               cursor,
                               iter->cursor,
                               iter->entry->repack_format,
 ...
 */
static int __curjoin_close(WT_CURSOR* cursor) {
    WT_CURSOR_JOIN* cjoin;
 ...
 */
static int __curjoin_endpoint_init_key(WT_SESSION_IMPL* session,
                                       WT_CURSOR_JOIN_ENTRY* entry,
 ...
 */
static int __curjoin_entries_in_range(WT_SESSION_IMPL* session,
                                      WT_CURSOR_JOIN* cjoin,
                                      WT_ITEM* curkey,
                                      WT_CURSOR_JOIN_ITER* iterarg) {
    WT_CURSOR_JOIN_ENTRY* entry;
    WT_CURSOR_JOIN_ITER* iter;
    WT_DECL_RET;
    u_int pos;
 ...
 *	WT_NOTFOUND if not.
 */
static int __curjoin_entry_in_range(WT_SESSION_IMPL* session,
                                    WT_CURSOR_JOIN_ENTRY* entry,
                                    WT_ITEM* curkey,
                                    WT_CURSOR_JOIN_ITER* iter) {
 ...
 */
static int __curjoin_extract_insert(WT_CURSOR* cursor) {
 ...
 */
static int __curjoin_entry_member(WT_SESSION_IMPL* session,
                                  WT_CURSOR_JOIN_ENTRY* entry,
                                  WT_ITEM* key,
                                  WT_CURSOR_JOIN_ITER* iter) {
    ...
    if (entry->subjoin != NULL) {
        ...
        ret = __curjoin_entries_in_range(
    ...
    if ((idx = entry->index) != NULL && idx->extractor != NULL &&
        ...
        WT_ERR(idx->extractor->extract(
            idx->extractor, &session->iface, key, &v, &extract_cursor.iface));
 ...
 */
static int __curjoin_get_key(WT_CURSOR* cursor, ...) {
    WT_CURSOR_JOIN* cjoin;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
    va_list ap;
 ...
 */
static int __curjoin_get_value(WT_CURSOR* cursor, ...) {
    WT_CURSOR_JOIN* cjoin;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
    va_list ap;
 ...
 */
static int __curjoin_init_bloom(WT_SESSION_IMPL* session,
                                WT_CURSOR_JOIN* cjoin,
                                WT_CURSOR_JOIN_ENTRY* entry,
                                WT_BLOOM* bloom) {
    WT_COLLATOR* collator;
 ...
 */
static int __curjoin_init_next(WT_SESSION_IMPL* session, WT_CURSOR_JOIN* cjoin, bool iterable) {
    WT_BLOOM* bloom;
    WT_CURSOR* origcur;
    WT_CURSOR_JOIN_ENDPOINT* end;
    ...
    for (je = cjoin->entries; je < jeend; je++) {
        ...
        if (iterable && je->ends_next == 1 && F_ISSET(&je->ends[0], WT_CURJOIN_END_LT)) {
            ...
            WT_ERR(__wt_open_cursor(session,
                                    origcur->uri,
                                    (WT_CURSOR*)cjoin,
        ...
        if (!iterable && F_ISSET(je, WT_CURJOIN_ENTRY_BLOOM)) {
            ...
            if (je->bloom == NULL) {
                 ...
                 */
                WT_ERR(__wt_bloom_create(session,
                                         NULL,
                                         NULL,
                                         je->count,
                                         je->bloom_bit_count,
 ...
 */
static int __curjoin_insert_endpoint(WT_SESSION_IMPL* session,
                                     WT_CURSOR_JOIN_ENTRY* entry,
                                     u_int pos,
 ...
 */
static int __curjoin_next(WT_CURSOR* cursor) {
    WT_CURSOR* c;
    WT_CURSOR_JOIN* cjoin;
    WT_CURSOR_JOIN_ITER* iter;
    WT_DECL_RET;
 ...
 */
static int __curjoin_open_main(WT_SESSION_IMPL* session,
                               WT_CURSOR_JOIN* cjoin,
                               WT_CURSOR_JOIN_ENTRY* entry) {
    WT_DECL_RET;
    WT_INDEX* idx;
 ...
 *
 */
static int __curjoin_pack_recno(
 ...
 */
static int __curjoin_reset(WT_CURSOR* cursor) {
    WT_CURSOR_JOIN* cjoin;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 *
 */
static int __curjoin_split_key(WT_SESSION_IMPL* session,
                               WT_CURSOR_JOIN* cjoin,
                               WT_ITEM* idxkey,
                               WT_CURSOR* tocur,
                               WT_CURSOR* fromcur,
                               const char* repack_fmt,
                               bool isindex) {
 ...
 */
int __wt_curjoin_open(WT_SESSION_IMPL* session,
                      const char* uri,
                      WT_CURSOR* owner,
                      const char* cfg[],
                      WT_CURSOR** cursorp) {
 ...
 */
int __wt_curjoin_join(WT_SESSION_IMPL* session,
                      WT_CURSOR_JOIN* cjoin,
                      WT_INDEX* idx,
