...
static int __json_unpack_put(WT_SESSION_IMPL*, void*, u_char*, size_t, WT_CONFIG_ITEM*, size_t*);
static inline int __json_struct_size(
    WT_SESSION_IMPL*, const void*, size_t, const char*, WT_CONFIG_ITEM*, bool, size_t*);
static inline int __json_struct_unpackv(WT_SESSION_IMPL*,
                                        const void*,
                                        size_t,
                                        const char*,
                                        WT_CONFIG_ITEM*,
 ...
 */
static int __json_unpack_put(WT_SESSION_IMPL* session,
                             void* voidpv,
                             u_char* buf,
                             size_t bufsz,
                             WT_CONFIG_ITEM* name,
 ...
 *	Calculate the size of a packed byte string as formatted for JSON.
 */
static inline int __json_struct_size(WT_SESSION_IMPL* session,
                                     const void* buffer,
                                     size_t size,
                                     const char* fmt,
 ...
 */
static inline int __json_struct_unpackv(WT_SESSION_IMPL* session,
                                        const void* buffer,
                                        size_t size,
                                        const char* fmt,
                                        WT_CONFIG_ITEM* names,
 ...
 */
int __wt_json_alloc_unpack(WT_SESSION_IMPL* session,
                           const void* buffer,
                           size_t size,
                           const char* fmt,
                           WT_CURSOR_JSON* json,
 ...
 */
void __wt_json_close(WT_SESSION_IMPL* session, WT_CURSOR* cursor) {
    WT_CURSOR_JSON* json;
 ...
 */
size_t __wt_json_unpack_char(u_char ch, u_char* buf, size_t bufsz, bool force_unicode)
 ...
 *	of column names.
 */
void __wt_json_column_init(WT_CURSOR* cursor,
                           const char* uri,
 ...
 *		'F'	:  false
 */
int __wt_json_token(WT_SESSION* wt_session,
                    const char* src,
                    int* toktype,
                    const char** tokstart,
 ...
 *	__wt_json_token.
 */
const char* __wt_json_tokname(int toktype) WT_GCC_FUNC_ATTRIBUTE((visibility("default"))) {
    switch (toktype) {
        case 0:
            return ("<EOF>");
        case 's':
 ...
 */
static int json_string_arg(WT_SESSION_IMPL* session, const char** jstr, WT_ITEM* item) {
    int tok;
    const char* tokstart;
 ...
 *	in the JSON string.
 */
static int json_int_arg(WT_SESSION_IMPL* session, const char** jstr, int64_t* ip) {
    size_t toksize;
    int tok;
    char* end;
    const char* tokstart;
 ...
 *	in the JSON string.
 */
static int json_uint_arg(WT_SESSION_IMPL* session, const char** jstr, uint64_t* up) {
    size_t toksize;
    int tok;
    char* end;
    const char* tokstart;
 ...
 *	if not.
 */
static int __json_pack_size(WT_SESSION_IMPL* session,
                            const char* fmt,
                            WT_CONFIG_ITEM* names,
                            bool iskey,
                            const char* jstr,
 ...
 */
int __wt_json_to_item(WT_SESSION_IMPL* session,
                      const char* jstr,
                      const char* format,
                      WT_CURSOR_JSON* json,
                      bool iskey,
 ...
 */
ssize_t __wt_json_strlen(const char* src, size_t srclen)
    WT_GCC_FUNC_ATTRIBUTE((visibility("default"))) {
    size_t dstlen;
    u_char hi, lo;
    const char* srcend;
 ...
 */
int __wt_json_strncpy(WT_SESSION* wt_session,
                      char** pdst,
                      size_t dstlen,
                      const char* src,
