 ...
 */
static int __curds_txn_enter(WT_SESSION_IMPL* session, bool update) {
 ...
 */
static int __curds_reserve(WT_CURSOR* cursor) {
    WT_CURSOR* source;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
int __wt_curds_open(WT_SESSION_IMPL* session,
                    const char* uri,
                    WT_CURSOR* owner,
                    const char* cfg[],
                    WT_DATA_SOURCE* dsrc,
