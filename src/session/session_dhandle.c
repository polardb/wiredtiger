 ...
 */
static int __session_add_dhandle(WT_SESSION_IMPL* session) {
 ...
 */
static void __session_discard_dhandle(WT_SESSION_IMPL* session,
                                      WT_DATA_HANDLE_CACHE* dhandle_cache) {
    uint64_t bucket;
 ...
 */
static void __session_find_dhandle(WT_SESSION_IMPL* session,
                                   const char* uri,
 ...
 *	file.
 */
int __wt_session_lock_dhandle(WT_SESSION_IMPL* session, uint32_t flags, bool* is_deadp) {
    WT_BTREE* btree;
    WT_DATA_HANDLE* dhandle;
    WT_DECL_RET;
 ...
 */
int __wt_session_release_dhandle(WT_SESSION_IMPL* session) {
    WT_BTREE* btree;
    WT_DATA_HANDLE* dhandle;
    WT_DATA_HANDLE_CACHE* dhandle_cache;
    WT_DECL_RET;
 ...
 */
int __wt_session_get_btree_ckpt(WT_SESSION_IMPL* session,
                                const char* uri,
                                const char* cfg[],
                                uint32_t flags) {
    WT_CONFIG_ITEM cval;
 ...
 */
static void __session_dhandle_sweep(WT_SESSION_IMPL* session) {
 ...
 */
static int __session_find_shared_dhandle(WT_SESSION_IMPL* session,
                                         const char* uri,
                                         const char* checkpoint) {
    WT_DECL_RET;
    ...
    WT_WITH_HANDLE_LIST_WRITE_LOCK(session,
                                   if ((ret = __wt_conn_dhandle_alloc(session, uri, checkpoint)) ==
                                       0) WT_DHANDLE_ACQUIRE(session->dhandle));
 ...
 *	connection.
 */
static int __session_get_dhandle(WT_SESSION_IMPL* session,
                                 const char* uri,
                                 const char* checkpoint) {
    WT_DATA_HANDLE_CACHE* dhandle_cache;
    WT_DECL_RET;
 ...
 */
int __wt_session_get_dhandle(WT_SESSION_IMPL* session,
                             const char* uri,
                             const char* checkpoint,
                             const char* cfg[],
                             uint32_t flags) {
...
int __wt_session_lock_checkpoint(WT_SESSION_IMPL* session, const char* checkpoint) {
     ...
     */
    WT_ERR(__wt_session_get_dhandle(session,
                                    saved_dhandle->name,
                                    checkpoint,
                                    NULL,
