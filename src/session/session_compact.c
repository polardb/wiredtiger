 ...
 */
static int __compact_start(WT_SESSION_IMPL* session) {
    WT_BM* bm;
 ...
 */
static int __compact_end(WT_SESSION_IMPL* session) {
    WT_BM* bm;
 ...
 */
static int __compact_uri_analyze(WT_SESSION_IMPL* session, const char* uri, bool* skipp) {
    /*
 ...
 *	Called via the schema_worker function.
 */
static int __compact_handle_append(WT_SESSION_IMPL* session, const char* cfg[]) {
    WT_DECL_RET;
 ...
 */
int __wt_session_compact_check_timeout(WT_SESSION_IMPL* session) {
    struct timespec end;
    ...
    return (session->compact->max_time > WT_TIMEDIFF_SEC(end, session->compact->begin) ? 0
                                                                                       : ETIMEDOUT);
}
 ...
 */
static int __compact_checkpoint(WT_SESSION_IMPL* session) {
    WT_DECL_RET;
 ...
 */
static int __compact_worker(WT_SESSION_IMPL* session) {
    WT_DECL_RET;
    u_int i, loop;
    bool another_pass;
...
int __wt_session_compact(WT_SESSION* wt_session, const char* uri, const char* config) {
    ...
    if (!WT_PREFIX_MATCH(uri, "colgroup:") && !WT_PREFIX_MATCH(uri, "file:") &&
        ...
        if ((dsrc = __wt_schema_get_source(session, uri)) != NULL)
            ret = dsrc->compact == NULL ? __wt_object_unsupported(session, uri)
 ...
 */
int __wt_session_compact_readonly(WT_SESSION* wt_session, const char* uri, const char* config) {
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
