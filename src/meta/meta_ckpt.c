 ...
 */
int __wt_meta_checkpoint(WT_SESSION_IMPL* session,
                         const char* fname,
 ...
 */
int __wt_meta_checkpoint_last_name(WT_SESSION_IMPL* session,
                                   const char* fname,
                                   const char** namep) {
    WT_DECL_RET;
    char* config;
 ...
 */
static int __ckpt_named(WT_SESSION_IMPL* session,
                        const char* checkpoint,
                        const char* config,
                        WT_CKPT* ckpt) {
    WT_CONFIG ckptconf;
 ...
 */
static int WT_CDECL __ckpt_compare_order(const void* a, const void* b) {
    WT_CKPT *ackpt, *bckpt;
 ...
 */
static int __ckpt_load(WT_SESSION_IMPL* session,
                       WT_CONFIG_ITEM* k,
                       WT_CONFIG_ITEM* v,
                       WT_CKPT* ckpt) {
    WT_CONFIG_ITEM a;
    ...
    WT_RET_NOTFOUND_OK(ret);
    ckpt->newest_start_ts = ret == WT_NOTFOUND || a.len == 0 ? WT_TS_NONE : (uint64_t)a.val;
    ...
    __wt_timestamp_addr_check(
        session, ckpt->oldest_start_ts, ckpt->newest_start_ts, ckpt->newest_stop_ts);
 ...
 */
int __wt_meta_ckptlist_set(WT_SESSION_IMPL* session,
                           const char* fname,
                           WT_CKPT* ckptbase,
                           WT_LSN* ckptlsn) {
    WT_CKPT* ckpt;
    ...
    WT_CKPT_FOREACH(ckptbase, ckpt) {
        ...
        if (strcmp(ckpt->name, WT_CHECKPOINT) == 0)
            WT_ERR(__wt_buf_catfmt(session, buf, ".%" PRId64, ckpt->order));
        ...
        WT_ERR(__wt_buf_catfmt(session,
                               ...
                               ",oldest_start_ts=%" PRId64 ",newest_start_ts=%" PRId64
                               ",newest_stop_ts=%" PRId64 ",write_gen=%" PRId64 ")",
                               (int)ckpt->addr.size,
                               (char*)ckpt->addr.data,
                               ckpt->order,
                               ckpt->sec,
 ...
 */
void __wt_meta_ckptlist_free(WT_SESSION_IMPL* session, WT_CKPT** ckptbasep) {
    WT_CKPT *ckpt, *ckptbase;
 ...
 */
int __wt_meta_sysinfo_set(WT_SESSION_IMPL* session) {
    WT_DECL_ITEM(buf);
    WT_DECL_RET;
