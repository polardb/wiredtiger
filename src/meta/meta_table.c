 ...
 */
static bool __metadata_turtle(const char* key) {
    switch (key[0]) {
        case 'C':
 ...
 */
int __wt_metadata_turtle_rewrite(WT_SESSION_IMPL* session) {
    WT_DECL_RET;
    char* value;
 ...
 */
int __wt_metadata_cursor_open(WT_SESSION_IMPL* session, const char* config, WT_CURSOR** cursorp) {
    WT_BTREE* btree;
    WT_DECL_RET;
 ...
 */
int __wt_metadata_cursor(WT_SESSION_IMPL* session, WT_CURSOR** cursorp) {
    WT_CURSOR* cursor;
 ...
 */
int __wt_metadata_cursor_release(WT_SESSION_IMPL* session, WT_CURSOR** cursorp) {
    WT_CURSOR* cursor;
...
int __wt_metadata_insert(WT_SESSION_IMPL* session, const char* key, const char* value) {
    ...
    __wt_verbose(session,
                 ...
                 key,
                 value,
                 WT_META_TRACKING(session) ? "true" : "false",
 ...
 */
int __wt_metadata_salvage(WT_SESSION_IMPL* session) {
