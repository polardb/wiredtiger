...
int util_load(WT_SESSION* session, int argc, char* argv[]) {
    uint32_t flags;
    int ch;
    const char* filename;
 ...
 */
static int load_dump(WT_SESSION* session) {
    WT_CURSOR* cursor;
    WT_DECL_RET;
    int tret;
 ...
 */
int config_exec(WT_SESSION* session, char** list) {
    WT_DECL_RET;
 ...
 */
int config_list_add(WT_SESSION* session, CONFIG_LIST* clp, char* val) {
 ...
 */
void config_list_free(CONFIG_LIST* clp) {
    char** entry;
 ...
 */
static int config_read(WT_SESSION* session, char*** listp, bool* hexp) {
    ULINE l;
    WT_DECL_RET;
    int entry, max_entry;
    bool eof;
 ...
 */
int config_reorder(WT_SESSION* session, char** list) {
    char **entry, *p;
 ...
 *	config we found.
 */
int config_update(WT_SESSION* session, char** list) {
    WT_DECL_RET;
    size_t cnt;
    int found;
    char **configp, **listp;
 ...
 *	Update the URI name.
 */
static int config_rename(WT_SESSION* session, char** urip, const char* name) {
    WT_DECL_RET;
    size_t len;
    char *buf, *p;
 ...
 */
static int format(WT_SESSION* session) {
 ...
 */
static int insert(WT_CURSOR* cursor, const char* name) {
    ULINE key, value;
    WT_DECL_RET;
    WT_SESSION* session;
