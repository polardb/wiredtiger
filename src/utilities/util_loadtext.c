...
int util_loadtext(WT_SESSION* session, int argc, char* argv[]) {
    WT_DECL_RET;
    int ch;
    char* uri;
 ...
 */
static int text(WT_SESSION* session, const char* uri) {
    WT_CURSOR* cursor;
    WT_DECL_RET;
    int tret;
    bool readkey;
 ...
 */
static int insert(WT_CURSOR* cursor, const char* name, bool readkey) {
    ULINE key, value;
    WT_DECL_RET;
    WT_SESSION* session;
