...
int util_cerr(WT_CURSOR* cursor, const char* op, int ret) {
 ...
 * 	Report an error.
 */
int util_err(WT_SESSION* session, int e, const char* fmt, ...) {
    va_list ap;
 ...
 */
int util_read_line(WT_SESSION* session, ULINE* l, bool eof_expected, bool* eofp) {
 ...
 */
int util_str2num(WT_SESSION* session, const char* p, bool endnul, uint64_t* vp) {
    uint64_t v;
    char* endptr;
 ...
 */
int util_flush(WT_SESSION* session, const char* uri) {
    WT_DECL_RET;
    size_t len;
    char* buf;
