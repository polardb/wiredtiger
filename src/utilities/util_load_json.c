 ...
 */
static int json_column_group_index(WT_SESSION* session,
                                   JSON_INPUT_STATE* ins,
                                   CONFIG_LIST* clp,
                                   int idx) {
    WT_DECL_RET;
    char *config, *p, *uri;
 ...
 */
static int json_kvraw_append(WT_SESSION* session,
                             JSON_INPUT_STATE* ins,
                             const char* str,
                             size_t len) {
    WT_DECL_RET;
 ...
 */
static int json_strdup(WT_SESSION* session, JSON_INPUT_STATE* ins, char** resultp) {
    WT_DECL_RET;
    size_t srclen;
    ssize_t resultlen;
 ...
 *	values.
 */
static int json_data(WT_SESSION* session, JSON_INPUT_STATE* ins, CONFIG_LIST* clp, uint32_t flags) {
    WT_CURSOR* cursor;
    WT_DECL_RET;
    ...
    while (json_peek(session, ins) == '{') {
        ...
        while (json_peek(session, ins) == 's') {
            ...
            if (++nfield == nkeys) {
                ...
                if ((ret = json_kvraw_append(session,
                                             ins,
 ...
 */
static int json_top_level(WT_SESSION* session, JSON_INPUT_STATE* ins, uint32_t flags) {
    CONFIG_LIST cl;
    WT_DECL_RET;
 ...
 */
static int json_peek(WT_SESSION* session, JSON_INPUT_STATE* ins) {
    WT_DECL_RET;
 ...
 */
static int json_expect(WT_SESSION* session, JSON_INPUT_STATE* ins, int wanttok) {
 ...
 *	that string.
 */
static int json_skip(WT_SESSION* session, JSON_INPUT_STATE* ins, const char** matches) {
    const char* hit;
    const char** match;
 ...
 */
int util_load_json(WT_SESSION* session, const char* filename, uint32_t flags) {
