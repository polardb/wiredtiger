...
int util_backup(WT_SESSION* session, int argc, char* argv[]) {
    WT_CURSOR* cursor;
    WT_DECL_ITEM(tmp);
    WT_DECL_RET;
    ...
    while ((ch = __wt_getopt(progname, argc, argv, "t:")) != EOF)
        switch (ch) {
            case 't':
                ...
                WT_ERR(
                    __wt_buf_catfmt(session_impl, tmp, "%s\"%s\"", target ? "," : "", __wt_optarg));
                target = true;
                break;
            case '?':
            default:
...
static int copy(WT_SESSION* session, const char* directory, const char* name) {
    WT_DECL_RET;
    size_t len;
    char* to;
