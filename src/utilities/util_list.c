...
int util_list(WT_SESSION* session, int argc, char* argv[]) {
    WT_DECL_RET;
    int ch;
    char* uri;
    bool cflag, vflag;
 ...
 */
static int list_get_allocsize(WT_SESSION* session, const char* key, size_t* allocsize) {
 ...
 */
static int list_print(WT_SESSION* session, const char* uri, bool cflag, bool vflag) {
    WT_CURSOR* cursor;
    WT_DECL_RET;
 ...
 */
static int list_print_checkpoint(WT_SESSION* session, const char* key) {
    WT_BLOCK_CKPT ci;
