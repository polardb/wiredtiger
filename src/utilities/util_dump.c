...
int util_dump(WT_SESSION* session, int argc, char* argv[]) {
    WT_CURSOR* cursor;
    WT_DECL_ITEM(tmp);
    WT_DECL_RET;
 ...
 */
static int dump_config(
    WT_SESSION* session, const char* uri, WT_CURSOR* cursor, bool hex, bool json) {
    WT_CURSOR* mcursor;
    WT_DECL_RET;
    int tret;
 ...
 */
static int dump_json_begin(WT_SESSION* session) {
    if (fprintf(fp, "{\n") < 0)
 ...
 */
static int dump_json_end(WT_SESSION* session) {
    if (fprintf(fp, "\n}\n") < 0)
        return (util_err(session, EIO, NULL));
    return (0);
}
 ...
 */
static int dump_json_separator(WT_SESSION* session) {
    if (fprintf(fp, ",\n") < 0)
        return (util_err(session, EIO, NULL));
    return (0);
}
 ...
 */
static int dump_json_table_end(WT_SESSION* session) {
    if (fprintf(fp, "            ]\n        }\n    ]") < 0)
        return (util_err(session, EIO, NULL));
    return (0);
}
 ...
 */
static int dump_add_config(WT_SESSION* session, char** bufp, size_t* leftp, const char* fmt, ...)
 ...
 */
static int dump_projection(WT_SESSION* session,
                           const char* config,
                           WT_CURSOR* cursor,
                           char** newconfigp) {
 ...
 *	Output the dump file header prefix.
 */
static int dump_prefix(WT_SESSION* session, bool hex, bool json) {
 ...
 */
static int dump_record(WT_CURSOR* cursor, bool reverse, bool json) {
    WT_DECL_RET;
    WT_SESSION* session;
 ...
 *	Output the dump file header suffix.
 */
static int dump_suffix(WT_SESSION* session, bool json) {
    if (json) {
        if (fprintf(fp,
                    "        },\n"
                    "        {\n"
 ...
 */
static int dup_json_string(const char* str, char** result) {
    size_t left, nchars;
    const char* p;
    char* q;
...
static int print_config(
    WT_SESSION* session, const char* key, const char* cfg, bool json, bool toplevel) {
    WT_DECL_RET;
    char* jsonconfig;
