 ...
 */
static inline int config_check_search(WT_SESSION_IMPL* session,
 ...
 * specified in an array of check strings.
 */
static int config_check(WT_SESSION_IMPL* session,
                        const WT_CONFIG_CHECK* checks,
    ...
    while ((ret = __wt_config_next(&parser, &k, &v)) == 0) {
        ...
        if (strcmp(checks[i].type, "boolean") == 0) {
            ...
            ret = config_check(session,
                               checks[i].subconfigs,
                               checks[i].subconfigs_entries,
                               k.str + strlen(checks[i].name) + 1,
                               v.len);
            if (ret != EINVAL)
                badtype = false;
            else
