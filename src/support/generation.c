 ...
 */
void __wt_gen_init(WT_SESSION_IMPL* session) {
    int i;
 ...
 */
uint64_t __wt_gen(WT_SESSION_IMPL* session, int which) {
 ...
 */
uint64_t __wt_gen_next(WT_SESSION_IMPL* session, int which) {
 ...
 */
uint64_t __wt_gen_next_drain(WT_SESSION_IMPL* session, int which) {
    uint64_t v;
 ...
 */
void __wt_gen_drain(WT_SESSION_IMPL* session, int which, uint64_t generation) {
 ...
 */
static uint64_t __gen_oldest(WT_SESSION_IMPL* session, int which) {
    WT_CONNECTION_IMPL* conn;
    WT_SESSION_IMPL* s;
    uint64_t oldest, v;
    uint32_t i, session_cnt;
 ...
 */
bool __wt_gen_active(WT_SESSION_IMPL* session, int which, uint64_t generation) {
    WT_CONNECTION_IMPL* conn;
    WT_SESSION_IMPL* s;
    uint64_t v;
    uint32_t i, session_cnt;
 ...
 */
uint64_t __wt_session_gen(WT_SESSION_IMPL* session, int which) {
 ...
 */
void __wt_session_gen_enter(WT_SESSION_IMPL* session, int which) {
    /*
 ...
 */
void __wt_session_gen_leave(WT_SESSION_IMPL* session, int which) {
 ...
 */
static void __stash_discard(WT_SESSION_IMPL* session, int which) {
    WT_CONNECTION_IMPL* conn;
 ...
 *	Discard any memory from a session stash that we can.
 */
void __wt_stash_discard(WT_SESSION_IMPL* session) {
    WT_SESSION_STASH* session_stash;
    int which;
 ...
 */
int __wt_stash_add(WT_SESSION_IMPL* session, int which, uint64_t generation, void* p, size_t len) {
    WT_CONNECTION_IMPL* conn;
    WT_SESSION_STASH* session_stash;
    WT_STASH* stash;
 ...
 */
void __wt_stash_discard_all(WT_SESSION_IMPL* session_safe, WT_SESSION_IMPL* session) {
    WT_SESSION_STASH* session_stash;
    WT_STASH* stash;
    size_t i;
    int which;
