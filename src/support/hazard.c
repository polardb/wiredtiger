 ...
 */
static int hazard_grow(WT_SESSION_IMPL* session) {
    WT_HAZARD* nhazard;
    size_t size;
    uint64_t hazard_gen;
    void* ohazard;
 ...
 */
int __wt_hazard_set(WT_SESSION_IMPL* session,
                    WT_REF* ref,
                    bool* busyp
#ifdef HAVE_DIAGNOSTIC
                    ,
 ...
 */
int __wt_hazard_clear(WT_SESSION_IMPL* session, WT_REF* ref) {
    WT_HAZARD* hp;
 ...
 */
static inline void hazard_get_reference(WT_SESSION_IMPL* session,
                                        WT_HAZARD** hazardp,
                                        uint32_t* hazard_inusep) {
    /*
 ...
 */
u_int __wt_hazard_count(WT_SESSION_IMPL* session, WT_REF* ref) {
    WT_HAZARD* hp;
 ...
 */
bool __wt_hazard_check_assert(WT_SESSION_IMPL* session, void* ref, bool waitfor) {
    WT_HAZARD* hp;
    WT_SESSION_IMPL* s;
    int i;
 ...
 */
static void __hazard_dump(WT_SESSION_IMPL* session) {
    WT_HAZARD* hp;
