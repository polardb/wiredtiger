 ...
 */
int __wt_modify_pack(WT_SESSION_IMPL* session,
                     WT_ITEM** modifyp,
                     WT_MODIFY* entries,
                     int nentries) {
    WT_ITEM* modify;
 ...
 */
static int __modify_apply_one(WT_SESSION_IMPL* session,
                              WT_CURSOR* cursor,
                              size_t data_size,
                              size_t offset,
                              size_t size,
 ...
 * interface.
 */
int __wt_modify_apply_api(WT_SESSION_IMPL* session,
                          WT_CURSOR* cursor,
                          WT_MODIFY* entries,
                          int nentries) WT_GCC_FUNC_ATTRIBUTE((visibility("default"))) {
    size_t modified;
    int i;

    for (modified = 0, i = 0; i < nentries; ++i) {
        modified += entries[i].size;
        WT_RET(__modify_apply_one(session,
                                  cursor,
                                  entries[i].data.size,
                                  entries[i].offset,
                                  entries[i].size,
 ...
 */
int __wt_modify_apply(WT_SESSION_IMPL* session, WT_CURSOR* cursor, const void* modify) {
