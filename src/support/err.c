 ...
 */
static int __handle_error_default(WT_EVENT_HANDLER* handler,
 ...
 */
static int __handle_message_default(WT_EVENT_HANDLER* handler,
                                    WT_SESSION* wt_session,
                                    const char* message) {
    WT_SESSION_IMPL* session;
 ...
 */
static int __handle_progress_default(WT_EVENT_HANDLER* handler,
                                     WT_SESSION* wt_session,
 ...
 */
static int __handle_close_default(WT_EVENT_HANDLER* handler,
                                  WT_SESSION* wt_session,
                                  WT_CURSOR* cursor) {
    WT_UNUSED(handler);
    WT_UNUSED(wt_session);
...
static WT_EVENT_HANDLER __event_handler_default = {__handle_error_default,
                                                   __handle_message_default,
                                                   __handle_progress_default,
                                                   __handle_close_default};
 ...
 */
static void __handler_failure(WT_SESSION_IMPL* session,
                              int error,
                              const char* which,
                              bool error_handler_failed) {
 ...
 */
static int __eventv(WT_SESSION_IMPL* session,
                    bool msg_event,
                    int error,
                    const char* func,
                    int line,
    ...
    WT_ERROR_APPEND(p,
                    ...
                    "[%" PRIuMAX ":%" PRIuMAX "][%s]",
                    (uintmax_t)ts.tv_sec,
                    (uintmax_t)ts.tv_nsec / WT_THOUSAND,
                    tid);
 ...
 */
void __wt_errx_func(WT_SESSION_IMPL* session, const char* func, int line, const char* fmt, ...)
 ...
 */
int __wt_set_return_func(WT_SESSION_IMPL* session, const char* func, int line, int err) {
 ...
 */
int __wt_ext_err_printf(WT_EXTENSION_API* wt_api, WT_SESSION* wt_session, const char* fmt, ...)
 ...
 * 	Verbose message.
 */
void __wt_verbose_worker(WT_SESSION_IMPL* session, const char* fmt, ...)
 ...
 */
int __wt_msg(WT_SESSION_IMPL* session, const char* fmt, ...) WT_GCC_FUNC_ATTRIBUTE((cold))
 ...
 */
int __wt_ext_msg_printf(WT_EXTENSION_API* wt_api, WT_SESSION* wt_session, const char* fmt, ...)
    WT_GCC_FUNC_ATTRIBUTE((format(printf, 3, 4))) {
    WT_DECL_ITEM(buf);
    WT_DECL_RET;
    WT_EVENT_HANDLER* handler;
 ...
 */
int __wt_panic(WT_SESSION_IMPL* session) WT_GCC_FUNC_ATTRIBUTE((cold))
    WT_GCC_FUNC_ATTRIBUTE((visibility("default"))) {
    /*
     * !!!
     * This function MUST handle a NULL WT_SESSION_IMPL handle.
 ...
 */
int __wt_illegal_value_func(WT_SESSION_IMPL* session, uintmax_t v, const char* func, int line)
    WT_GCC_FUNC_ATTRIBUTE((cold)) WT_GCC_FUNC_ATTRIBUTE((visibility("default"))) {
    __wt_err_func(session,
                  EINVAL,
                  func,
                  line,
                  "%s: 0x%" PRIxMAX,
 ...
 */
int __wt_inmem_unsupported_op(WT_SESSION_IMPL* session, const char* tag)
 ...
 * object type.
 */
int __wt_bad_object_type(WT_SESSION_IMPL* session, const char* uri) WT_GCC_FUNC_ATTRIBUTE((cold)) {
 ...
 */
int __wt_unexpected_object_type(WT_SESSION_IMPL* session, const char* uri, const char* expect)
    WT_GCC_FUNC_ATTRIBUTE((cold)) {
