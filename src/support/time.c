 ...
 */
static void __time_check_monotonic(WT_SESSION_IMPL* session, struct timespec* tsp) {
    /*
 ...
 */
void __wt_epoch(WT_SESSION_IMPL* session, struct timespec* tsp)
 ...
 */
void __wt_seconds(WT_SESSION_IMPL* session, uint64_t* secondsp)
    WT_GCC_FUNC_ATTRIBUTE((visibility("default"))) {
    struct timespec t;
 ...
 */
uint64_t __wt_clock_to_nsec(uint64_t end, uint64_t begin) {
    double clock_diff;
