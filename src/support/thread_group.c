 ...
 */
static WT_THREAD_RET __thread_run(void* arg) {
    WT_DECL_RET;
 ...
 */
static int __thread_group_shrink(WT_SESSION_IMPL* session,
 ...
 */
static int __thread_group_resize(WT_SESSION_IMPL* session,
                                 WT_THREAD_GROUP* group,
                                 uint32_t new_min,
                                 uint32_t new_max,
                                 uint32_t flags) {
    ...
    if (new_min > new_max)
        WT_RET_MSG(session,
                   EINVAL,
                   "Illegal thread group resize: %s, from min: %" PRIu32 " -> %" PRIu32
 ...
 */
int __wt_thread_group_resize(WT_SESSION_IMPL* session,
                             WT_THREAD_GROUP* group,
                             uint32_t new_min,
                             uint32_t new_max,
                             uint32_t flags) {
 ...
 *	zero initialized.
 */
int __wt_thread_group_create(WT_SESSION_IMPL* session,
                             WT_THREAD_GROUP* group,
                             const char* name,
                             uint32_t min,
                             uint32_t max,
 ...
 */
int __wt_thread_group_destroy(WT_SESSION_IMPL* session, WT_THREAD_GROUP* group) {
    WT_DECL_RET;
 ...
 */
void __wt_thread_group_start_one(WT_SESSION_IMPL* session, WT_THREAD_GROUP* group, bool is_locked) {
    WT_THREAD* thread;
 ...
 */
void __wt_thread_group_stop_one(WT_SESSION_IMPL* session, WT_THREAD_GROUP* group) {
    WT_THREAD* thread;
