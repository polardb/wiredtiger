...
int __wt_stat_dsrc_desc(WT_CURSOR_STAT* cst, int slot, const char** p) {
    WT_UNUSED(cst);
    *p = __stats_dsrc_desc[slot];
    return (0);
}

void __wt_stat_dsrc_init_single(WT_DSRC_STATS* stats) {
    memset(stats, 0, sizeof(*stats));
}

int __wt_stat_dsrc_init(WT_SESSION_IMPL* session, WT_DATA_HANDLE* handle) {
    int i;
...
void __wt_stat_dsrc_discard(WT_SESSION_IMPL* session, WT_DATA_HANDLE* handle) {
    __wt_free(session, handle->stat_array);
}

void __wt_stat_dsrc_clear_single(WT_DSRC_STATS* stats) {
...
void __wt_stat_dsrc_clear_all(WT_DSRC_STATS** stats) {
    u_int i;
...
void __wt_stat_dsrc_aggregate_single(WT_DSRC_STATS* from, WT_DSRC_STATS* to) {
...
void __wt_stat_dsrc_aggregate(WT_DSRC_STATS** from, WT_DSRC_STATS* to) {
    int64_t v;
...
int __wt_stat_connection_desc(WT_CURSOR_STAT* cst, int slot, const char** p) {
    WT_UNUSED(cst);
    *p = __stats_connection_desc[slot];
    return (0);
}

void __wt_stat_connection_init_single(WT_CONNECTION_STATS* stats) {
    memset(stats, 0, sizeof(*stats));
}

int __wt_stat_connection_init(WT_SESSION_IMPL* session, WT_CONNECTION_IMPL* handle) {
    int i;
...
void __wt_stat_connection_discard(WT_SESSION_IMPL* session, WT_CONNECTION_IMPL* handle) {
    __wt_free(session, handle->stat_array);
}

void __wt_stat_connection_clear_single(WT_CONNECTION_STATS* stats) {
...
void __wt_stat_connection_clear_all(WT_CONNECTION_STATS** stats) {
    u_int i;
...
void __wt_stat_connection_aggregate(WT_CONNECTION_STATS** from, WT_CONNECTION_STATS* to) {
    int64_t v;
    ...
    to->cache_eviction_force_time += WT_STAT_READ(from, cache_eviction_force_time);
    to->cache_eviction_force_delete += WT_STAT_READ(from, cache_eviction_force_delete);
    to->cache_eviction_force_delete_time += WT_STAT_READ(from, cache_eviction_force_delete_time);
...
int __wt_stat_join_desc(WT_CURSOR_STAT* cst, int slot, const char** p) {
    WT_UNUSED(cst);
    *p = __stats_join_desc[slot];
    return (0);
}

void __wt_stat_join_init_single(WT_JOIN_STATS* stats) {
    memset(stats, 0, sizeof(*stats));
}

void __wt_stat_join_clear_single(WT_JOIN_STATS* stats) {
    stats->main_access = 0;
    stats->bloom_false_positive = 0;
...
void __wt_stat_join_clear_all(WT_JOIN_STATS** stats) {
    u_int i;
...
void __wt_stat_join_aggregate(WT_JOIN_STATS** from, WT_JOIN_STATS* to) {
...
int __wt_stat_session_desc(WT_CURSOR_STAT* cst, int slot, const char** p) {
    WT_UNUSED(cst);
    *p = __stats_session_desc[slot];
    return (0);
}

void __wt_stat_session_init_single(WT_SESSION_STATS* stats) {
    memset(stats, 0, sizeof(*stats));
}

void __wt_stat_session_clear_single(WT_SESSION_STATS* stats) {
    stats->bytes_read = 0;
    stats->bytes_write = 0;
