 ...
 *	Alter an object
 */
static int __alter_apply(WT_SESSION_IMPL* session,
                         const char* uri,
                         const char* newcfg[],
 ...
 *	Alter a file.
 */
static int __alter_file(WT_SESSION_IMPL* session, const char* newcfg[]) {
    const char* uri;
 ...
 */
static int __alter_tree(WT_SESSION_IMPL* session, const char* name, const char* newcfg[]) {
    WT_CONFIG_ITEM cval;
 ...
 *	Alter a table.
 */
static int __alter_table(WT_SESSION_IMPL* session,
                         const char* uri,
                         const char* newcfg[],
 ...
 *	Alter an object.
 */
static int __schema_alter(WT_SESSION_IMPL* session, const char* uri, const char* newcfg[]) {
    WT_CONFIG_ITEM cv;
    uint32_t flags;
    bool exclusive_refreshed;
 ...
 *	Alter an object.
 */
int __wt_schema_alter(WT_SESSION_IMPL* session, const char* uri, const char* newcfg[]) {
    WT_DECL_RET;
