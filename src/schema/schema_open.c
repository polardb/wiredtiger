 ...
 */
int __wt_schema_open_colgroups(WT_SESSION_IMPL* session, WT_TABLE* table) {
 ...
 *	Open an index.
 */
static int __open_index(WT_SESSION_IMPL* session, WT_TABLE* table, WT_INDEX* idx) {
    WT_CONFIG colconf;
 ...
 */
static int __schema_open_index(
 ...
 */
int __wt_schema_open_index(
    WT_SESSION_IMPL* session, WT_TABLE* table, const char* idxname, size_t len, WT_INDEX** indexp) {
    WT_DECL_RET;

    WT_WITH_TABLE_WRITE_LOCK(
        session,
        WT_WITH_TXN_ISOLATION(session,
 ...
 */
int __wt_schema_open_indices(WT_SESSION_IMPL* session, WT_TABLE* table) {
 ...
 */
static int __schema_open_table(WT_SESSION_IMPL* session, const char* cfg[]) {
    WT_CONFIG cparser;
    WT_CONFIG_ITEM ckey, cval;
    WT_DECL_RET;
    WT_TABLE* table;
 ...
 */
int __wt_schema_get_colgroup(WT_SESSION_IMPL* session,
                             const char* uri,
                             bool quiet,
                             WT_TABLE** tablep,
 ...
 *	Open a named table.
 */
int __wt_schema_open_table(WT_SESSION_IMPL* session, const char* cfg[]) {
    WT_DECL_RET;
