 ...
 */
int __wt_log_printf(WT_SESSION_IMPL* session, const char* format, ...) {
    WT_DECL_RET;
    va_list ap;
 ...
 */
static bool __log_checksum_match(WT_SESSION_IMPL* session, WT_ITEM* buf, uint32_t reclen) {
 ...
 */
static int __log_get_files(WT_SESSION_IMPL* session,
 ...
 */
static int __log_get_files_single(WT_SESSION_IMPL* session,
                                  const char* file_prefix,
                                  char*** filesp,
                                  u_int* countp) {
    WT_CONNECTION_IMPL* conn;
 ...
 */
static int __log_prealloc_remove(WT_SESSION_IMPL* session) {
    WT_DECL_RET;
    WT_LOG* log;
    uint32_t lognum;
    u_int i, logcount;
 ...
 */
static void __log_wait_for_earlier_slot(WT_SESSION_IMPL* session, WT_LOGSLOT* slot) {
    WT_CONNECTION_IMPL* conn;
    WT_LOG* log;
    int yield_count;
 ...
 *	thread as needed.
 */
void __wt_log_ckpt(WT_SESSION_IMPL* session, WT_LSN* ckp_lsn) {
    WT_CONNECTION_IMPL* conn;
    WT_LOG* log;
 ...
 */
int __wt_log_flush_lsn(WT_SESSION_IMPL* session, WT_LSN* lsn, bool start) {
    WT_CONNECTION_IMPL* conn;
    WT_LOG* log;
 ...
 *	thread as needed.
 */
void __wt_log_background(WT_SESSION_IMPL* session, WT_LSN* lsn) {
    WT_CONNECTION_IMPL* conn;
    WT_LOG* log;
 ...
 */
int __wt_log_force_sync(WT_SESSION_IMPL* session, WT_LSN* min_lsn) {
    WT_DECL_RET;
    WT_FH* log_fh;
    WT_LOG* log;
    ...
    if (log->sync_dir_lsn.l.file < min_lsn->l.file) {
        __wt_verbose(session,
                     WT_VERB_LOG,
                     "log_force_sync: sync directory %s to LSN %" PRIu32 "/%" PRIu32,
                     log->log_dir_fh->name,
                     min_lsn->l.file,
                     min_lsn->l.offset);
 ...
 */
int __wt_log_needs_recovery(WT_SESSION_IMPL* session, WT_LSN* ckp_lsn, bool* recp) {
    WT_CONNECTION_IMPL* conn;
    WT_CURSOR* c;
    WT_DECL_RET;
 ...
 */
static int __log_filename(WT_SESSION_IMPL* session,
                          uint32_t id,
                          const char* file_prefix,
                          WT_ITEM* buf) {
    return (__wt_filename_construct(
        session, S2C(session)->log_path, file_prefix, UINTMAX_MAX, id, buf));
}
 ...
 */
int __wt_log_reset(WT_SESSION_IMPL* session, uint32_t lognum) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_LOG* log;
    uint32_t old_lognum;
 ...
 *	Zero a log file.
 */
static int __log_zero(WT_SESSION_IMPL* session, WT_FH* fh, wt_off_t start_off, wt_off_t len) {
    WT_CONNECTION_IMPL* conn;
    ...
    while (off < (uint32_t)len) {
        ...
        WT_ERR(__wt_write(session, fh, (wt_off_t)off, wrlen, zerobuf->mem));
        off += wrlen;
    }
err:
...
static int __log_prealloc(WT_SESSION_IMPL* session, WT_FH* fh) {
    ...
    if (FLD_ISSET(conn->log_flags, WT_CONN_LOG_ZERO_FILL))
        return (__log_zero(session, fh, log->first_record, conn->log_file_max));
...
static int __log_decompress(WT_SESSION_IMPL* session, WT_ITEM* in, WT_ITEM* out) {
    ...
    WT_RET(compressor->decompress(compressor,
                                  &session->iface,
                                  (uint8_t*)in->mem + skip,
                                  in->size - skip,
                                  (uint8_t*)out->mem + skip,
                                  uncompressed_size - skip,
                                  &result_len));
 ...
 */
static int __log_decrypt(WT_SESSION_IMPL* session, WT_ITEM* in, WT_ITEM* out) {
    WT_CONNECTION_IMPL* conn;
 ...
 */
static int __log_file_header(WT_SESSION_IMPL* session, WT_FH* fh, WT_LSN* end_lsn, bool prealloc) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_ITEM(buf);
    WT_DECL_RET;
    WT_LOG* log;
 ...
 */
static int __log_openfile(WT_SESSION_IMPL* session, uint32_t id, uint32_t flags, WT_FH** fhp) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_ITEM(buf);
    WT_DECL_RET;
    u_int wtopen_flags;
 ...
 *	this log file.
 */
static int __log_open_verify(WT_SESSION_IMPL* session,
                             uint32_t id,
                             WT_FH** fhp,
                             WT_LSN* lsnp,
                             uint16_t* versionp,
 ...
 */
static int __log_record_verify(WT_SESSION_IMPL* session,
                               WT_FH* log_fh,
                               uint32_t offset,
                               WT_LOG_RECORD* logrecp,
                               bool* corrupt) {
    WT_LOG_RECORD logrec;
    size_t i;
    ...
    *corrupt = false;
    ...
    if (F_ISSET(&logrec, ~(WT_LOG_RECORD_ALL_FLAGS))) {
        WT_RET(__wt_msg(session,
                        "%s: log record at position %" PRIu32 " has flag corruption 0x%" PRIx16,
                        log_fh->name,
                        offset,
                        logrec.flags));
        *corrupt = true;
    ...
    if (logrec.mem_len != 0 &&
        ...
        WT_RET(__wt_msg(session,
                        ...
                        " has memory len corruption 0x%" PRIx32,
                        log_fh->name,
                        offset,
                        logrec.mem_len));
        *corrupt = true;
    ...
    if (logrec.len <= offsetof(WT_LOG_RECORD, record)) {
        WT_RET(__wt_msg(session,
                        "%s: log record at position %" PRIu32
                        " has record len corruption 0x%" PRIx32,
                        log_fh->name,
                        offset,
                        logrec.len));
        *corrupt = true;
...
static int __log_alloc_prealloc(WT_SESSION_IMPL* session, uint32_t to_num) {
    ...
    log = conn->log;
    logfiles = NULL;
    WT_ERR(__log_get_files_single(session, WT_LOG_PREPNAME, &logfiles, &logcount));
    if (logcount == 0)
        return (WT_NOTFOUND);
    ...
    __wt_verbose(session,
                 ...
                 (const char*)from_path->data,
                 (const char*)to_path->data);
 ...
 */
static int __log_newfile(WT_SESSION_IMPL* session, bool conn_open, bool* created) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_FH* log_fh;
    WT_LOG* log;
 ...
 *	pre-allocated files.
 */
int __wt_log_set_version(WT_SESSION_IMPL* session,
                         uint16_t version,
                         uint32_t first_rec,
                         bool downgrade,
                         bool live_chg,
 ...
 */
int __wt_log_acquire(WT_SESSION_IMPL* session, uint64_t recsize, WT_LOGSLOT* slot) {
    WT_CONNECTION_IMPL* conn;
    WT_LOG* log;
    bool created_log;
 ...
 */
static int __log_truncate_file(WT_SESSION_IMPL* session, WT_FH* log_fh, wt_off_t offset) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_LOG* log;
 ...
 */
static int __log_truncate(WT_SESSION_IMPL* session, WT_LSN* lsn, bool this_log, bool salvage_mode) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_FH* log_fh;
    WT_LOG* log;
    ...
    if (salvage_first != 0) {
        if (salvage_last > salvage_first)
            WT_TRET(__wt_msg(session,
                             "salvage: log files %" PRIu32 "-%" PRIu32 " truncated at beginning",
                             salvage_first,
                             salvage_last));
        else
            WT_TRET(__wt_msg(
 ...
 */
int __wt_log_allocfile(WT_SESSION_IMPL* session, uint32_t lognum, const char* dest) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_ITEM(from_path);
    WT_DECL_ITEM(to_path);
    WT_DECL_RET;
...
int __wt_log_open(WT_SESSION_IMPL* session) {
    ...
    if (log->log_dir_fh == NULL) {
        ...
        WT_RET(__wt_open(
            session, conn->log_path, WT_FS_OPEN_FILE_TYPE_DIRECTORY, 0, &log->log_dir_fh));
    }
 ...
 *	file is zeroes.
 */
static int __log_has_hole(WT_SESSION_IMPL* session,
                          WT_FH* fh,
                          wt_off_t log_size,
                          wt_off_t offset,
                          wt_off_t* error_offset,
                          bool* hole) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_LOG* log;
 ...
 */
static bool __log_check_partial_write(WT_SESSION_IMPL* session, WT_ITEM* buf, uint32_t reclen) {
    uint8_t* rec;
 ...
 *	Release a log slot.
 */
int __wt_log_release(WT_SESSION_IMPL* session, WT_LOGSLOT* slot, bool* freep) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_LOG* log;
    WT_LSN sync_lsn;
    ...
    while (F_ISSET(slot, WT_SLOT_SYNC | WT_SLOT_SYNC_DIR)) {
        ...
        if (F_ISSET(slot, WT_SLOT_SYNC_DIR) && (log->sync_dir_lsn.l.file < sync_lsn.l.file)) {
            ...
            __wt_verbose(session,
                         WT_VERB_LOG,
                         "log_release: sync directory %s to LSN %" PRIu32 "/%" PRIu32,
                         log->log_dir_fh->name,
                         sync_lsn.l.file,
                         sync_lsn.l.offset);
            time_start = __wt_clock(session);
        ...
        if (F_ISSET(slot, WT_SLOT_SYNC) && __wt_log_cmp(&log->sync_lsn, &slot->slot_end_lsn) < 0) {
            __wt_verbose(session,
                         WT_VERB_LOG,
                         "log_release: sync log %s to LSN %" PRIu32 "/%" PRIu32,
                         log->log_fh->name,
                         sync_lsn.l.file,
                         sync_lsn.l.offset);
            WT_STAT_CONN_INCR(session, log_sync);
 ...
 */
static int __log_salvage_message(WT_SESSION_IMPL* session,
                                 const char* log_name,
                                 const char* extra_msg,
                                 wt_off_t offset) {
    WT_RET(__wt_msg(session,
                    "log file %s corrupted%s at position %" PRIuMAX ", truncated",
                    log_name,
                    extra_msg,
                    (uintmax_t)offset));
 ...
 */
int __wt_log_scan(WT_SESSION_IMPL* session,
                  WT_LSN* lsnp,
                  uint32_t flags,
        ...
        if (!__log_checksum_match(session, buf, reclen)) {
            ...
            if (!__log_check_partial_write(session, buf, reclen)) {
                ...
                WT_TRET(__log_salvage_message(
                    session, log_fh->name, ", bad checksum", rd_lsn.l.offset));
            } else {
                /*
...
err:
    ...
    if (ret != 0 && firstrecord && LF_ISSET(WT_LOGSCAN_RECOVER)) {
        __wt_errx(session, "WiredTiger is unable to read the recovery log.");
        __wt_errx(session,
 ...
 */
int __wt_log_force_write(WT_SESSION_IMPL* session, bool retry, bool* did_work) {
    WT_LOG* log;
    WT_MYSLOT myslot;
...
int __wt_log_write(WT_SESSION_IMPL* session, WT_ITEM* record, WT_LSN* lsnp, uint32_t flags) {
    ...
    if ((compressor = conn->log_compressor) != NULL && record->size < log->allocsize) {
        ...
        WT_ERR(compressor->compress(compressor,
                                    &session->iface,
                                    src,
                                    src_len,
                                    dst,
 ...
 */
static int __log_write_internal(WT_SESSION_IMPL* session,
                                WT_ITEM* record,
                                WT_LSN* lsnp,
                                uint32_t flags) {
    WT_CONNECTION_IMPL* conn;
...
err:
     ...
     */
    WT_ASSERT(session,
              ret != 0 || !LF_ISSET(WT_LOG_FLUSH) || __wt_log_cmp(&log->write_lsn, &lsn) >= 0);
    WT_ASSERT(session,
              ret != 0 || !LF_ISSET(WT_LOG_FSYNC) || __wt_log_cmp(&log->sync_lsn, &lsn) >= 0);
    return (ret);
}
 ...
 */
int __wt_log_flush(WT_SESSION_IMPL* session, uint32_t flags) {
    WT_CONNECTION_IMPL* conn;
    WT_LOG* log;
    WT_LSN last_lsn, lsn;
