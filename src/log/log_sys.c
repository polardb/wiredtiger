 ...
 */
int __wt_log_system_record(WT_SESSION_IMPL* session, WT_FH* log_fh, WT_LSN* lsn) {
 ...
 */
int __wt_log_recover_system(WT_SESSION_IMPL* session,
                            const uint8_t** pp,
                            const uint8_t* end,
                            WT_LSN* lsnp) {
    WT_DECL_RET;
 ...
 */
int __wt_verbose_dump_log(WT_SESSION_IMPL* session) {
    ...
    WT_RET(__wt_msg(
        ...
        !FLD_ISSET(conn->txn_logsync, WT_LOG_SYNC_ENABLED)
            ? "none"
            : FLD_ISSET(conn->txn_logsync, WT_LOG_DSYNC) ? "dsync" : FLD_ISSET(conn->txn_logsync,
                                                                               WT_LOG_FLUSH)
                    ? "write to OS"
