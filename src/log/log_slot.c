 ...
 */
static void __log_slot_dump(WT_SESSION_IMPL* session) {
 ...
 */
void __wt_log_slot_activate(WT_SESSION_IMPL* session, WT_LOGSLOT* slot) {
    WT_CONNECTION_IMPL* conn;
    WT_LOG* log;
 ...
 */
static int __log_slot_close(WT_SESSION_IMPL* session,
                            WT_LOGSLOT* slot,
                            bool* releasep,
                            bool forced) {
    WT_CONNECTION_IMPL* conn;
 ...
 */
static void __log_slot_dirty_max_check(WT_SESSION_IMPL* session, WT_LOGSLOT* slot) {
    WT_CONNECTION_IMPL* conn;
    WT_LOG* log;
 ...
 */
static int __log_slot_new(WT_SESSION_IMPL* session) {
    WT_CONNECTION_IMPL* conn;
    WT_LOG* log;
    WT_LOGSLOT* slot;
    int32_t i, pool_i;
 ...
 */
static int __log_slot_switch_internal(WT_SESSION_IMPL* session,
                                      WT_MYSLOT* myslot,
                                      bool forced,
                                      bool* did_work) {
    WT_DECL_RET;
 ...
 */
int __wt_log_slot_init(WT_SESSION_IMPL* session, bool alloc) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_LOG* log;
    WT_LOGSLOT* slot;
 ...
 */
void __wt_log_slot_join(WT_SESSION_IMPL* session,
                        uint64_t mysize,
                        uint32_t flags,
                        WT_MYSLOT* myslot) {
    WT_CONNECTION_IMPL* conn;
 ...
 *	the memory buffer.
 */
int64_t __wt_log_slot_release(WT_MYSLOT* myslot, int64_t size) {
    WT_LOGSLOT* slot;
 ...
 */
void __wt_log_slot_free(WT_SESSION_IMPL* session, WT_LOGSLOT* slot) {
    /*
