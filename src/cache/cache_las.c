 ...
 */
static void __las_set_isolation(WT_SESSION_IMPL* session, WT_TXN_ISOLATION* saved_isolationp) {
 ...
 *	Restore isolation.
 */
static void __las_restore_isolation(WT_SESSION_IMPL* session, WT_TXN_ISOLATION saved_isolation) {
 ...
 */
static uint64_t __las_entry_count(WT_CACHE* cache) {
 ...
 *	Return when there are entries in the lookaside table.
 */
bool __wt_las_empty(WT_SESSION_IMPL* session) {
 ...
 */
void __wt_las_stats_update(WT_SESSION_IMPL* session) {
    WT_CACHE* cache;
 ...
 */
int __wt_las_create(WT_SESSION_IMPL* session) {
    WT_CACHE* cache;
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    int i;
 ...
 */
int __wt_las_destroy(WT_SESSION_IMPL* session) {
    WT_CACHE* cache;
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
 ...
 */
int __wt_las_cursor_open(WT_SESSION_IMPL* session) {
    WT_BTREE* btree;
    WT_CURSOR* cursor;
    WT_DECL_RET;
    ...
    WT_WITHOUT_DHANDLE(session,
                       ret = __wt_open_cursor(session, WT_LAS_URI, NULL, open_cursor_cfg, &cursor));
    WT_RET(ret);
 ...
 */
void __wt_las_cursor(WT_SESSION_IMPL* session, WT_CURSOR** cursorp, uint32_t* session_flags) {
    WT_CACHE* cache;
    int i;
 ...
 */
int __wt_las_cursor_close(WT_SESSION_IMPL* session, WT_CURSOR** cursorp, uint32_t session_flags) {
    WT_CACHE* cache;
    WT_CURSOR* cursor;
    WT_DECL_RET;
    int i;
 ...
 */
bool __wt_las_page_skip_locked(WT_SESSION_IMPL* session, WT_REF* ref) {
    WT_TXN* txn;
 ...
 */
bool __wt_las_page_skip(WT_SESSION_IMPL* session, WT_REF* ref) {
 ...
 */
static int __las_remove_block(WT_CURSOR* cursor,
                              uint64_t pageid,
                              bool lock_wait,
                              uint64_t* remove_cntp) {
    WT_CONNECTION_IMPL* conn;
    WT_DECL_RET;
    WT_ITEM las_key;
 ...
 */
static void __las_insert_block_verbose(WT_SESSION_IMPL* session, WT_BTREE* btree, WT_MULTI* multi) {
    WT_CACHE* cache;
    WT_CONNECTION_IMPL* conn;
...
int __wt_las_insert_block(
    ...
    for (las_counter = 0, i = 0, list = multi->supd; i < multi->supd_entries; ++i, ++list) {
            ...
            case WT_PAGE_ROW_LEAF:
                if (list->ins == NULL) {
                    WT_WITH_BTREE(session,
                                  btree,
                                  ret = __wt_row_leaf_key(session, page, list->ripcip, key, false));
                    WT_ERR(ret);
                } else {
        ...
        upd = list->ins == NULL ? page->modify->mod_row_update[slot] : list->ins->upd;
 ...
 */
int __wt_las_cursor_position(WT_CURSOR* cursor, uint64_t pageid) {
    WT_ITEM las_key;
    uint64_t las_counter, las_pageid;
    uint32_t las_id;
    int exact;
 ...
 */
int __wt_las_remove_block(WT_SESSION_IMPL* session, uint64_t pageid) {
    WT_CONNECTION_IMPL* conn;
    WT_CURSOR* cursor;
    WT_DECL_RET;
    uint64_t remove_cnt;
 ...
 */
int __wt_las_save_dropped(WT_SESSION_IMPL* session) {
    WT_BTREE* btree;
    WT_CACHE* cache;
    WT_DECL_RET;
 ...
 */
static inline uint64_t __las_sweep_count(WT_CACHE* cache) {
 ...
 */
static int __las_sweep_init(WT_SESSION_IMPL* session) {
    WT_CACHE* cache;
    WT_DECL_RET;
    u_int i;
 ...
 */
int __wt_las_sweep(WT_SESSION_IMPL* session) {
    WT_CACHE* cache;
    WT_CURSOR* cursor;
