 ...
 */
static int __verify_config_offsets(WT_SESSION_IMPL* session, const char* cfg[], bool* quitp) {
    WT_CONFIG list;
 ...
 *	Dump the tree shape.
 */
static int __verify_layout(WT_SESSION_IMPL* session, WT_VSTUFF* vs) {
    size_t i;
    uint64_t total;
...
int __wt_verify(WT_SESSION_IMPL* session, const char* cfg[]) {
    ...
    WT_CKPT_FOREACH(ckptbase, ckpt) {
        __wt_verbose(
            session, WT_VERB_VERIFY, "%s: checkpoint %s", btree->dhandle->name, ckpt->name);
        ...
        if (root_addr_size != 0) {
            ...
            WT_WITH_PAGE_INDEX(session,
                               ret = __verify_tree(session, &btree->root, &addr_unpack, vs));
 ...
 */
static int __verify_addr_ts(WT_SESSION_IMPL* session,
                            WT_REF* ref,
 ...
 */
static int __verify_tree(WT_SESSION_IMPL* session,
                         WT_REF* ref,
                         WT_CELL_UNPACK* addr_unpack,
                         WT_VSTUFF* vs) {
    WT_BM* bm;
 ...
 */
static int __verify_overflow(WT_SESSION_IMPL* session,
                             const uint8_t* addr,
                             size_t addr_size,
                             WT_VSTUFF* vs) {
    WT_BM* bm;
 ...
 */
static int __verify_ts_addr_cmp(WT_SESSION_IMPL* session,
                                WT_REF* ref,
                                uint32_t cell_num,
                                const char* ts1_name,
                                wt_timestamp_t ts1,
 ...
 */
static int __verify_page_cell(WT_SESSION_IMPL* session,
                              WT_REF* ref,
                              WT_CELL_UNPACK* addr_unpack,
                              WT_VSTUFF* vs) {
    WT_BTREE* btree;
    ...
    WT_CELL_FOREACH_BEGIN(session, btree, dsk, unpack, false) {
        ...
        switch (unpack.type) {
            ...
            case WT_CELL_ADDR_LEAF_NO:
                ...
                WT_RET(__verify_ts_addr_cmp(session,
                                            ref,
                                            cell_num - 1,
                                            "oldest start",
