 ...
 */
int __wt_debug_set_verbose(WT_SESSION_IMPL* session, const char* v) {
    char buf[256];
 ...
 */
static inline int __debug_hex_byte(WT_DBG* ds, uint8_t v) {
 ...
 */
static int __debug_bytes(WT_DBG* ds, const void* data_arg, size_t size) {
    size_t i;
    const uint8_t* data;
    u_char ch;
 ...
 */
static int __debug_item(WT_DBG* ds, const char* tag, const void* data_arg, size_t size) {
 ...
 */
static int __debug_item_key(WT_DBG* ds, const char* tag, const void* data_arg, size_t size) {
 ...
 */
static int __debug_item_value(WT_DBG* ds, const char* tag, const void* data_arg, size_t size) {
    WT_SESSION_IMPL* session;
 ...
 */
static int __dmsg_event(WT_DBG* ds, const char* fmt, ...) {
    WT_DECL_RET;
    WT_ITEM* msg;
    WT_SESSION_IMPL* session;
    size_t len, space;
 ...
 */
static int __dmsg_file(WT_DBG* ds, const char* fmt, ...) {
    WT_DECL_RET;
    va_list ap;
 ...
 */
static int __debug_config(WT_SESSION_IMPL* session, WT_DBG* ds, const char* ofile) {
    WT_BTREE* btree;
    WT_DECL_RET;
 ...
 */
static int __debug_wrapup(WT_DBG* ds) {
    WT_DECL_RET;
    WT_ITEM* msg;
    WT_SESSION_IMPL* session;
 ...
 */
int __wt_debug_addr_print(WT_SESSION_IMPL* session, const uint8_t* addr, size_t addr_size) {
    WT_DECL_ITEM(buf);
    WT_DECL_RET;
 ...
 */
int __wt_debug_offset_blind(WT_SESSION_IMPL* session, wt_off_t offset, const char* ofile) {
 ...
 */
int __wt_debug_offset(WT_SESSION_IMPL* session,
                      wt_off_t offset,
                      uint32_t size,
                      uint32_t checksum,
                      const char* ofile) {
 ...
 */
int __wt_debug_disk(WT_SESSION_IMPL* session, const WT_PAGE_HEADER* dsk, const char* ofile) {
    WT_DBG *ds, _ds;
    WT_DECL_RET;
 ...
 */
static int __debug_dsk_col_fix(WT_DBG* ds, const WT_PAGE_HEADER* dsk) {
    WT_BTREE* btree;
    uint32_t i;
    uint8_t v;
 ...
 */
static int __debug_dsk_cell(WT_DBG* ds, const WT_PAGE_HEADER* dsk) {
    WT_BTREE* btree;
 ...
 */
static char* __debug_tree_shape_info(WT_REF* ref, char* buf, size_t len) {
    WT_PAGE* page;
    uint64_t v;
    const char* unit;
    ...
    (void)__wt_snprintf(buf,
                        len,
                        "(%p, %" PRIu64 "%s, evict gen %" PRIu64 ", create gen %" PRIu64 ")",
                        (void*)ref,
                        v,
                        unit,
                        page->evict_pass_gen,
                        page->cache_create_gen);
    return (buf);
}
 ...
 */
static int __debug_tree_shape_worker(WT_DBG* ds, WT_REF* ref, int level) {
    WT_REF* walk;
    WT_SESSION_IMPL* session;
    char buf[128];
 ...
 */
int __wt_debug_tree_shape(WT_SESSION_IMPL* session, WT_REF* ref, const char* ofile) {
    WT_DBG *ds, _ds;
    WT_DECL_RET;
 ...
 */
int __wt_debug_tree_all(void* session_arg, WT_BTREE* btree, WT_REF* ref, const char* ofile) {
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
int __wt_debug_tree(void* session_arg, WT_BTREE* btree, WT_REF* ref, const char* ofile) {
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
int __wt_debug_page(void* session_arg, WT_BTREE* btree, WT_REF* ref, const char* ofile) {
    WT_DBG *ds, _ds;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
int __wt_debug_cursor_page(void* cursor_arg, const char* ofile) {
    WT_CURSOR* cursor;
    WT_CURSOR_BTREE* cbt;
 ...
 */
static int __debug_tree(WT_SESSION_IMPL* session, WT_REF* ref, const char* ofile, uint32_t flags) {
    WT_DBG *ds, _ds;
    WT_DECL_RET;
 ...
 */
static int __debug_page(WT_DBG* ds, WT_REF* ref, uint32_t flags) {
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
 ...
 */
static int __debug_page_metadata(WT_DBG* ds, WT_REF* ref) {
    WT_PAGE* page;
 ...
 */
static int __debug_page_col_fix(WT_DBG* ds, WT_REF* ref) {
    WT_BTREE* btree;
    WT_INSERT* ins;
    WT_PAGE* page;
 ...
 */
static int __debug_page_col_int(WT_DBG* ds, WT_PAGE* page, uint32_t flags) {
    WT_REF* ref;
    WT_SESSION_IMPL* session;
 ...
 */
static int __debug_page_col_var(WT_DBG* ds, WT_REF* ref) {
    WT_CELL* cell;
 ...
 */
static int __debug_page_row_int(WT_DBG* ds, WT_PAGE* page, uint32_t flags) {
    WT_REF* ref;
    WT_SESSION_IMPL* session;
    size_t len;
    void* p;
 ...
 */
static int __debug_page_row_leaf(WT_DBG* ds, WT_PAGE* page) {
    WT_CELL_UNPACK *unpack, _unpack;
    WT_DECL_ITEM(key);
    WT_DECL_RET;
 ...
 */
static int __debug_col_skip(WT_DBG* ds, WT_INSERT_HEAD* head, const char* tag, bool hexbyte) {
    WT_INSERT* ins;
 ...
 *	Dump an insert list.
 */
static int __debug_row_skip(WT_DBG* ds, WT_INSERT_HEAD* head) {
    WT_INSERT* ins;
 ...
 */
static int __debug_modify(WT_DBG* ds, WT_UPDATE* upd) {
 ...
 *	Dump an update list.
 */
static int __debug_update(WT_DBG* ds, WT_UPDATE* upd, bool hexbyte) {
 ...
 */
static int __debug_ref(WT_DBG* ds, WT_REF* ref) {
    WT_SESSION_IMPL* session;
    size_t addr_size;
    const uint8_t* addr;
    const char* state;
 ...
 */
static int __debug_cell(WT_DBG* ds, const WT_PAGE_HEADER* dsk, WT_CELL_UNPACK* unpack) {
    WT_DECL_ITEM(buf);
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
    ...
    switch (unpack->raw) {
        ...
        case WT_CELL_ADDR_LEAF_NO:
            ...
            WT_RET(ds->f(ds, ", ts %s,%s,%s", ts_string[0], ts_string[1], ts_string[2]));
            break;
        case WT_CELL_DEL:
        case WT_CELL_VALUE:
...
static int __debug_cell_data(
    WT_DBG* ds, WT_PAGE* page, int page_type, const char* tag, WT_CELL_UNPACK* unpack) {
    WT_DECL_ITEM(buf);
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
    const char* p;
