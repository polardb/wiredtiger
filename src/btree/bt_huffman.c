 ...
 */
static int __huffman_confchk_file(WT_SESSION_IMPL* session,
                                  WT_CONFIG_ITEM* v,
                                  bool* is_utf8p,
                                  WT_FSTREAM** fsp) {
    WT_DECL_RET;
 ...
 */
static int __huffman_confchk(WT_SESSION_IMPL* session, WT_CONFIG_ITEM* v) {
    if (v->len == 0)
        return (0);
 ...
 */
static int __wt_huffman_read(WT_SESSION_IMPL* session,
                             WT_CONFIG_ITEM* ip,
                             struct __wt_huffman_table** tablep,
                             u_int* entriesp,
                             u_int* numbytesp) {
