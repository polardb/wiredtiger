...
static int __slvg_trk_leaf_walk(WT_SESSION_IMPL*, const WT_PAGE_HEADER*, WT_TRACK*);
 ...
 */
static int __slvg_trk_init(WT_SESSION_IMPL* session,
                           const WT_PAGE_HEADER* dsk,
                           uint8_t* addr,
                           size_t addr_size,
                           WT_STUFF* ss,
    ...
    if (!__wt_process.page_version_ts || dsk->type == WT_PAGE_COL_FIX) {
        trk->trk_oldest_start_ts = trk->trk_newest_start_ts = WT_TS_NONE;
 ...
 *	Track a leaf page.
 */
static int __slvg_trk_leaf(WT_SESSION_IMPL* session,
                           const WT_PAGE_HEADER* dsk,
                           uint8_t* addr,
                           size_t addr_size,
                           WT_STUFF* ss) {
    ...
    switch (dsk->type) {
        ...
        case WT_PAGE_ROW_LEAF:
            ...
            WT_ERR(__wt_row_leaf_key_copy(
                session, page, &page->pg_row[page->entries - 1], &trk->row_stop));
            ...
            __wt_verbose(session,
                         ...
                         __wt_buf_set_printable(
                             session, trk->row_start.data, trk->row_start.size, ss->tmp2));
            __wt_verbose(
                session,
                WT_VERB_SALVAGE,
                "%s stop key %s",
 ...
 */
static int __slvg_trk_ovfl(WT_SESSION_IMPL* session,
                           const WT_PAGE_HEADER* dsk,
                           uint8_t* addr,
                           size_t addr_size,
                           WT_STUFF* ss) {
 ...
 */
static void __slvg_trk_leaf_ts(WT_TRACK* trk, WT_CELL_UNPACK* unpack) {
    trk->trk_oldest_start_ts = WT_MIN(unpack->start_ts, trk->trk_oldest_start_ts);
    trk->trk_newest_start_ts = WT_MAX(unpack->start_ts, trk->trk_newest_start_ts);
    trk->trk_newest_stop_ts = WT_MAX(unpack->stop_ts, trk->trk_newest_stop_ts);
}
 ...
 */
static int __slvg_trk_leaf_walk(WT_SESSION_IMPL* session,
                                const WT_PAGE_HEADER* dsk,
                                WT_TRACK* trk) {
    WT_BTREE* btree;
    WT_CELL_UNPACK unpack;
 ...
 */
static int __slvg_col_range_overlap(WT_SESSION_IMPL* session,
                                    uint32_t a_slot,
                                    uint32_t b_slot,
                                    WT_STUFF* ss) {
    WT_DECL_RET;
 ...
 */
static void __slvg_col_range_missing(WT_SESSION_IMPL* session, WT_STUFF* ss) {
    WT_TRACK* trk;
    uint64_t r;
    uint32_t i;
 ...
 */
static int __slvg_col_ovfl(WT_SESSION_IMPL* session,
                           WT_TRACK* trk,
                           WT_PAGE* page,
                           uint64_t recno,
                           uint64_t skip,
 ...
 */
static int __slvg_row_range_overlap(WT_SESSION_IMPL* session,
                                    uint32_t a_slot,
                                    uint32_t b_slot,
                                    WT_STUFF* ss) {
    WT_BTREE* btree;
 ...
 */
static int __slvg_row_trk_update_start(WT_SESSION_IMPL* session,
                                       WT_ITEM* stop,
                                       uint32_t slot,
                                       WT_STUFF* ss) {
    WT_BTREE* btree;
    ...
    for (refp = pindex->index, i = 0; i < ss->pages_next; ++i) {
        ...
        if (F_ISSET(trk, WT_TRACK_MERGE)) {
            ...
            WT_ERR(__wt_row_ikey_incr(
                session, page, 0, trk->row_start.data, trk->row_start.size, ref));
 ...
 */
static int __slvg_row_build_leaf(WT_SESSION_IMPL* session,
                                 WT_TRACK* trk,
                                 WT_REF* ref,
                                 WT_STUFF* ss) {
    WT_BTREE* btree;
 ...
 * referenced.
 */
static int __slvg_row_ovfl_single(WT_SESSION_IMPL* session, WT_TRACK* trk, WT_CELL_UNPACK* unpack) {
    WT_TRACK* ovfl;
    uint32_t i;
 ...
 */
static int WT_CDECL __slvg_trk_compare_addr(const void* a, const void* b) {
    WT_DECL_RET;
 ...
 */
static int __slvg_ovfl_ref(WT_SESSION_IMPL* session, WT_TRACK* trk, bool multi_panic) {
 ...
 */
static int __slvg_trk_free(WT_SESSION_IMPL* session, WT_TRACK** trkp, bool free_on_last_ref) {
    WT_TRACK* trk;
