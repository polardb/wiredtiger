 ...
 */
static inline void __cursor_state_save(WT_CURSOR* cursor, WT_CURFILE_STATE* state) {
 ...
 */
static inline void __cursor_state_restore(WT_CURSOR* cursor, WT_CURFILE_STATE* state) {
 ...
 */
static inline bool __cursor_page_pinned(WT_CURSOR_BTREE* cbt) {
    WT_CURSOR* cursor;
 ...
 */
static inline void __cursor_disable_bulk(WT_SESSION_IMPL* session, WT_BTREE* btree) {
    /*
 ...
 */
static inline bool __cursor_fix_implicit(WT_BTREE* btree, WT_CURSOR_BTREE* cbt) {
    /*
 ...
 */
int __wt_cursor_valid(WT_CURSOR_BTREE* cbt, WT_UPDATE** updp, bool* valid) {
    WT_BTREE* btree;
    WT_CELL* cell;
    WT_COL* cip;
    WT_PAGE* page;
 ...
 */
static inline int __cursor_col_search(WT_SESSION_IMPL* session,
                                      WT_CURSOR_BTREE* cbt,
                                      WT_REF* leaf) {
    WT_DECL_RET;
 ...
 */
static inline int __cursor_row_search(WT_SESSION_IMPL* session,
                                      WT_CURSOR_BTREE* cbt,
                                      WT_REF* leaf,
                                      bool insert) {
    WT_DECL_RET;
    ...
    WT_WITH_PAGE_INDEX(session,
                       ret = __wt_row_search(session, &cbt->iface.key, leaf, cbt, insert, false));
    return (ret);
}
 ...
 */
static inline int __cursor_col_modify_v(WT_SESSION_IMPL* session,
                                        WT_CURSOR_BTREE* cbt,
                                        WT_ITEM* value,
                                        u_int modify_type) {
 ...
 */
static inline int __cursor_row_modify_v(WT_SESSION_IMPL* session,
                                        WT_CURSOR_BTREE* cbt,
                                        WT_ITEM* value,
                                        u_int modify_type) {
 ...
 */
static inline int __cursor_col_modify(WT_SESSION_IMPL* session,
                                      WT_CURSOR_BTREE* cbt,
                                      u_int modify_type) {
    return (__wt_col_modify(
        session, cbt, cbt->iface.recno, &cbt->iface.value, NULL, modify_type, false));
}
 ...
 */
static inline int __cursor_row_modify(WT_SESSION_IMPL* session,
                                      WT_CURSOR_BTREE* cbt,
                                      u_int modify_type) {
 ...
 */
static void __cursor_restart(WT_SESSION_IMPL* session,
 ...
 */
int __wt_btcur_reset(WT_CURSOR_BTREE* cbt) {
    WT_CURSOR* cursor;
    WT_SESSION_IMPL* session;
 ...
 *	ones.
 */
int __wt_btcur_search_uncommitted(WT_CURSOR_BTREE* cbt, WT_UPDATE** updp) {
    WT_BTREE* btree;
    WT_CURSOR* cursor;
    WT_SESSION_IMPL* session;
    WT_UPDATE* upd;
 ...
 */
int __wt_btcur_insert_check(WT_CURSOR_BTREE* cbt) {
    WT_CURSOR* cursor;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
    uint64_t yield_count, sleep_usecs;
 ...
 */
int __wt_btcur_remove(WT_CURSOR_BTREE* cbt, bool positioned) {
    WT_BTREE* btree;
    WT_CURFILE_STATE state;
    WT_CURSOR* cursor;
    WT_DECL_RET;
 ...
 */
static int __btcur_update(WT_CURSOR_BTREE* cbt, WT_ITEM* value, u_int modify_type) {
    WT_BTREE* btree;
    WT_CURFILE_STATE state;
    WT_CURSOR* cursor;
    WT_DECL_RET;
 ...
 */
static bool __cursor_chain_exceeded(WT_CURSOR_BTREE* cbt) {
    WT_CURSOR* cursor;
    WT_PAGE* page;
    WT_SESSION_IMPL* session;
    WT_UPDATE* upd;
    ...
    if (upd != NULL && upd->type == WT_UPDATE_STANDARD && __wt_txn_upd_visible_all(session, upd) &&
 ...
 */
int __wt_btcur_modify(WT_CURSOR_BTREE* cbt, WT_MODIFY* entries, int nentries) {
    WT_CURFILE_STATE state;
    WT_CURSOR* cursor;
    WT_DECL_ITEM(modify);
    WT_DECL_RET;
 ...
 */
int __wt_btcur_reserve(WT_CURSOR_BTREE* cbt) {
    WT_CURSOR* cursor;
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
    bool overwrite;
 ...
 */
static inline bool __cursor_equals(WT_CURSOR_BTREE* a, WT_CURSOR_BTREE* b) {
    switch (a->btree->type) {
        case BTREE_COL_FIX:
        case BTREE_COL_VAR:
            /*
 ...
 * tree.
 */
static int __cursor_truncate(WT_SESSION_IMPL* session,
 ...
 */
static int __cursor_truncate_fix(WT_SESSION_IMPL* session,
                                 WT_CURSOR_BTREE* start,
                                 WT_CURSOR_BTREE* stop,
                                 int (*rmfunc)(WT_SESSION_IMPL*, WT_CURSOR_BTREE*, u_int)) {
    WT_DECL_RET;
 ...
 */
void __wt_btcur_init(WT_SESSION_IMPL* session, WT_CURSOR_BTREE* cbt) {
 ...
 *	Open a btree cursor.
 */
void __wt_btcur_open(WT_CURSOR_BTREE* cbt) {
 ...
 */
int __wt_btcur_close(WT_CURSOR_BTREE* cbt, bool lowlevel) {
    WT_DECL_RET;
    WT_SESSION_IMPL* session;
