 ...
 */
int __wt_verify_dsk_image(WT_SESSION_IMPL* session,
                          const char* tag,
 ...
 */
int __wt_verify_dsk(WT_SESSION_IMPL* session, const char* tag, WT_ITEM* buf) {
 ...
 */
static int __verify_dsk_ts_addr_cmp(WT_SESSION_IMPL* session,
                                    uint32_t cell_num,
                                    const char* ts1_name,
                                    wt_timestamp_t ts1,
                                    const char* ts2_name,
 ...
 */
static int __verify_dsk_ts(WT_SESSION_IMPL* session,
    ...
    switch (unpack->type) {
        ...
        case WT_CELL_ADDR_LEAF_NO:
            ...
            if (unpack->newest_start_ts > unpack->newest_stop_ts)
                WT_RET_VRFY(session,
                            "cell %" PRIu32
                            " on page at %s has a newest start "
            ...
            if (addr == NULL)
                break;
            WT_RET(__verify_dsk_ts_addr_cmp(session,
                                            cell_num - 1,
                                            "oldest start",
 ...
 */
static int __verify_dsk_row(WT_SESSION_IMPL* session,
                            const char* tag,
                            const WT_PAGE_HEADER* dsk,
                            WT_ADDR* addr) {
    WT_BM* bm;
    ...
    if (dsk->type == WT_PAGE_ROW_INT && key_cnt * 2 != dsk->u.entries)
        WT_ERR_VRFY(session,
                    "%s page at %s has a key count of %" PRIu32
                    " and a "
 ...
 */
static int __verify_dsk_col_int(WT_SESSION_IMPL* session,
                                const char* tag,
                                const WT_PAGE_HEADER* dsk,
                                WT_ADDR* addr) {
    WT_BM* bm;
 ...
 */
static int __verify_dsk_col_fix(WT_SESSION_IMPL* session,
                                const char* tag,
                                const WT_PAGE_HEADER* dsk) {
    WT_BTREE* btree;
    uint32_t datalen;
 ...
 */
static int __verify_dsk_col_var(WT_SESSION_IMPL* session,
                                const char* tag,
                                const WT_PAGE_HEADER* dsk,
                                WT_ADDR* addr) {
    struct {
 ...
 */
static int __verify_dsk_memsize(WT_SESSION_IMPL* session,
                                const char* tag,
                                const WT_PAGE_HEADER* dsk,
                                WT_CELL* cell) {
    size_t len;
 ...
 */
static int __verify_dsk_chunk(WT_SESSION_IMPL* session,
                              const char* tag,
                              const WT_PAGE_HEADER* dsk,
                              uint32_t datalen) {
    WT_BTREE* btree;
 ...
 */
static int __err_cell_corrupt(WT_SESSION_IMPL* session,
                              int retval,
                              uint32_t entry_num,
                              const char* tag) {
    WT_RET_VRFY_RETVAL(
 ...
 */
static int __err_cell_corrupt_or_eof(WT_SESSION_IMPL* session,
                                     int retval,
                                     uint32_t entry_num,
                                     const char* tag) {
    WT_RET_VRFY_RETVAL(session,
                       retval,
                       "item %" PRIu32
 ...
 */
static int __err_cell_type(WT_SESSION_IMPL* session,
                           uint32_t entry_num,
                           const char* tag,
                           uint8_t cell_type,
                           uint8_t dsk_type) {
    ...
    WT_RET_VRFY(session,
                ...
                entry_num,
                tag,
                __wt_cell_type_string(cell_type),
                __wt_page_type_string(dsk_type));
}
