...
int __wt_bt_read(WT_SESSION_IMPL* session, WT_ITEM* buf, const uint8_t* addr, size_t addr_size) {
    ...
    if (F_ISSET(dsk, WT_PAGE_COMPRESSED)) {
        ...
        ret = btree->compressor->decompress(btree->compressor,
                                            &session->iface,
 ...
 * checksum.
 */
int __wt_bt_write(WT_SESSION_IMPL* session,
                  WT_ITEM* buf,
                  uint8_t* addr,
                  size_t* addr_sizep,
    ...
    if (btree->compressor == NULL || btree->compressor->compress == NULL || compressed)
        ...
        if (btree->compressor->pre_size == NULL)
            ...
            WT_ERR(btree->compressor->pre_size(
                btree->compressor, &session->iface, src, src_len, &len));
