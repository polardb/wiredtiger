 ...
 */
int __wt_page_alloc(WT_SESSION_IMPL* session,
                    uint8_t type,
 ...
 */
static void __inmem_col_var_repeats(WT_SESSION_IMPL* session, WT_PAGE* page, uint32_t* np) {
    WT_BTREE* btree;
    WT_CELL_UNPACK unpack;
 ...
 *	column-store trees.
 */
static int __inmem_col_var(WT_SESSION_IMPL* session, WT_PAGE* page, uint64_t recno, size_t* sizep) {
    WT_BTREE* btree;
    WT_CELL_UNPACK unpack;
    WT_COL* cip;
    WT_COL_RLE* repeats;
 ...
 */
static int __inmem_row_leaf_entries(WT_SESSION_IMPL* session,
                                    const WT_PAGE_HEADER* dsk,
                                    uint32_t* nindxp) {
    WT_BTREE* btree;
    WT_CELL_UNPACK unpack;
