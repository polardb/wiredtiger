 ...
 */
static void __rebalance_discard(WT_SESSION_IMPL* session, WT_REBALANCE_STUFF* rs) {
 ...
 */
static int __rebalance_leaf_append(WT_SESSION_IMPL* session,
                                   const uint8_t* key,
                                   size_t key_len,
 ...
 */
static int __rebalance_fl_append(WT_SESSION_IMPL* session,
                                 const uint8_t* addr,
                                 size_t addr_len,
                                 WT_REBALANCE_STUFF* rs) {
    WT_ADDR* copy;
 ...
 * found.
 */
static int __rebalance_internal(WT_SESSION_IMPL* session, WT_REBALANCE_STUFF* rs) {
    WT_BTREE* btree;
    WT_DECL_RET;
    WT_PAGE* page;
 ...
 */
static int __rebalance_free_original(WT_SESSION_IMPL* session, WT_REBALANCE_STUFF* rs) {
    WT_ADDR* addr;
    uint64_t i;
 ...
 */
static int __rebalance_col_walk(WT_SESSION_IMPL* session,
 ...
 */
static int __rebalance_row_leaf_key(WT_SESSION_IMPL* session,
                                    const uint8_t* addr,
                                    size_t addr_len,
                                    WT_ITEM* key,
                                    WT_REBALANCE_STUFF* rs) {
 ...
 */
static int __rebalance_row_walk(WT_SESSION_IMPL* session,
                                const WT_PAGE_HEADER* dsk,
                                WT_REBALANCE_STUFF* rs) {
    WT_BTREE* btree;
    ...
    WT_CELL_FOREACH_BEGIN(session, btree, dsk, unpack, true) {
        switch (unpack.type) {
            ...
            case WT_CELL_ADDR_LEAF_NO:
                ...
                if (first_cell) {
                    WT_ERR(
                        __rebalance_row_leaf_key(session, unpack.data, unpack.size, leafkey, rs));
                    p = leafkey->data;
                    len = leafkey->size;
 ...
 */
int __wt_bt_rebalance(WT_SESSION_IMPL* session, const char* cfg[]) {
    WT_BTREE* btree;
    WT_DECL_RET;
