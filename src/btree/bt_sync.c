 ...
 */
static inline bool __sync_checkpoint_can_skip(WT_SESSION_IMPL* session, WT_PAGE* page) {
    WT_MULTI* multi;
    WT_PAGE_MODIFY* mod;
    WT_TXN* txn;
    u_int i;
 ...
 */
static inline int __sync_dup_walk(WT_SESSION_IMPL* session,
                                  WT_REF* walk,
                                  uint32_t flags,
                                  WT_REF** dupp) {
    WT_REF* old;
 ...
 */
int __wt_sync_file(WT_SESSION_IMPL* session, WT_CACHE_OP syncop) {
    WT_BTREE* btree;
