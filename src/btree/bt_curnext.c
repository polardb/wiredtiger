 ...
 */
static inline int __cursor_fix_append_next(WT_CURSOR_BTREE* cbt, bool newpage, bool restart) {
 ...
 */
static inline int __cursor_fix_next(WT_CURSOR_BTREE* cbt, bool newpage, bool restart) {
    WT_BTREE* btree;
    WT_PAGE* page;
    WT_SESSION_IMPL* session;
    WT_UPDATE* upd;
 ...
 */
static inline int __cursor_var_append_next(WT_CURSOR_BTREE* cbt, bool newpage, bool restart) {
    WT_SESSION_IMPL* session;
    WT_UPDATE* upd;
 ...
 */
static inline int __cursor_var_next(WT_CURSOR_BTREE* cbt, bool newpage, bool restart) {
    WT_CELL* cell;
 ...
 */
static inline int __cursor_row_next(WT_CURSOR_BTREE* cbt, bool newpage, bool restart) {
    WT_INSERT* ins;
    WT_ITEM* key;
    WT_PAGE* page;
    WT_ROW* rip;
 ...
 */
static int __cursor_key_order_check_col(WT_SESSION_IMPL* session, WT_CURSOR_BTREE* cbt, bool next) {
    int cmp;
    ...
    WT_PANIC_RET(session,
                 EINVAL,
                 "WT_CURSOR.%s out-of-order returns: returned key %" PRIu64
                 " then "
                 "key %" PRIu64,
                 next ? "next" : "prev",
                 cbt->lastrecno,
 ...
 */
static int __cursor_key_order_check_row(WT_SESSION_IMPL* session, WT_CURSOR_BTREE* cbt, bool next) {
    WT_BTREE* btree;
    WT_DECL_ITEM(a);
    WT_DECL_ITEM(b);
    WT_DECL_RET;
 ...
 */
int __wt_cursor_key_order_check(WT_SESSION_IMPL* session, WT_CURSOR_BTREE* cbt, bool next) {
 ...
 * search.
 */
int __wt_cursor_key_order_init(WT_SESSION_IMPL* session, WT_CURSOR_BTREE* cbt) {
    /*
 ...
 */
void __wt_cursor_key_order_reset(WT_CURSOR_BTREE* cbt) {
    /*
 ...
 */
void __wt_btcur_iterate_setup(WT_CURSOR_BTREE* cbt) {
    WT_PAGE* page;
 ...
 */
int __wt_btcur_next(WT_CURSOR_BTREE* cbt, bool truncating) {
    WT_CURSOR* cursor;
    WT_DECL_RET;
    WT_PAGE* page;
    WT_SESSION_IMPL* session;
