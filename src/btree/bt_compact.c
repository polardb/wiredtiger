 ...
 */
static int __compact_rewrite(WT_SESSION_IMPL* session, WT_REF* ref, bool* skipp) {
    WT_BM* bm;
    WT_MULTI* multi;
    WT_PAGE* page;
    WT_PAGE_MODIFY* mod;
 ...
 */
static int __compact_rewrite_lock(WT_SESSION_IMPL* session, WT_REF* ref, bool* skipp) {
    WT_BTREE* btree;
    WT_DECL_RET;
 ...
 */
static void __compact_progress(WT_SESSION_IMPL* session) {
 ...
 *	Compact a file.
 */
int __wt_compact(WT_SESSION_IMPL* session) {
    WT_BM* bm;
    WT_DECL_RET;
    WT_REF* ref;
    u_int i;
 ...
 */
int __wt_compact_page_skip(WT_SESSION_IMPL* session, WT_REF* ref, void* context, bool* skipp) {
    WT_BM* bm;
    WT_DECL_RET;
    size_t addr_size;
    const uint8_t* addr;
