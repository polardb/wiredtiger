 ...
 */
static int __col_instantiate(WT_SESSION_IMPL* session,
                             uint64_t recno,
                             WT_REF* ref,
                             WT_CURSOR_BTREE* cbt,
                             WT_UPDATE* updlist) {
 ...
 */
static void __las_page_instantiate_verbose(WT_SESSION_IMPL* session, uint64_t las_pageid) {
    WT_CACHE* cache;
 ...
 */
static int __las_page_instantiate(WT_SESSION_IMPL* session, WT_REF* ref) {
    WT_CACHE* cache;
    WT_CURSOR* cursor;
    WT_CURSOR_BTREE cbt;
 ...
 */
static bool __evict_force_check(WT_SESSION_IMPL* session, WT_REF* ref) {
    WT_BTREE* btree;
    WT_PAGE* page;
    size_t footprint;
 ...
 *	page access.
 */
static inline int __page_read_lookaside(WT_SESSION_IMPL* session,
                                        WT_REF* ref,
                                        uint32_t previous_state,
                                        uint32_t* final_statep) {
    /*
 ...
 */
int __wt_page_in_func(WT_SESSION_IMPL* session,
                      WT_REF* ref,
                      uint32_t flags
