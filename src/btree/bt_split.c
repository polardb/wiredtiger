 ...
 */
static void __split_verify_intl_key_order(WT_SESSION_IMPL* session, WT_PAGE* page) {
    WT_BTREE* btree;
 ...
 */
static int __split_verify_root(WT_SESSION_IMPL* session, WT_PAGE* page) {
    WT_DECL_RET;
    WT_REF* ref;
    uint32_t read_flags;
 ...
 * information.
 */
static int __split_ref_move(WT_SESSION_IMPL* session,
                            WT_PAGE* from_home,
                            WT_REF** from_refp,
                            size_t* decrp,
                            WT_REF** to_refp,
 ...
 */
static void __split_ref_final(WT_SESSION_IMPL* session, WT_PAGE*** lockedp) {
    WT_PAGE** locked;
    size_t i;
 ...
 */
static int __split_ref_prepare(WT_SESSION_IMPL* session,
 ...
 */
static int __split_root(WT_SESSION_IMPL* session, WT_PAGE* root) {
    WT_BTREE* btree;
    WT_DECL_RET;
    WT_PAGE *child, **locked;
    ...
    __wt_verbose(session,
                 WT_VERB_SPLIT,
                 "%p: %" PRIu32 " root page elements, splitting into %" PRIu32 " children",
                 (void*)root,
                 pindex->entries,
                 children);
 ...
 */
static int __split_parent(WT_SESSION_IMPL* session,
                          WT_REF* ref,
                          WT_REF** ref_new,
    ...
    for (deleted_entries = 0, i = 0; i < parent_entries; ++i) {
        ...
        if ((discard && next_ref == ref) ||
            ((!WT_BTREE_SYNCING(btree) || WT_SESSION_BTREE_SYNC(session)) &&
             ...
             __wt_atomic_casv32(&next_ref->state, WT_REF_DELETED, WT_REF_SPLIT))) {
    ...
    if (ref->page == NULL)
        ...
        __wt_verbose(session,
                     WT_VERB_SPLIT,
                     "%p: split into parent %p, %" PRIu32 " -> %" PRIu32 " (+%" PRIu32 ")",
                     (void*)ref->page,
                     (void*)parent,
                     parent_entries,
                     result_entries,
 ...
 */
static int __split_internal(WT_SESSION_IMPL* session, WT_PAGE* parent, WT_PAGE* page) {
    WT_BTREE* btree;
    WT_DECL_RET;
    WT_PAGE *child, **locked;
 ...
 */
static int __split_internal_lock(WT_SESSION_IMPL* session,
                                 WT_REF* ref,
                                 bool trylock,
                                 WT_PAGE** parentp) {
    WT_PAGE* parent;
 ...
 */
static void __split_internal_unlock(WT_SESSION_IMPL* session, WT_PAGE* parent) {
    WT_PAGE_UNLOCK(session, parent);
}
 ...
 */
static bool __split_internal_should_split(WT_SESSION_IMPL* session, WT_REF* ref) {
    WT_BTREE* btree;
    WT_PAGE* page;
    WT_PAGE_INDEX* pindex;
 ...
 */
static int __split_parent_climb(WT_SESSION_IMPL* session, WT_PAGE* page) {
    WT_DECL_RET;
    WT_PAGE* parent;
    WT_REF* ref;
 ...
 */
static int __split_multi_inmem(WT_SESSION_IMPL* session,
                               WT_PAGE* orig,
                               WT_MULTI* multi,
                               WT_REF* ref) {
    WT_CURSOR_BTREE cbt;
 ...
 */
static void __split_multi_inmem_final(WT_PAGE* orig, WT_MULTI* multi) {
    WT_SAVE_UPD* supd;
    uint32_t i, slot;
 ...
 */
static void __split_multi_inmem_fail(WT_SESSION_IMPL* session, WT_PAGE* orig, WT_REF* ref) {
    /*
 ...
 */
int __wt_multi_to_ref(WT_SESSION_IMPL* session,
                      WT_PAGE* page,
                      WT_MULTI* multi,
                      WT_REF** refp,
                      size_t* incrp,
     ...
     */
    WT_ASSERT(session,
              multi->page_las.las_pageid != 0 || multi->addr.addr != NULL ||
                  multi->disk_image != NULL);
    ...
    WT_ASSERT(session,
              multi->disk_image == NULL ||
                  __wt_verify_dsk_image(
                      session, "[page instantiate]", multi->disk_image, 0, &multi->addr, true) ==
                      0);
 ...
 */
static int __split_insert(WT_SESSION_IMPL* session, WT_REF* ref) {
    WT_DECL_ITEM(key);
    WT_DECL_RET;
 ...
 *	Split a page's last insert list entries into a separate page.
 */
static int __split_insert_lock(WT_SESSION_IMPL* session, WT_REF* ref) {
    WT_DECL_RET;
    WT_PAGE* parent;
 ...
 *	Split a page's last insert list entries into a separate page.
 */
int __wt_split_insert(WT_SESSION_IMPL* session, WT_REF* ref) {
    WT_DECL_RET;
 ...
 */
static int __split_multi(WT_SESSION_IMPL* session, WT_REF* ref, bool closing) {
    WT_DECL_RET;
    WT_PAGE* page;
    WT_PAGE_MODIFY* mod;
    WT_REF** ref_new;
    ...
    for (i = 0; i < new_entries; ++i)
        WT_ERR(__wt_multi_to_ref(
            session, page, &mod->mod_multi[i], &ref_new[i], &parent_incr, closing));
 ...
 *	Split a page into multiple pages.
 */
static int __split_multi_lock(WT_SESSION_IMPL* session, WT_REF* ref, int closing) {
    WT_DECL_RET;
    WT_PAGE* parent;
 ...
 *	Split a page into multiple pages.
 */
int __wt_split_multi(WT_SESSION_IMPL* session, WT_REF* ref, int closing) {
    WT_DECL_RET;
 ...
 */
static int __split_reverse(WT_SESSION_IMPL* session, WT_REF* ref) {
    WT_DECL_RET;
    WT_PAGE* parent;
 ...
 *	Reverse split (rewrite a parent page's index to reflect an empty page).
 */
int __wt_split_reverse(WT_SESSION_IMPL* session, WT_REF* ref) {
    WT_DECL_RET;
 ...
 */
int __wt_split_rewrite(WT_SESSION_IMPL* session, WT_REF* ref, WT_MULTI* multi) {
    WT_DECL_RET;
    WT_PAGE* page;
    WT_REF* new;
