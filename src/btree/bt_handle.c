 ...
 */
void __wt_btree_page_version_config(WT_SESSION_IMPL* session) {
 ...
 */
static int __btree_clear(WT_SESSION_IMPL* session) {
    WT_BTREE* btree;
    WT_DECL_RET;
...
int __wt_btree_open(WT_SESSION_IMPL* session, const char* op_cfg[]) {
     ...
     */
    if (btree->original || F_ISSET(btree,
                                   WT_BTREE_IN_MEMORY | WT_BTREE_REBALANCE | WT_BTREE_SALVAGE |
                                       WT_BTREE_UPGRADE | WT_BTREE_VERIFY)) {
 ...
 *	Discard a Btree.
 */
int __wt_btree_discard(WT_SESSION_IMPL* session) {
    WT_BTREE* btree;
    WT_DECL_RET;
 ...
 */
static int __btree_conf(WT_SESSION_IMPL* session, WT_CKPT* ckpt) {
    WT_BTREE* btree;
    ...
    if (btree->type == BTREE_ROW) {
        ...
        if (cval.len != 0) {
            ...
            WT_RET(__wt_collator_config(session,
                                        btree->dhandle->name,
                                        &cval,
                                        &metadata,
                                        &btree->collator,
                                        &btree->collator_owned));
        }
    ...
    if (cval.len == 0)
        ...
        ret = __wt_encryptor_config(
            session, &cval, &keyid, (WT_CONFIG_ARG*)enc_cfg, &btree->kencryptor);
 ...
 */
void __wt_root_ref_init(WT_REF* root_ref, WT_PAGE* root, bool is_recno) {
...
int __wt_btree_tree_open(WT_SESSION_IMPL* session, const uint8_t* addr, size_t addr_size) {
    ...
    if (ret != 0 && WT_IS_METADATA(session->dhandle)) {
        __wt_errx(session, "WiredTiger has failed to open its metadata");
        __wt_errx(session,
    ...
    WT_ERR(__wt_page_inmem(session,
                           NULL,
                           dsk.data,
                           WT_DATA_IN_ITEM(&dsk) ? WT_PAGE_DISK_ALLOC : WT_PAGE_DISK_MAPPED,
                           &page));
    dsk.mem = NULL;
 ...
 */
static int __btree_tree_open_empty(WT_SESSION_IMPL* session, bool creation) {
    WT_BTREE* btree;
    WT_DECL_RET;
    WT_PAGE *leaf, *root;
 ...
 */
bool __wt_btree_immediately_durable(WT_SESSION_IMPL* session) {
    WT_BTREE* btree;
