 ...
 */
int __wt_delete_page(WT_SESSION_IMPL* session, WT_REF* ref, bool* skipp) {
    WT_ADDR* ref_addr;
    WT_DECL_RET;
 ...
 */
int __wt_delete_page_rollback(WT_SESSION_IMPL* session, WT_REF* ref) {
    WT_UPDATE** updp;
    ...
    for (locked = false, sleep_usecs = yield_count = 0;;) {
        switch (current_state = ref->state) {
            case WT_REF_DELETED:
                 ...
                 * reset the state.
                 */
                if (__wt_atomic_casv32(&ref->state, WT_REF_DELETED, ref->page_del->previous_state))
                    goto done;
                break;
            case WT_REF_LOCKED:
                /*
            ...
            case WT_REF_MEM:
            case WT_REF_SPLIT:
                if (__wt_atomic_casv32(&ref->state, current_state, WT_REF_LOCKED))
                    locked = true;
                break;
            case WT_REF_DISK:
            case WT_REF_LIMBO:
 ...
 */
bool __wt_delete_page_skip(WT_SESSION_IMPL* session, WT_REF* ref, bool visible_all) {
    bool skip;
 ...
 */
static int __tombstone_update_alloc(WT_SESSION_IMPL* session,
