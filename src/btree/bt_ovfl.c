 ...
 */
static int __ovfl_read(WT_SESSION_IMPL* session,
                       const uint8_t* addr,
                       size_t addr_size,
                       WT_ITEM* store) {
    WT_BTREE* btree;
 ...
 */
int __wt_ovfl_read(WT_SESSION_IMPL* session,
                   WT_PAGE* page,
 ...
 */
void __wt_ovfl_discard_remove(WT_SESSION_IMPL* session, WT_PAGE* page) {
    WT_OVFL_TRACK* track;
    uint32_t i;
 ...
 */
int __wt_ovfl_remove(WT_SESSION_IMPL* session,
                     WT_PAGE* page,
                     WT_CELL_UNPACK* unpack,
                     bool evicting) {
    /*
 ...
 */
int __wt_ovfl_discard(WT_SESSION_IMPL* session, WT_PAGE* page, WT_CELL* cell) {
    WT_BM* bm;
    WT_BTREE* btree;
