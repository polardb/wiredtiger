...
int __wt_thread_join(WT_SESSION_IMPL* session, wt_thread_t* tid) {
    ...
    if (CloseHandle(tid->id) == 0) {
        windows_error = __wt_getlasterror();
        __wt_errx(
            session, "thread join: CloseHandle: %s", __wt_formatmessage(session, windows_error));
 ...
 */
void __wt_thread_id(uintmax_t* id) {
 ...
 */
int __wt_thread_str(char* buf, size_t buflen) {
    return (__wt_snprintf(buf,
                          buflen,
                          "%" PRIu64 ":%" PRIu64,
