 ...
 */
static int __win_fs_exist(WT_FILE_SYSTEM* file_system,
 ...
 *	Remove a file.
 */
static int __win_fs_remove(WT_FILE_SYSTEM* file_system,
                           WT_SESSION* wt_session,
                           const char* name,
                           uint32_t flags) {
    WT_DECL_ITEM(name_wide);
 ...
 *	Rename a file.
 */
static int __win_fs_rename(WT_FILE_SYSTEM* file_system,
                           WT_SESSION* wt_session,
                           const char* from,
                           const char* to,
                           uint32_t flags) {
     ...
     * modern filesystem.
     */
    WT_WINCALL_RETRY(MoveFileExW(from_wide->data,
                                 to_wide->data,
 ...
 */
int __wt_win_fs_size(WT_FILE_SYSTEM* file_system,
                     WT_SESSION* wt_session,
                     const char* name,
                     wt_off_t* sizep) {
    DWORD windows_error;
 ...
 *	ANSI C close.
 */
static int __win_file_close(WT_FILE_HANDLE* file_handle, WT_SESSION* wt_session) {
    WT_DECL_RET;
 ...
 *	Lock/unlock a file.
 */
static int __win_file_lock(WT_FILE_HANDLE* file_handle, WT_SESSION* wt_session, bool lock) {
    WT_DECL_RET;
    WT_FILE_HANDLE_WIN* win_fh;
    WT_SESSION_IMPL* session;
    DWORD windows_error;
 ...
 */
static int __win_file_size(WT_FILE_HANDLE* file_handle, WT_SESSION* wt_session, wt_off_t* sizep) {
    DWORD windows_error;
    LARGE_INTEGER size;
    WT_DECL_RET;
    WT_FILE_HANDLE_WIN* win_fh;
 ...
 *	MSVC fsync.
 */
static int __win_file_sync(WT_FILE_HANDLE* file_handle, WT_SESSION* wt_session) {
    WT_DECL_RET;
    WT_FILE_HANDLE_WIN* win_fh;
    WT_SESSION_IMPL* session;
    DWORD windows_error;
 ...
 */
static int __win_file_set_end(WT_FILE_HANDLE* file_handle, WT_SESSION* wt_session, wt_off_t len) {
    DWORD windows_error;
 ...
 *	Write a chunk.
 */
static int __win_file_write(WT_FILE_HANDLE* file_handle,
                            WT_SESSION* wt_session,
                            wt_off_t offset,
                            size_t len,
                            const void* buf) {
 ...
 *	Open a file handle.
 */
static int __win_open_file(WT_FILE_SYSTEM* file_system,
                           WT_SESSION* wt_session,
                           const char* name,
 ...
 */
static int __win_terminate(WT_FILE_SYSTEM* file_system, WT_SESSION* wt_session) {
    WT_SESSION_IMPL* session;
 ...
 */
int __wt_os_win(WT_SESSION_IMPL* session) {
    WT_CONNECTION_IMPL* conn;
